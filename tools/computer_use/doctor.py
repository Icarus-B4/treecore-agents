"""
`treecore computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
def run_doctor(
    ...
    *,
    ...
    Honors `TREECORE_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Hermes identity under treecore_identity so existing parsers
        # that only read overall/checks keep working.
        payload = dict(report)
        payload["treecore_identity"] = identity
