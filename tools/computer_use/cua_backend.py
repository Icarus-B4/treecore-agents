...
etc.) surface as specific blocked checks via `treecore computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and NousResearch/treecore-agent#67052.
    """
...
# fetches the latest release, so a `TREECORE_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `TREECORE_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "TREECORE_CUA_DRIVER_CMD"
...
class _EmbeddedCuaDaemon:
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\treecore-cua-{token}"
        else:
    ...
    def start(self) -> None:
        ...
        self._stderr_thread = threading.Thread(
            ...
            args=(self._process,),
            name="treecore-cua-daemon-stderr",
            daemon=True,
        )
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/treecore-agent#47072: instead of hardcoding
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``TREECORE_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    tool even though `treecore computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``TREECORE_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `treecore computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  treecore computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `treecore tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/treecore-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of NousResearch/treecore-agent#47072: per-tool
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `treecore computer-use doctor` and check "
                f"{display_treecore_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/treecore-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of NousResearch/treecore-agent#47072: per-snapshot
        ...
        self._session_id: str = f"treecore-{uuid.uuid4().hex[:12]}"
    ...
    def capture(
        ...
        Maps treecore `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/treecore-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/treecore-agent#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See NousResearch/treecore-agent#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/treecore-agent#47072). cua-driver-rs gained an explicit
