...
`health_report` (surfaced by `treecore computer-use doctor`) reports the
...
# See NousResearch/treecore-agent#67052 gap 4.
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "TREECORE_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown TREECORE_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `treecore computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NousResearch/treecore-agent#67052).
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/treecore-agent#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$TREECORE_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_treecore_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    `treecore computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
