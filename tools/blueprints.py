...
`treecore skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.treecore.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.treecore.blueprint`` is a mapping containing
    ...
    treecore = meta.get("treecore") if isinstance(meta, dict) else None
    blueprint = treecore.get("blueprint") if isinstance(treecore, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.treecore.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.treecore.blueprint`` block) they can hand
    to ``treecore skills publish`` to share. ``body`` is the plain-language
