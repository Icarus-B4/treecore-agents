...
(``treecore-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``treecore tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``TREECORE_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/treecore`` read-only), setting
  ``TREECORE_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (TREECORE_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (treecore trace upload / /upload-trace).
    #
    ...
    # presence, the `treecore update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "TREECORE_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``TREECORE_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("TREECORE_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``treecore update`` lazy-refresh pass.
    """
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``treecore update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="treecore-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``treecore_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = treecore_subprocess_env(inherit_credentials=False)
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # deployment sets TREECORE_MANAGED=true *and* TREECORE_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        try:
            from treecore_cli.config import get_managed_system
...
def feature_install_command(feature: str, *, venv_pip: bool = False) -> Optional[str]:
    ...
    (default install, ``TREECORE_HOME`` overrides, profile installs) and
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``TREECORE_DISABLE_LAZY_INSTALLS=1``) and sets
      ``TREECORE_LAZY_INSTALL_TARGET``, installs are redirected to the writable
...
def active_features() -> list[str]:
    ...
    Used by ``treecore update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``treecore update``. Never raises; lazy-install failures
