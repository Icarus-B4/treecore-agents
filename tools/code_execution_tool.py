...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `treecore_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `treecore_tools.py` with file-based RPC stubs
...
# match a safe prefix, the operational TREECORE_ allowlist, or (on Windows) an
...
# TREECORE_HOME.
#
# NB: the broad "TREECORE_" prefix was deliberately removed (#27303) — it leaked
# TREECORE_*-named config that lacks a secret substring (e.g. TREECORE_BASE_URL,
# TREECORE_KANBAN_DB, TREECORE_*_WEBHOOK).  The child only needs the few
# location/profile vars in _TREECORE_CHILD_ALLOWED below; TREECORE_RPC_SOCKET /
# TREECORE_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational TREECORE_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set treecore_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_TREECORE_CHILD_ALLOWED = frozenset({
    "TREECORE_HOME",
    "TREECORE_PROFILE",
    "TREECORE_CONFIG",
    "TREECORE_ENV",
    "TREECORE_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational TREECORE_* vars (_TREECORE_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret TREECORE_* vars dropped by the tightened allowlist (#27303). The
    # broad "TREECORE_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like TREECORE_KANBAN_DB / TREECORE_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _TREECORE_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("TREECORE_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped TREECORE_* var.
            _dropped_treecore.append(k)
    if _dropped_treecore:
        logger.debug(
            "execute_code: dropped %d non-allowlisted TREECORE_* var(s) from the "
            ...
            len(_dropped_treecore),
            ", ".join(sorted(_dropped_treecore)),
        )
...
# ---------------------------------------------------------------------------
# treecore_tools.py code generator
# ---------------------------------------------------------------------------
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    treecore_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'treecore_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_treecore_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the treecore_tools.py stub module.
...
def _connect():
    ...
    TREECORE_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["TREECORE_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("TREECORE_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
_RPC_DIR = os.environ.get("TREECORE_RPC_DIR") or os.path.join(tempfile.gettempdir(), "treecore_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated treecore_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/treecore_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_treecore_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/treecore_tools.py", tools_src)
        ...
        env_prefix = (
            f"TREECORE_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"TREECORE_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("TREECORE_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with treecore_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="treecore_sandbox_")
    ...
    # selector from TREECORE_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"treecore_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated treecore_tools module.
        ...
        tools_src = generate_treecore_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "treecore_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  TREECORE_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["TREECORE_RPC_SOCKET"] = rpc_endpoint
        child_env["TREECORE_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the treecore-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from treecore_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _treecore_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _treecore_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # TREECORE_TIMEZONE is an internal Hermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("TREECORE_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("TREECORE_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.treecore/.env')).
...
def _load_config() -> dict:
    ...
    ``treecore --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (treecore-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``treecore tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and treecore-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.treecore/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        ...
        f"Available via `from treecore_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from treecore_tools import {import_str}` "
