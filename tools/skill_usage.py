...
Tracks per-skill usage metadata in a sidecar JSON file (~/.treecore/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.treecore/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.treecore/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.treecore/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``treecore update`` would
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.treecore/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``treecore curator
    list-archived`` to help users pass a name to ``treecore curator restore``.
    """
...
def _is_curator_managed_record(record: Any) -> bool:
    ...
    * management = "may autonomous curation mutate/archive this" — a policy
      decision the user can change at any time via ``treecore curator adopt``.
...
def list_unmanaged_skill_names() -> List[str]:
    ...
    * It was created by a FOREGROUND ``skill_manage(action="create")`` call,
    ...
    every automatic transition. ``treecore curator status`` surfaces this count
    so the blind spot is legible instead of silent, and ``treecore curator
...
def _emit_skill_lifecycle(
    ...
    *,
    ...
    try:
        from treecore_cli.lifecycle import has_hook, invoke_hook
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.treecore/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.treecore/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.treecore/skills/<skill>/SKILL.md) and category-nested
    (~/.treecore/skills/<category>/<skill>/SKILL.md) layouts. Uses the gated
