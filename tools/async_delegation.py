...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``TREECORE_SESSION_ID`` via ``get_session_env`` — is
    ...
    The request-scoped ``TREECORE_SESSION_CHAT_ID`` binding survives child
    ...
    try:
        ...
        if get_session_env("TREECORE_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("TREECORE_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
...
def dispatch_async_delegation(
    *,
    ...
    try:
        ...
        # get_treecore_home() under the right profile.
