...
provider** — turn on a backend by enabling a plugin (``treecore plugins
enable video_gen/<name>``) and selecting it in ``treecore tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/treecore-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `treecore tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `treecore plugins list` to see "
            f"installed video gen backends, or `treecore tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `treecore tools` → "
...
# `treecore tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`treecore tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `treecore tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
