...
transport, discovers their tools, and registers them into the treecore-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.treecore/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.treecore/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            treecore_home = os.path.expanduser(
                os.getenv(
                    "TREECORE_HOME", os.path.join(os.path.expanduser("~"), ".treecore")
                )
            )
            candidates = [
                os.path.join(treecore_home, "node", "bin", resolved_command),
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`treecore_cli.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (TREECORE_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit TREECORE_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Hermes' logging (agent.log via treecore_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `treecore setup` to install MCP support, "
                "then retry."
            )
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"treecore-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `treecore mcp login {server_name}` (or delete the tokens "
        f"file under ~/.treecore/mcp-tokens/ and restart). Do NOT retry "
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_treecore_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local TREECORE_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_treecore_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_treecore_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local TREECORE_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_treecore_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.treecore/.env`` loaded at startup).
    """
    try:
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``treecore tools`` interactive configuration — connects to each
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple treecore
