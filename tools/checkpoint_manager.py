...
-----------------------------------------------------------------------------
    ...
    ~/.treecore/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/treecore/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_treecore_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/treecore"
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("TREECORE_CHECKPOINT_TIMEOUT", 30)))
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``treecore checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `treecore checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "treecore@local"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "TREECORE_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``TREECORE_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at TREECORE_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "TREECORE_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `treecore checkpoints` CLI
# ---------------------------------------------------------------------------
