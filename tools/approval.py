...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("TREECORE_YOLO_MODE", ""))
...
# os.environ["TREECORE_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export TREECORE_INTERACTIVE.
_treecore_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "treecore_interactive",
    default=None,
)


def set_treecore_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["TREECORE_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``TREECORE_INTERACTIVE`` env var for legacy callers.
    """
    return _treecore_interactive_ctx.set("1" if interactive else "")


def reset_treecore_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_treecore_interactive_context`."""
    _treecore_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``TREECORE_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _treecore_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("TREECORE_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("TREECORE_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("TREECORE_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("TREECORE_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("TREECORE_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("TREECORE_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set TREECORE_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind TREECORE_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds TREECORE_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("TREECORE_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $TREECORE_HOME, or by the resolved absolute
# active profile home path such as /home/treecore/.treecore/config.yaml. The
# resolved-absolute form is folded into the ~/.treecore/ patterns at detection
...
# go stale when TREECORE_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_TREECORE_ENV_PATH = (
    r'(?:~\/\.treecore/|'
    r'(?:\$home|\$\{home\})/\.treecore/|'
    r'(?:\$treecore_home|\$\{treecore_home\})/)'
    r'\.env\b'
)
# ~/.treecore/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _TREECORE_ENV_PATH; matches the TREECORE_HOME override form as
# well as ~/.treecore/.
_TREECORE_CONFIG_PATH = (
    r'(?:~\/\.treecore/|'
    r'(?:\$home|\$\{home\})/\.treecore/|'
    r'(?:\$treecore_home|\$\{treecore_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_TREECORE_ENV_PATH}|'
    rf'{_TREECORE_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_treecore_home() / "cache" / "blocked-scripts"
...
DANGEROUS_PATTERNS = [
    ...
    # `treecore` and `gateway` (e.g. `treecore -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\btreecore\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart treecore gateway (kills running agents)"),
    (r'\btreecore\s+update\b', "treecore update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `treecore gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `treecore ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart treecore-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart treecore-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(treecore|gateway|cli\.py)\b', "kill treecore/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill treecore` but not
    # `kill -9 $(pgrep -f treecore)` because the substitution is opaque
    ...
    # the `treecore gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.treecore.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(treecore|ai\.treecore)\b', "stop/restart treecore launchd service (kills running agents)"),
    ...
    # ~/.treecore/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Hermes-managed security file (~/.treecore/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_TREECORE_CONFIG_PATH}|{_TREECORE_ENV_PATH})', "in-place edit of Hermes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_TREECORE_CONFIG_PATH}|{_TREECORE_ENV_PATH})', "in-place edit of Hermes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_TREECORE_CONFIG_PATH}|{_TREECORE_ENV_PATH})', "in-place edit of Hermes config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.treecore/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / TREECORE_HOME even when those are set after this module is
    ...
    # user home (C:\Users\alice\AppData\...\treecore), so folding the user home
    # first would eat the prefix the Hermes-home fold needs.
    command = _rewrite_resolved_treecore_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / TREECORE_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.treecore``); the matched
...
def _rewrite_resolved_treecore_home(command: str) -> str:
    """Rewrite the resolved absolute Hermes home prefix to ``~/.treecore/``.

    Resolves the active ``TREECORE_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.treecore/`` so the static ``_TREECORE_CONFIG_PATH`` / ``_TREECORE_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/treecore/.treecore/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$TREECORE_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from treecore_constants import get_treecore_home
        home = get_treecore_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.treecore")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``treecore send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"treecore-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["TREECORE_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "TREECORE_SPINNER_PAUSE" in os.environ:
            del os.environ["TREECORE_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    ...
    into one place:
      - process-scoped ``--yolo`` / ``TREECORE_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            TREECORE_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set TREECORE_INTERACTIVE or "
                "TREECORE_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set TREECORE_INTERACTIVE or "
            "TREECORE_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("TREECORE_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``TREECORE_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("TREECORE_EXEC_ASK")
        ...
        if _is_cron_approval_context():
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from treecore_cli.config import load_config_readonly as _load_cfg
