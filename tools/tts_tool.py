...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.treecore/config.yaml``. Hermes
...
Configuration is loaded from ~/.treecore/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``treecore_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from treecore_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``treecore auth add <provider_id>``).
...
from tools.xai_http import treecore_xai_user_agent
...
# Base URL now resolved via treecore_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_treecore_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.treecore/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.treecore/config.yaml.
    ...
    try:
        ...
        logger.debug("treecore_cli.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``treecore tools``); otherwise the historical Edge backend remains active.
    """
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__TREECORE_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _run_command_tts(
    ...
    scrubbed = treecore_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from treecore the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``treecore_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `treecore setup` to configure, "
    ...
    from treecore_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `treecore model` or set XAI_API_KEY.")
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": treecore_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import treecore_cli as _treecore_cli

            _treecore_version = str(_treecore_cli.__version__)
        except Exception:
            _treecore_version = "0.0.0"
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.treecore/cache/piper-voices/`` under the active
    TREECORE_HOME so voice downloads follow profile boundaries.
    """
    from treecore_constants import get_treecore_dir
    root = Path(get_treecore_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.treecore/config.yaml (tts: section).
    ...
    platform = get_session_env("TREECORE_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `treecore setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run treecore setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'treecore setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'treecore tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_treecore_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
