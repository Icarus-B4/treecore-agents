...
# display_treecore_home imported lazily at call site (stale-module safety during treecore update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of treecore directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "treecore-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("TREECORE_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("TREECORE_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("TREECORE_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (TREECORE_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (TREECORE_INTERACTIVE=1 or a registered sudo password callback):
    ...
    should_prompt_for_sudo = (
        env_var_enabled("TREECORE_INTERACTIVE") or has_sudo_prompt_callback
    )
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``treecore-agent=1`` for the current
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.treecore/.env or environment variables."
        )
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``treecore
    ...
    be stale from ``treecore setup``). Environment values for omitted terminal
    ...
    try:
        from treecore_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    from treecore_cli.config import _is_ssh_remote_tilde_cwd
    ...
    return {
        ...
        # Startup orphan reaper for treecore-tagged containers left behind by
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/treecore
        # restart|stop targeting treecore-gateway) must never run inside the
        ...
        # never restart. This mirrors the `treecore gateway restart` guard in
        # treecore_cli/gateway.py and the cron-path guard in treecore_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_TREECORE_GATEWAY") == "1":
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `treecore gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # treecore-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in treecore-agent dev work (PRs #31329, "
                            ...
                            "name='github/treecore-agent-dev', "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `treecore -z`, a "
                        ...
                        _gw_platform = _gse("TREECORE_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("TREECORE_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("TREECORE_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("TREECORE_SESSION_USER_ID", "")
                            _gw_user_name = _gse("TREECORE_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("TREECORE_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("TREECORE_INTERACTIVE"):
                    output += (
