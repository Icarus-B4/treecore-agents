...
it is automatically downloaded from GitHub releases to $TREECORE_HOME/bin/tirith.
...
def _get_treecore_home() -> str:
    """Return the Hermes home directory, respecting TREECORE_HOME env var."""
    return str(get_treecore_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_treecore_home(), ".tirith-install-failed")
...
def _treecore_bin_dir() -> str:
    """Return $TREECORE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_treecore_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $TREECORE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_treecore_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $TREECORE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $TREECORE_HOME/bin/tirith
    ...
    treecore_bin = os.path.join(_treecore_bin_dir(), "tirith")
    if os.path.isfile(treecore_bin) and os.access(treecore_bin, os.X_OK):
        _resolved_path = treecore_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return treecore_bin
