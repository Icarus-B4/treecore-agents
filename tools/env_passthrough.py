...
def _is_treecore_provider_credential(name: str) -> bool:
    """True if ``name`` is a Hermes-managed provider credential (API key,
    token, or similar) per ``_TREECORE_PROVIDER_ENV_BLOCKLIST``.
    ...
    if _is_treecore_internal_secret(name):
        return True
    return name in _TREECORE_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    ``_TREECORE_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_treecore_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Hermes provider "
                "credential %r (blocked by _TREECORE_PROVIDER_ENV_BLOCKLIST). "
            ...
            for item in passthrough:
                ...
                if _is_treecore_provider_credential(name):
                    logger.warning(
                        ...
                        "by _TREECORE_PROVIDER_ENV_BLOCKLIST). Operator "
