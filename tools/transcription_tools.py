...
LOCAL_STT_COMMAND_ENV = "TREECORE_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "TREECORE_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via treecore_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``TREECORE_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _try_lazy_install_stt() -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "then `su - <owner> -c 'VIRTUAL_ENV=/opt/treecore/.venv "
...
#
# The single-env-var ``TREECORE_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__TREECORE_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"treecore-cmd-stt-{provider_name}-") as tmpdir:
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set TREECORE_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `treecore plugins list` to see "
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="treecore-silk-")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(
            target=_watch, name="treecore-stt-idle-unload", daemon=True
        )
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="treecore-local-stt-") as output_dir:
            ...
            child_env = treecore_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``TREECORE_LOCAL_STT_LANGUAGE`` (env). When none is set, Groq
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="treecore-stt-") as work_dir:
                try:
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `treecore model` or set XAI_API_KEY",
        }
...
def _transcribe_deepinfra(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    ``treecore_cli.models`` helpers so every DeepInfra surface resolves the
...
def _trim_silence_for_cloud_stt(
    ...
    work_dir = tempfile.mkdtemp(prefix="treecore-stt-trim-")
