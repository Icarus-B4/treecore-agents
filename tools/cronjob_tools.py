...
# uses 30s) — comfortably below the 1800s default TREECORE_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (TREECORE_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with TREECORE_CRON_TIMEOUT=0
...
#      saying "cat ~/.treecore/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `treecore-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.treecore/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("TREECORE_SESSION_PLATFORM")
    origin_chat_id = get_session_env("TREECORE_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("TREECORE_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes TREECORE_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``TREECORE_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within TREECORE_HOME/scripts/.
    ...
    # Only relative paths within ~/.treecore/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.treecore/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.treecore/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_treecore_home() / "scripts"
...
def _latest_job_output_excerpt(job_id: str, max_chars: int = 2000) -> Optional[str]:
    ...
    ``~/.treecore/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(
    ...
    None
        ...
        ``treecore -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    if not session_key:
        # Direct Python callers (`treecore cron run`, tests) have no agent
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("TREECORE_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("TREECORE_INTERACTIVE")
        or env_var_enabled("TREECORE_GATEWAY_SESSION")
        or env_var_enabled("TREECORE_EXEC_ASK")
    )
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `treecore cron create/edit --model`, or hand-edited jobs). The agent
