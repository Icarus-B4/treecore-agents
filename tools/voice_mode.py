...
def _voice_capture_install_hint() -> str:
    ...
    # ~/.treecore/profiles/<name>/treecore-agent/venv/), `pip install` on the
...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "treecore_voice")
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.treecore/config.yaml during a
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"treecore-tts-{_unique}.wav")
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("TREECORE_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    TREECORE_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
