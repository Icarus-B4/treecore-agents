...
`mcp_tool.py`, and `treecore_cli/mcp_config.py`. This module is the ONLY place
...
def _make_treecore_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._treecore_server_name = server_name
            self._treecore_home = ""
            ...
            self._treecore_preregistered = preregistered
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._treecore_server_name, exc,
                    )
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._treecore_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._treecore_server_name, asm.token_endpoint,
                        )
                        break
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``treecore mcp reauth``.
            ...
            back to ``treecore mcp reauth``.
            """
            try:
                if self._treecore_preregistered:
                    return
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._treecore_server_name,
                    treecore_home=self._treecore_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
...
_TREECORE_PROVIDER_CLS: Optional[type] = _make_treecore_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._treecore_home = key[0]
    ...
    def _build_provider(
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _TREECORE_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `treecore mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``treecore mcp remove <name>`` and (indirectly) by
        ``treecore mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, treecore_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, treecore_home=treecore_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, treecore_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, treecore_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, treecore_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(treecore_home) / f"{_safe_filename(server_name)}.json"
            try:
