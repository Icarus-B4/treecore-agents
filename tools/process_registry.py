...
CHECKPOINT_PATH = get_treecore_home() / "processes.json"
...
#
# Wrapping the spawn in ``systemd-run --user --scope --unit=treecore-worker-<pid>``
        ...
        if not _IS_WINDOWS:
            try:
                ...
                if binary:
                    ...
                    probe_unit = f"treecore-probe-scope-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def _build_systemd_scope_argv(
    ...
    if binary is None:
        ...
        return shell_argv
    unit_name = f"treecore-worker-{unit_suffix}"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from treecore_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        if use_pty:
            ...
            try:
                ...
                if pty_use_systemd_scope:
                    ...
                    session.systemd_unit = f"treecore-worker-{session.id}.scope"
        ...
        if use_systemd_scope:
            ...
            session.systemd_unit = f"treecore-worker-{unit_suffix}.scope"
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/treecore_bg_{session.id}.log"
        pid_path = f"{temp_dir}/treecore_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/treecore_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``treecore_cli.goals``) to support
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `treecore update` restarting the gateway) is still
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.treecore/processes.json with the raw command
