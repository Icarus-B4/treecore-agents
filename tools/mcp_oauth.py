...
from treecore_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__treecore_user_skipped__"
...
def _get_token_dir(treecore_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses TREECORE_HOME so each profile gets its own OAuth tokens.
    Layout: ``TREECORE_HOME/mcp-tokens/``
    """
    from treecore_constants import get_treecore_home

    base = Path(treecore_home) if treecore_home is not None else Path(get_treecore_home())
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``treecore mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `treecore mcp login <server>` interactively to (re)authorize, "
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        TREECORE_HOME/mcp-tokens/<server_name>.json         -- tokens
        TREECORE_HOME/mcp-tokens/<server_name>.client.json   -- client info
        TREECORE_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, treecore_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._treecore_home = Path(treecore_home) if treecore_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._treecore_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._treecore_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._treecore_home) / f"{self._server_name}.meta.json"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._treecore_home)
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://treecore-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `treecore mcp login <server>` later to "
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = HermesTokenStorage(server_name, treecore_home=treecore_home)
    storage.remove()
    ...
    *,
    ...
    if _is_figma_remote_mcp(server_name, server_url):
        return (
            ...
            "clear it and re-run:\n"
            f"  treecore mcp login {server_name}"
        )
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`treecore mcp login {server_name}` interactively first to complete "
