...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.treecore/.env``.
...
from treecore_cli.config import cfg_get, load_config, read_raw_config
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    with _sessions_lock:
        ...
        if identity_override:
            ...
            session = {
                "user_id": f"treecore_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_treecore_home() / "browser_screenshots"
