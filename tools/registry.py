"""Central registry for all treecore-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (treecore_constants itself is stdlib-only, so no cycle).
        from treecore_constants import get_treecore_home

        return Path(get_treecore_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``treecore tools``
...
def _check_fn_cached(fn: Callable) -> bool:
    ...
    with _check_fn_cache_lock:
        ...
        # No recent success (or grace expired) — honor the failure. A normal
        # False means an optional capability is simply not configured, so keep
        # that expected state at DEBUG. Exceptions remain WARNING-level below.
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``treecore tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("treecore_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``treecore_plugins.{name}``), not the exact module string.
                # A handler defined in ``treecore_plugins.pkg.handlers`` is
                # still owned by the ``treecore_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("treecore_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``treecore tools enable foo``)
