...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``treecore_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from treecore_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where treecore previously hard-rejected an oversized read
...
_treecore_config_resolved: str | None = None
_treecore_config_resolved_loaded = False


def _get_treecore_config_resolved() -> str | None:
    """Return the resolved absolute path of the Hermes config file (cached)."""
    global _treecore_config_resolved, _treecore_config_resolved_loaded
    if _treecore_config_resolved_loaded:
        return _treecore_config_resolved
    _treecore_config_resolved_loaded = True
    try:
        from treecore_cli.config import get_config_path
        _treecore_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _treecore_config_resolved = str(Path(_expand_tilde("~/.treecore/config.yaml")).resolve())
        except Exception:
            _treecore_config_resolved = None
    return _treecore_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # this file.
    treecore_config = _get_treecore_config_resolved()
    if treecore_config and (resolved == treecore_config or normalized == treecore_config):
        return (
            f"Refusing to write to Hermes config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.treecore/config.yaml directly or use 'treecore config' instead."
        )
    return None
...
# .cursorrules (or a project-local .treecore config tree) outlives the current
...
_real_treecore_home_cached: str | None = None
_real_treecore_home_loaded = False


def _get_real_treecore_home() -> str | None:
    """Return the realpath of the authoritative Hermes home (cached)."""
    global _real_treecore_home_cached, _real_treecore_home_loaded
    if _real_treecore_home_loaded:
        return _real_treecore_home_cached
    _real_treecore_home_loaded = True
    try:
        from treecore_constants import get_treecore_home
        _real_treecore_home_cached = os.path.realpath(str(get_treecore_home()))
    except Exception:
        try:
            _real_treecore_home_cached = os.path.realpath(_expand_tilde("~/.treecore"))
        except Exception:
            _real_treecore_home_cached = None
    return _real_treecore_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from treecore_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.treecore home is governed by its own guards
    ...
    # ``.treecore`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_treecore_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .treecore config dirs (e.g. <repo>/.treecore/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.treecore`` — matching
        # any ancestor named .treecore would gate every write inside a
        # checkout that happens to live under ~/.treecore (e.g. the
        # treecore-agent repo itself at ~/.treecore/treecore-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".treecore":
            return candidate
    return None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.treecore/…`` mirror created by a
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.treecore/…`` path.
    ...
    # in a session that cd'd into ``~/.treecore/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # and block credential stores under TREECORE_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == TREECORE_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with treecore_tools.write_file() for very "
            "large files."
        )
