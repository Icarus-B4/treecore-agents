...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``treecore tools`` repaint, tool-registration scans,
    ...
    2. ``~/.treecore/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``treecore auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_treecore_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.treecore/.env`` first, then ``os.environ``.

    Wraps :func:`treecore_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from treecore_cli.config import get_env_value as _treecore_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _treecore_get_env_value(name)
    return value if value is not None else default


def treecore_xai_user_agent() -> str:
...
def treecore_xai_default_headers() -> Dict[str, str]:
    ...
    """
    return {"User-Agent": treecore_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    ...
    try:
        ...
        marker_dir = get_treecore_home() / "state"
...
def resolve_xai_http_credentials(
    *,
    ...
    to ``XAI_API_KEY`` resolved via ``treecore_cli.config.get_env_value`` so keys
    stored in ``~/.treecore/.env`` (the standard Hermes location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("TREECORE_XAI_BASE_URL")
