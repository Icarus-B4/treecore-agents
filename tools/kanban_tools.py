...
running under the dispatcher (env var ``TREECORE_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``treecore chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``treecore kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``treecore kanban complete …``
   inside the container, where ``treecore`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.treecore/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``treecore kanban …``), the dashboard
(``treecore dashboard``), and the slash command (``/kanban …``) — all
...
from treecore_cli.goals import judge_goal
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    a worker — i.e. whenever TREECORE_KANBAN_* is present but not ours."""
    try:
...
def _reject_delegated_child_mutation(tool_name: str) -> Optional[str]:
    ...
    inherited TREECORE_KANBAN_* env vars are not proof of dispatcher ownership.
...
def _check_kanban_mode() -> bool:
    ...
    1. ``TREECORE_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``treecore chat`` without the kanban toolset see zero
    ...
    if _is_delegated_child_context():
        return False
    if os.environ.get("TREECORE_KANBAN_TASK") and _is_dispatcher_owned_worker():
        return True
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if not _is_dispatcher_owned_worker():
        ...
        return None
    env_tid = os.environ.get("TREECORE_KANBAN_TASK")
    return env_tid or None


def _worker_run_id(task_id: str) -> Optional[int]:
    """Return this worker's dispatcher run id when it is scoped to task_id."""
    if os.environ.get("TREECORE_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("TREECORE_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("TREECORE_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("TREECORE_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``TREECORE_KANBAN_TASK`` set
    ...
    ``TREECORE_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``TREECORE_KANBAN_DB`` → ``TREECORE_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting Hermes.
    """
    from treecore_cli import kanban_db as kb
...
#   - No-op outside dispatcher-spawned worker context (no ``TREECORE_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``TREECORE_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``TREECORE_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``TREECORE_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("TREECORE_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("TREECORE_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("TREECORE_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    (``TREECORE_KANBAN_TASK`` set) and ``agent`` exposes ``steer``. Returns True
    ...
    by ``TREECORE_PROFILE``) are skipped to avoid echoing itself.
    """
    tid = os.environ.get("TREECORE_KANBAN_TASK")
    ...
    own = (os.environ.get("TREECORE_PROFILE") or "").strip()
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set TREECORE_KANBAN_TASK in the env)"
        )
        ...
        try:
            ...
            if task and task.goal_mode and _goal_judge_available():
                ...
                try:
                    ...
                    # treecore_cli/goals.py. Unpacking fewer raises ValueError,
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # TREECORE_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``treecore-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("TREECORE_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        with httpx.stream(
            "GET",
            current_url,
            headers={"User-Agent": "treecore-kanban/attach"},
            timeout=30,
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("TREECORE_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets TREECORE_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    # Prefer the request-scoped api_server origin binding: TREECORE_SESSION_ID
    ...
    session_id = (
        ...
        or os.environ.get("TREECORE_SESSION_ID")
    )
    ...
    try:
        ...
        try:
            ...
            if _inherit_project and project_id is None:
                _self_tid = os.environ.get("TREECORE_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("TREECORE_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``treecore dashboard``).

    Subscription paths:

    - **Gateway** (telegram/discord/slack/etc): ``TREECORE_SESSION_PLATFORM``,
      ``TREECORE_SESSION_CHAT_ID``, and ``TREECORE_SESSION_CHAT_TYPE`` are set in
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``TREECORE_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # poller keys on TREECORE_SESSION_KEY.
            session_key = (
                get_session_env("TREECORE_SESSION_KEY", "")
                or os.environ.get("TREECORE_SESSION_KEY", "")
            )
            if not session_key:
                return False  # CLI / cron / test — no persistent channel
            platform = "tui"
            chat_id = session_key
        thread_id = get_session_env("TREECORE_SESSION_THREAD_ID", "") or None
        user_id = get_session_env("TREECORE_SESSION_USER_ID", "") or None
        chat_type = get_session_env("TREECORE_SESSION_CHAT_TYPE", "") or None
        message_id = get_session_env("TREECORE_SESSION_MESSAGE_ID", "") or ""
        notifier_profile = (
            get_session_env("TREECORE_SESSION_PROFILE", "")
            or os.environ.get("TREECORE_PROFILE")
        )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to TREECORE_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: TREECORE_KANBAN_DB env → "
    "TREECORE_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to TREECORE_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
