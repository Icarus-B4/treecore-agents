...
# Cache validation (mirrors treecore_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.treecore/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set TREECORE_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped TREECORE_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from TREECORE_HOME/.env."""
    env_path = get_treecore_home() / ".env"
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set TREECORE_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (TREECORE_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("TREECORE_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("TREECORE_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_treecore_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.treecore/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``TREECORE_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``TREECORE_PLATFORM`` environment variable
    3. ``TREECORE_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from treecore_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("TREECORE_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.treecore/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``treecore skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile TREECORE_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from treecore_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: treecore plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.treecore/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `treecore skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.treecore.* first (agentskills.io convention), fall back to top-level
        treecore_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            treecore_meta = metadata.get("treecore", {}) or {}

        tags = _parse_tags(treecore_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            treecore_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `treecore sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
