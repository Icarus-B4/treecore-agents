...
# TREECORE_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("TREECORE_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {TREECORE_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_treecore_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__TREECORE_CWD_{session_id}__"
...
# the FIRST session's TREECORE_SESSION_ID into the snapshot makes every LATER
...
# TREECORE_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# with one of these prefixes (or is TREECORE_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (TREECORE_SESSION_|TREECORE_UI_SESSION_ID|TREECORE_CRON_AUTO_DELIVER_|TREECORE_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``TREECORE_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!TREECORE_SESSION_*} ${!TREECORE_CRON_AUTO_DELIVER_*} "
        f"TREECORE_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/treecore-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/treecore-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__treecore_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__treecore_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__treecore_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__treecore_fns\" ] && declare -f $__treecore_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_TREECORE_RUNTIME_PASSTHROUGH_{name}"
        ...
        if self._snapshot_ready:
            parts.append(
                f"__treecore_snap_tmp=$(mktemp {_snap_tmp_template}) && "
        ...
        parts.append("exit $__treecore_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"TREECORE_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_treecore_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via TREECORE_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __TREECORE_CWD_{session}__ marker from stdout output.
