...
from tools.environments.local import (
    _TREECORE_PROVIDER_ENV_BLOCKLIST,
    _is_treecore_internal_secret,
)
...
_EGRESS_LABEL_KEY = "treecore-egress"
...
def _load_treecore_env_vars() -> dict[str, str]:
    """Load ~/.treecore/.env values without failing Docker command execution."""
    try:
        from treecore_cli.config import load_env
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale treecore-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=treecore-agent=1`` (created by this codebase)
    ...
    * (optional) ``label=treecore-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a treecore process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    filters = ["--filter", "label=treecore-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=treecore-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``TREECORE_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("TREECORE_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using TREECORE_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'treecore'
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``TREECORE_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `treecore egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `treecore egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `treecore egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `treecore egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/treecore-egress-ca.crt"
    ...
    # through the proxy.  Keep the TREECORE_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"TREECORE_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``treecore-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.treecore/sandboxes/). Non-persistent
        ...
        if egress_env_overrides:
            try:
                from treecore_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from treecore_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_TREECORE_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_TREECORE_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. treecore-agent:latest) already use /init as PID 1
        ...
        container_name = f"treecore-{uuid.uuid4().hex[:8]}"
        # Labels make treecore-created containers identifiable to:
        #   * the orphan reaper (`treecore-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`treecore-task-id`, `treecore-profile`)
        #   * operators running `docker ps --filter label=treecore-agent=1`
        ...
        label_args = [
            "--label", "treecore-agent=1",
            "--label", f"treecore-task-id={task_label}",
            "--label", f"treecore-profile={profile_name}",
        ...
        self._labels = {
            "treecore-agent": "1",
            "treecore-task-id": task_label,
            "treecore-profile": profile_name,
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # name-based blocklist doesn't cover — see _is_treecore_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_treecore_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _TREECORE_PROVIDER_ENV_BLOCKLIST)
        treecore_env = _load_treecore_env_vars() if forward_keys else {}
        unset_names: set[str] = set()
        for key in sorted(forward_keys):
            value = os.getenv(key) or treecore_env.get(key)
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("treecore-task-id", "")
        profile_label = self._labels.get("treecore-profile", "")
        existing = self._find_reusable_container(
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"treecore-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(
        ...
        matches containers that happened to be named ``treecore-*`` but were
        started by some other tool.
        """
        try:
            filters = [
                "--filter", "label=treecore-agent=1",
                "--filter", f"label=treecore-task-id={task_label}",
                "--filter", f"label=treecore-profile={profile_label}",
            ]
            if egress_label != "off":
                ...
                # treecore-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "treecore egress disable",
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"treecore-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``treecore /quit``
