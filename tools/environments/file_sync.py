...
_FORCE_SYNC_ENV = "TREECORE_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.treecore") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.treecore to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.treecore", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``TREECORE_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, treecore_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.treecore/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(treecore_home=treecore_home)

    def _sync_back_transaction(self, treecore_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .treecore/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (treecore_home or get_treecore_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="treecore-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.treecore/skills/a.md`` →
        ``~/.treecore/skills/a.md``, a new remote file at
        ``/root/.treecore/skills/b.md`` maps to ``~/.treecore/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
