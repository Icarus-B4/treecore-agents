...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Hermes was already launched from (for example ``treecore-agent``
    while the process cwd is ``~/.treecore/treecore-agent``), passing it through
    unchanged makes the wrapper run ``cd treecore-agent`` *inside* the project
    ...
    # Common recovery for config values like ``treecore-agent`` when Hermes was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./treecore-agent``; use the current directory instead.
...
_TREECORE_PROVIDER_ENV_FORCE_PREFIX = "_TREECORE_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from treecore_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "TREECORE_DASHBOARD_SESSION_TOKEN",
...
_TREECORE_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _is_treecore_internal_secret(key: str) -> bool:
    """Return True for Hermes-internal secrets injected under *dynamic* names.

    ``_TREECORE_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    non-terminal :func:`treecore_subprocess_env` helper all call it, so the
...
def _inject_context_treecore_home(env: dict) -> None:
    """Bridge the context-local Hermes home override into subprocess env."""
    try:
        from treecore_constants import get_treecore_home_override

        value = get_treecore_home_override()
        if value:
            env["TREECORE_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (TREECORE_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_TREECORE_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_treecore_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _TREECORE_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_TREECORE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_TREECORE_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_treecore_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_treecore_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _TREECORE_PROVIDER_ENV_BLOCKLIST and not passthrough:
    ...
    _inject_context_treecore_home(sanitized)

    from treecore_constants import apply_subprocess_home_env
...
# narrow subset of _TREECORE_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in treecore_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_treecore_internal_secret, but _ID has no secret suffix, so it must be
...
def treecore_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_TREECORE_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 2 (conditional):** the rest of ``_TREECORE_PROVIDER_ENV_BLOCKLIST``
    ...
    # legitimate use for them. See :func:`_is_treecore_internal_secret`.
    for key in list(env):
        if key.startswith(_TREECORE_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_treecore_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _TREECORE_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_treecore_home(env)
    from treecore_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose TREECORE_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's TREECORE_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`treecore_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``TREECORE_HOME`` / subprocess ``HOME`` contract) and the
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_treecore_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``TREECORE_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Hermes home override into ``TREECORE_HOME`` and apply the
      subprocess HOME contract (``treecore_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``TREECORE_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs TREECORE_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("TREECORE_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale TREECORE_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\treecore\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\treecore\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _treecore_portable_git = os.path.join(_local_appdata, "treecore", "git") if _local_appdata else ""
    if _treecore_portable_git:
        for candidate in (
            os.path.join(_treecore_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_treecore_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # TREECORE_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\treecore\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "TREECORE_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        ...
        "Or set TREECORE_GIT_BASH_PATH to your bash.exe location."
    )
...
# Cached directory containing the ``treecore`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_TREECORE_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_treecore_bin_dir() -> str | None:
    """Return the directory holding the ``treecore`` console-script, or None.
    ...
    treecore install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``treecore``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``treecore`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``treecore`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("treecore")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``treecore`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _TREECORE_BIN_DIR
    if _TREECORE_BIN_DIR is not _SENTINEL:
        return _TREECORE_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("treecore")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "treecore" or base.startswith("treecore."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "treecore.exe" if _IS_WINDOWS else "treecore"
    ...
    _TREECORE_BIN_DIR = candidate
    return candidate


def _prepend_treecore_bin_dir(existing_path: str) -> str:
    """Prepend the treecore install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_treecore_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    ...
    - ``$TREECORE_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$TREECORE_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_treecore_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_treecore_managed_node``, a first browser install).
    """
    try:
        from treecore_constants import get_treecore_home, iter_treecore_node_dirs

        candidates = [*iter_treecore_node_dirs(), get_treecore_home() / "bin"]
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_TREECORE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_TREECORE_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_treecore_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_treecore_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _TREECORE_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the treecore install dir is reachable so plugins can shell out
        # to bare ``treecore`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_treecore_bin_dir(new_path)

    _inject_context_treecore_home(run_env)
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``TREECORE_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under TREECORE_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_treecore_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "treecore_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._treecore_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_treecore_pgid", None)
                    if pgid is None:
                        raise
