...
from treecore_cli.config import DEFAULT_CONFIG, cfg_get
...
def _build_browser_env() -> dict:
    ...
    The ``treecore_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = treecore_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    treecore_node_bin = str(treecore_home / "node" / "bin")
    treecore_node_root = str(treecore_home / "node")
    treecore_nm_bin = str(treecore_home / "node_modules" / ".bin")
    return [treecore_node_bin, treecore_node_root, treecore_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``treecore doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.treecore/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.treecore/plugins/browser/<vendor>/``) participate
...
def _allow_private_urls() -> bool:
    ...
    if get_treecore_home_override() is not None:
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-treecore_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed treecore processes — this way every clean treecore exit sweeps
    ...
    # Sweep orphans from other crashed treecore processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live treecore processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current treecore PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live treecore process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning treecore process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced treecore PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent treecore instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-treecore_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live treecore process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    # node_modules binary, which disappears on the next ``treecore update`` and
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_treecore_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_treecore_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_treecore_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this treecore PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_treecore_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = treecore_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    from treecore_constants import get_treecore_dir
    screenshots_dir = get_treecore_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/treecore-agent:latest")
                else:
