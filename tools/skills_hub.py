...
Used by treecore_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _treecore_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            treecore_meta = metadata.get("treecore", {})
            if isinstance(treecore_meta, dict):
                tags = treecore_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            treecore_meta = metadata.get("treecore", {})
            if isinstance(treecore_meta, dict):
                raw_tags = treecore_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.treecore/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/treecore-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                treecore_meta = meta_block.get("treecore", {})
                if isinstance(treecore_meta, dict):
                    tags = treecore_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``treecore_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
TREECORE_INDEX_URL = "https://treecore-agent.nousresearch.com/docs/api/skills-index.json"
TREECORE_INDEX_TTL = 6 * 3600  # 6 hours


def _treecore_index_cache_file() -> Path:
    return _index_cache_dir() / "treecore-index.json"


def _load_treecore_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for TREECORE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    treecore_index_cache_file = _treecore_index_cache_file()
    if treecore_index_cache_file.exists():
        try:
            age = time.time() - treecore_index_cache_file.stat().st_mtime
            if age < TREECORE_INDEX_TTL:
                return json.loads(treecore_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    # Cache locally
    try:
        treecore_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        treecore_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class HermesIndexSource(SkillSource):
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_treecore_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "treecore-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "treecore-index"),
            identifier=entry.get("identifier", ""),
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "treecore-index"
