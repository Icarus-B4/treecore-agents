...
class PlatformEntry:
    ...
    # status displays (``treecore setup``, ``treecore status``, the dashboard
    ...
    # Env vars this platform needs (for ``treecore setup`` display).
    ...
    # built-ins).  Used by ``treecore gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `treecore`
        # invocation -- including plain `treecore chat`, which never touches any
        ...
        # `treecore setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``treecore setup``/``gateway status``, channel
