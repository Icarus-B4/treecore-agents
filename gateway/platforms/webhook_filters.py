...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.treecore to the active profile home."""
    ...
    if raw.startswith("~/.treecore/"):
        return treecore_home / raw.removeprefix("~/.treecore/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return treecore_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under TREECORE_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from treecore_constants import get_treecore_home

    scripts_root = (get_treecore_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.treecore" or raw_text.startswith("~/.treecore/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__treecore_ignore__") is True
        ):
            return False, None
