...
Create a plugin directory in `~/.treecore/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `treecore gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `treecore_cli/config.py` so the setup
...
And add it to the `treecore-gateway` composite:

```python
"treecore-gateway": {
    "includes": [..., "treecore-your-platform"]
}
```
...
## 12. Status Display (`treecore_cli/status.py`)
...
## 13. Gateway Setup Wizard (`treecore_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ treecore_cli/ toolsets.py \
  --include="*.py" -l | sort -u
