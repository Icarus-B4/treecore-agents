...
def _reply_anchor_for_event(event) -> str | None:
    ...
    if (
        ...
        and raw_message.get("_treecore_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``TREECORE_SESSION_PLATFORM`` contextvar — that contextvar is
...
from treecore_constants import get_default_treecore_root, get_treecore_dir, get_treecore_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.treecore/.env manually."
)
...
IMAGE_CACHE_DIR = get_treecore_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_treecore_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_treecore_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from treecore_cli.config import load_config_readonly as _load_config
...
AUDIO_CACHE_DIR = get_treecore_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_treecore_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_treecore_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_treecore_dir("cache/screenshots", "browser_screenshots")
...
_TREECORE_ROOT = get_default_treecore_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "TREECORE_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "TREECORE_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "TREECORE_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "TREECORE_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _TREECORE_HOME / "image_cache",
    _TREECORE_HOME / "audio_cache",
    _TREECORE_HOME / "video_cache",
    _TREECORE_HOME / "document_cache",
    _TREECORE_HOME / "browser_screenshots",
    ...
    _TREECORE_HOME / "cache" / "images",
    _TREECORE_HOME / "cache" / "audio",
    _TREECORE_HOME / "cache" / "videos",
    _TREECORE_HOME / "cache" / "documents",
    _TREECORE_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    ...
    list only covers the *active* TREECORE_HOME's cache, so a gateway running at
    the root (e.g. ``TREECORE_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when TREECORE_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _TREECORE_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("TREECORE_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _TREECORE_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # TREECORE_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.treecore
    ...
    for treecore_root in (_TREECORE_HOME, _TREECORE_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(treecore_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    secrets (``~/.treecore/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``TREECORE_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Hermes-managed cache, under an operator-allowlisted root
    (``TREECORE_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Hermes root (~/.treecore/.env,
    ...
    # ``MEDIA:~/.treecore/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?treecore\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+treecore[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``TREECORE_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-TREECORE_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("TREECORE_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "TREECORE_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "TREECORE_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-TREECORE_HOME holder may be replaced only when the runner
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.treecore/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from treecore_cli.commands import (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          TREECORE_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          TREECORE_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          TREECORE_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("TREECORE_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("TREECORE_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("TREECORE_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_treecore_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # TREECORE_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _treecore_run_generation is set on
    ...
    def build_source(
        ...
        per-profile TREECORE_HOME isolation.
        """
