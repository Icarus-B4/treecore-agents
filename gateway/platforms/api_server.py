...
- GET  /v1/models                  — lists treecore-agent and any configured model_routes aliases
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to treecore-agent
...
def _treecore_version() -> str:
    """Return the canonical Hermes Agent version string.

    ``treecore_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``treecore_agents-*.dist-info`` after a source update until the environment is
    reinstalled. Never raises — a version probe must not be able to break the
    health endpoint.
    """
    try:
        from treecore_cli import __version__
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from treecore_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("TREECORE_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``treecore-agent``)
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_treecore_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted TREECORE_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # treecore_state._WAL_INCOMPAT_MARKERS).
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through treecore-agent's AIAgent.
    """
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "treecore-agent"

        Delegates the tiered fallthrough to
        :func:`treecore_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from treecore_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "treecore-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from treecore_cli.auth import has_usable_secret
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            return None
        try:
            from treecore_cli.profiles import profiles_to_serve
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_treecore_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from treecore_cli.profiles import get_profile_dir
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``treecore sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_treecore_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_treecore_home())
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "treecore_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "treecore_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the treecore-api-server default.
        ...
        # treecore_cli.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `treecore auth add openai-codex` without `treecore model`),
        ...
        if not model and runtime_kwargs.get("provider"):
            try:
                from treecore_cli.models import get_default_model_for_provider
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "treecore-agent", "version": _treecore_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "treecore-agent",
            "version": _treecore_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list treecore-agent and any configured model_routes aliases.
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "treecore.api_server.capabilities",
            "platform": "treecore-agent",
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "treecore.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "treecore.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "treecore.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "treecore.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {
                "object": "treecore.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "treecore.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``treecore.tool.progress`` with ``status: running``.
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: treecore.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="treecore.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_treecore_api_runtime", {}) or {})
                    ...
                    with self._profile_scope(request_profile):
                        try:
                            ...
                            session_tokens = self._bind_api_server_session(
                                ...
                                # TREECORE_SESSION_CHAT_ID on /v1/runs and
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "treecore.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # that is the exact surface the treecore-0day campaign abused to write
            # ~/.treecore/config.yaml and plant persistence. Sandboxing (Docker /
