...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to TREECORE_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/treecore), this function
    mirrors the bridge source to a writable TREECORE_HOME location and returns that
    ...
    # Try TREECORE_HOME location first
    treecore_home = get_treecore_home()
    treecore_home_bridge = treecore_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to TREECORE_HOME if needed
    if treecore_home_bridge.exists():
        return treecore_home_bridge

    # Mirror the bridge source to TREECORE_HOME
    try:
        treecore_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            treecore_home_bridge,
            dirs_exist_ok=False,
        )
        return treecore_home_bridge
    except Exception:
