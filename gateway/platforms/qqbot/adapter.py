...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.treecore/.update_response. Set here so the cross-adapter gateway
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "treecore-agent",
                    "$device": "treecore-agent",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.treecore/.update_response`` for the
          detached ``treecore update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``treecore update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `treecore setup gateway` / `treecore gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``treecore update --gateway`` watcher. Button
        ...
        ``~/.treecore/.update_response`` so the detached update process
        can read it.
        """
