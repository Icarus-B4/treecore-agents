...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<treecore_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active TREECORE_HOME."""
    from treecore_constants import get_treecore_home

    flush_dir = get_treecore_home() / "pending_messages"
