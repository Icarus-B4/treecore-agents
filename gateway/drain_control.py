...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{TREECORE_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``TREECORE_HOME`` is a **durable** store — on Hermes
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``treecore gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting TREECORE_HOME."""
    base = home if home is not None else get_treecore_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable TREECORE_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (TREECORE_HOME is a
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable TREECORE_HOME volume is
