...
The PID file lives at ``{TREECORE_HOME}/gateway.pid``.  TREECORE_HOME defaults to
``~/.treecore`` but can be overridden via the environment variable.  This means
separate TREECORE_HOME directories naturally get separate PID files — a property
...
from treecore_constants import get_treecore_home, _get_platform_default_treecore_home
...
_GATEWAY_KIND = "treecore-gateway"
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_treecore_home() / "gateway-starts.log"
...
def _get_process_treecore_home() -> Path:
    """Return the process-level TREECORE_HOME, skipping context-local overrides.
    ...
    ``get_treecore_home()`` honors ``_TREECORE_HOME_OVERRIDE`` contextvar used for
...
def _canonical_treecore_home(path: Path | str) -> Path:
    """Return a stable absolute TREECORE_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_treecore_home(left: Path | str, right: Path | str) -> bool:
    """Compare TREECORE_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_treecore_home(left))) == os.path.normcase(
        str(_canonical_treecore_home(right))
    )


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting TREECORE_HOME."""
    home = _get_process_treecore_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("TREECORE_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "treecore" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    test also matched ``treecore_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\treecore-gateway.exe"``) survive, and strips
    ...
    for token in tokens:
        ...
        if basename in ("treecore-gateway", "treecore-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "treecore_cli.main" in joined
        or "treecore_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("treecore", "treecore.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `treecore gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a TREECORE_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.treecore`` or ``$TREECORE_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``treecore_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``TREECORE_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a TREECORE_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"treecore_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit TREECORE_HOME= on the argv. TREECORE_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "treecore_home=" in command_lc and f"treecore_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # TREECORE_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "treecore_home": str(_canonical_treecore_home(_get_process_treecore_home())),
    }
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `treecore status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``TREECORE_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``treecore_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    TREECORE_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different TREECORE_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``treecore.service`` + ``treecore-
# gateway.service``).
#
...
def _get_takeover_marker_path(treecore_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``treecore_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = treecore_home or _get_process_treecore_home()
    return _canonical_treecore_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-TREECORE_HOME --replace while
    ...
    our_home = _get_process_treecore_home()
    target_home = record.get("target_treecore_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_treecore_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_treecore_home")
        if replacer_home is not None and not _same_treecore_home(
    ...
    # ``treecore gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's TREECORE_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_treecore_home(
            target_home or _get_process_treecore_home()
        )
        ...
        record = {
            ...
            "target_treecore_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_treecore_home": str(
                _canonical_treecore_home(_get_process_treecore_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target TREECORE_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("treecore_home")
    if not isinstance(pid_record_home, str) or not _same_treecore_home(
