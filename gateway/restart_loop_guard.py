...
Defenses 1 and 2 (the ``_TREECORE_GATEWAY`` guard on ``treecore gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.treecore.gateway")``,
...
State lives in ``<TREECORE_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_treecore_home() / "gateway" / "restart_loop.json"
