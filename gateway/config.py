...
# dashboard's pre-write mutation validation (treecore_cli/web_server.py) so
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_treecore_home() / "sessions")
    ...
    # gateway behaves exactly as before — single TREECORE_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``treecore config set gateway.multiplex_profiles true``.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.treecore/config.yaml (primary user-facing config)
    3. ~/.treecore/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_treecore_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through treecore_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `treecore config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `treecore config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``treecore config set gateway.streaming.*``
