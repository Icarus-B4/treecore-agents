...
2. An event-loop heartbeat file at ``<TREECORE_HOME>/state/gateway.heartbeat`` so
...
def _process_treecore_home() -> Path:
    """TREECORE_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<TREECORE_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_treecore_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from treecore_logging import drain_log_queue
