...
def _normalize_slack_parent_command(
    ...
    """Mirror native Slack ``/treecore`` routing for authenticated relay text."""
    stripped = text.strip()
    parent_parts = stripped.split(maxsplit=1)
    if not parent_parts or parent_parts[0] != "/treecore":
...
def _event_from_wire(raw: Dict[str, Any]) -> MessageEvent:
    ...
    if platform_enum == Platform.SLACK:
        ...
        # gateway command rather than the legacy `treecore` parent name.
