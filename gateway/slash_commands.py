...
Module-level run.py helpers a handler needs (``_treecore_home``,
...
from treecore_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: treecore gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from treecore_cli.kanban import run_slash
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`treecore gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _treecore_home
        ...
        try:
            ...
            atomic_json_write(
                _treecore_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _treecore_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _treecore_home, _load_gateway_config
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _treecore_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from treecore_cli.context_switch_guard import (
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from treecore_cli.route_identity import should_clear_context_pin_async
                                ...
                                from treecore_cli.config import save_config
                        ...
                        from treecore_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        config_path = _treecore_home / 'config.yaml'
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_treecore_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from treecore_cli.goals import draft_contract
            ...
            from treecore_cli.goals import parse_contract
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from treecore_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `treecore cron` for durable schedules."
        )
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _treecore_home
        from treecore_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.treecore/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``treecore skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _treecore_home
        from treecore_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `treecore skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.treecore/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from gateway.slash_access import policy_for_source
        from treecore_cli.approval_mode import run_approval_mode_command
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _treecore_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _treecore_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from treecore_cli.session_listing import (
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `treecore bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``treecore debug share`` from the CLI.
        """
        import asyncio
        from treecore_cli.debug import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        Spawns ``treecore update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``treecore update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _treecore_home, _resolve_treecore_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from treecore_cli.config import is_managed, format_managed_message
        ...
        treecore_cmd = _resolve_treecore_bin()
        if not treecore_cmd:
            return t("gateway.update.treecore_cmd_not_found")

        pending_path = _treecore_home / ".update_pending.json"
        output_path = _treecore_home / ".update_output.txt"
        exit_code_path = _treecore_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `treecore update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `treecore update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # treecore_cmd is a list of argv parts we can pass directly
                ...
                treecore_cmd_str = " ".join(shlex.quote(part) for part in treecore_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {treecore_cmd_str} update --gateway"
