...
_TREECORE_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (TREECORE_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `treecore tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``treecore tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (TREECORE_KANBAN_TASK env "
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in treecore_cli/tools_config.py).
        "posture": True,
    },
    ...
    # the `treecore send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "treecore-cli": {
        ...
        "tools": _TREECORE_CORE_TOOLS,
        "includes": []
    },

    "treecore-cron": {
        # Mirrors treecore-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `treecore tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as treecore-cli; gated by `treecore tools`",
        "tools": _TREECORE_CORE_TOOLS,
        "includes": []
    },
    ...
    "treecore-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _TREECORE_CORE_TOOLS,
        "includes": []
    },

    "treecore-homeassistant": {
    ...
    "treecore-webhook": {
        ...
        "tools": _TREECORE_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "treecore-gateway": {
        ...
        "tools": [],
        "includes": ["treecore-telegram", "treecore-discord", "treecore-whatsapp", "treecore-slack", "treecore-signal", "treecore-bluebubbles", "treecore-homeassistant", "treecore-email", "treecore-sms", "treecore-mattermost", "treecore-matrix", "treecore-dingtalk", "treecore-feishu", "treecore-wecom", "treecore-wecom-callback", "treecore-weixin", "treecore-qqbot", "treecore-webhook", "treecore-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``treecore-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_TREECORE_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``treecore-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (treecore-<name>).
        # Gives them _TREECORE_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("treecore-"):
            platform_name = name[len("treecore-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_TREECORE_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
