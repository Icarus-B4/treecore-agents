...
# and crash the child in a retry loop (issue #51286).  ``treecore_bootstrap``
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``TREECORE_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("TREECORE_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("TREECORE_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    ...
    Each slash_worker child is its own process — the parent ``treecore serve``
...
def main():
    ...
    os.environ["TREECORE_SESSION_KEY"] = args.session_key
