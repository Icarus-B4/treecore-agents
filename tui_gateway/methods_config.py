...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from treecore_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_treecore_home), "display": display_treecore_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _treecore_home / "config.yaml"
        try:
...
def _(rid, params: dict) -> dict:
    try:
        from treecore_cli.main import _has_any_provider_configured
