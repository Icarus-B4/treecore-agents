...
def _default_registry_path() -> Path:
    return get_treecore_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_treecore_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_treecore_home = expected_treecore_home if expected_treecore_home is not None else str(get_treecore_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["TREECORE_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("treecore_home") or "")
        if got_home and got_home != self.expected_treecore_home:
            raise RuntimeError(f"compute host TREECORE_HOME mismatch: {got_home} != {self.expected_treecore_home}")
