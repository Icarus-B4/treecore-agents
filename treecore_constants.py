...
_UNSET = object()
_TREECORE_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_TREECORE_HOME_OVERRIDE", default=_UNSET
)
...
def set_treecore_home_override(path: str | Path | None) -> Token:
    ...
    return _TREECORE_HOME_OVERRIDE.set(value)


def reset_treecore_home_override(token: Token) -> None:
    """Restore the previous context-local Hermes home override."""
    _TREECORE_HOME_OVERRIDE.reset(token)


def get_treecore_home_override() -> str | None:
    """Return the active context-local Hermes home override, if any."""
    override = _TREECORE_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_treecore_home() -> Path:
    """Return the platform-native default Hermes home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "treecore"
    return Path.home() / ".treecore"


def _treecore_home_from_env() -> Path:
    """Resolve TREECORE_HOME from the process environment only.

    Reads the ``TREECORE_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_treecore_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_treecore_home`
    and :func:`get_process_treecore_home` so the two never drift.
    """
    val = os.environ.get("TREECORE_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_treecore_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``TREECORE_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_treecore_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[TREECORE_HOME fallback] TREECORE_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass TREECORE_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_treecore_home() -> Path:
    ...
    expected to propagate ``TREECORE_HOME`` explicitly (see the systemd
    template in ``treecore_cli/gateway.py`` and the kanban dispatcher in
    ``treecore_cli/kanban_db.py``).  See https://github.com/NousResearch/treecore-agent/issues/18594.
    """
    override = get_treecore_home_override()
    if override:
        return Path(override)

    if not os.environ.get("TREECORE_HOME", "").strip():
        _warn_profile_fallback_once()

    return _treecore_home_from_env()


def get_process_treecore_home() -> Path:
    """Return the Hermes home for the running process, ignoring task overrides.

    Unlike :func:`get_treecore_home`, this never follows the context-local
    override set by :func:`set_treecore_home_override`.  It resolves only the
    process ``TREECORE_HOME`` env var (falling back to the platform default),
...
def get_default_treecore_root() -> Path:
    """Return the root Hermes directory for profile-level operations.

    In standard deployments this is the platform-native Hermes home
    (``~/.treecore`` on POSIX, ``%LOCALAPPDATA%\\treecore`` on native Windows).

    In Docker or custom deployments where ``TREECORE_HOME`` points outside
    ``~/.treecore`` (e.g. ``/opt/data``), returns ``TREECORE_HOME`` directly
    — that IS the root.

    In profile mode where ``TREECORE_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.treecore/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_treecore_home()
    env_home = os.environ.get("TREECORE_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # TREECORE_HOME is under ~/.treecore (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — TREECORE_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``TREECORE_OPTIONAL_SKILLS``.
    """
    override = os.getenv("TREECORE_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_treecore_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``TREECORE_OPTIONAL_MCPS``.
    """
    override = os.getenv("TREECORE_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_treecore_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``TREECORE_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<TREECORE_HOME>/skills`` last-resort
    """
    override = os.getenv("TREECORE_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_treecore_home() / "skills"


def get_treecore_dir(
    new_subpath: str,
    old_name: str,
    *,
    home: Path | None = None,
    ...
    Args:
        new_subpath: Preferred path relative to TREECORE_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to TREECORE_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Hermes home. Profile-aware callers that manage
            ...
            temporarily mutating the process or context-local TREECORE_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_treecore_home()
...
def iter_treecore_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    into ``%LOCALAPPDATA%\\treecore\\node``. POSIX installs use
    ``$TREECORE_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_treecore_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with treecoreManagedNodePathEntries() in
...
_TREECORE_NODE_TARGET_MAJOR = int(os.environ.get("TREECORE_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Hermes-managed Node trees live under ``$TREECORE_HOME/node`` (or a profile's
    ``TREECORE_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_treecore_node_executable``
    used to trust file presence alone, so ``treecore update`` would pick that
...
def treecore_managed_node_tree_present(home: Path | None = None) -> bool:
    ...
    for directory in iter_treecore_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%TREECORE_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_TREECORE_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_TREECORE_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$TREECORE_HOME/node`` on POSIX.
    ...
    ``TREECORE_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_treecore_managed_node() -> str | None:
    ...
    its own tree under ``$TREECORE_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_treecore_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_treecore_node_dirs():
...
def heal_treecore_managed_node() -> bool:
    ...
    if _managed_node_heal_attempted:
        return False
    if not treecore_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "TREECORE_HOME": str(get_treecore_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_treecore_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_TREECORE_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_treecore_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _TREECORE_NODE_TARGET_MAJOR
    return False


def find_treecore_node_executable(command: str) -> str | None:
    """Return a Hermes-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_TREECORE_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_treecore_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_treecore_node_executable(command)
    if managed:
        return managed
    if treecore_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_treecore_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Hermes-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_treecore_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``treecore update`` — leaving a **dangling symlink** that ``which`` still
...
def display_treecore_home() -> str:
    """Return a user-friendly display string for the current TREECORE_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.treecore``
        profile:  ``~/.treecore/profiles/coder``
        custom:   ``/opt/treecore-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.treecore``.  For code that needs a real ``Path``, use
    :func:`get_treecore_home` instead.
    """
    home = get_treecore_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``TREECORE_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/treecore-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{TREECORE_HOME}/home`` when the profile-home directory exists."""
    treecore_home = get_treecore_home_override() or (env or {}).get("TREECORE_HOME") or os.getenv("TREECORE_HOME")
    if not treecore_home:
        return None
    profile_home = os.path.join(treecore_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("TREECORE_REAL_HOME") or os.getenv("TREECORE_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    ...
    ``TREECORE_HOME`` scopes Hermes state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={TREECORE_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{TREECORE_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{TREECORE_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    ...
    real_home = get_real_home(env)
    if real_home:
        env["TREECORE_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: NousResearch/treecore-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under TREECORE_HOME.

    Replaces the ``get_treecore_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, treecore_logging.py, treecore_time.py, etc.).
    """
    return get_treecore_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under TREECORE_HOME."""
    return get_treecore_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under TREECORE_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_treecore_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._treecore_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``treecore_cli`` modules using three different Windows
    ...
    A few sites outside ``treecore_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``treecore_cli.main._is_windows`` and friends);
...
# `treecore_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("treecore_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `treecore update` is exactly
    ...
    return [
        ...
        "    treecore update",
        "If that also fails, reinstall: https://treecore-agent.nousresearch.com",
    ]
