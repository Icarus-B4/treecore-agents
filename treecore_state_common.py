...
Extracted verbatim from treecore_state.py so the SessionDB mixin modules
(treecore_state_search / treecore_state_schema / treecore_state_portability) can
reference them without importing treecore_state (which would be a cycle).
treecore_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``treecore sessions optimize-storage``. A legacy DB sits at
...
# `treecore sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `treecore db optimize`. This is
