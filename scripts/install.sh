...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Icarus-B4/treecore-agents/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:Icarus-B4/treecore-agents.git"
REPO_URL_HTTPS="https://github.com/Icarus-B4/treecore-agents.git"
TREECORE_HOME="${TREECORE_HOME:-$HOME/.treecore}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${TREECORE_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$TREECORE_INSTALL_DIR"
...
#   code at /usr/local/lib/treecore-agent, command at /usr/local/bin/treecore_cli,
#   data still at /root/.treecore (TREECORE_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$TREECORE_HOME/.no-bundled-skills so future"
            echo "                   'treecore_cli update' runs never inject bundled skills either"
            ...
            echo "                   default (non-root):  ~/.treecore/treecore-agent"
            echo "                   default (root, Linux): /usr/local/lib/treecore-agent"
            echo "  --treecore_cli-home PATH  Data directory (default: ~/.treecore, or \$TREECORE_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hermes installs the code under"
            echo "  /usr/local/lib/treecore-agent and links the command into"
            echo "  /usr/local/bin/treecore_cli (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$TREECORE_HOME"
            echo "  (default /root/.treecore).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$TREECORE_HOME/treecore-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `treecore_cli update`
...
# Decide where the repo checkout + venv live, and where the `treecore` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $TREECORE_HOME/treecore-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $TREECORE_HOME/treecore-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/treecore-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $TREECORE_HOME/treecore-agent — then preserve it)
#
# Always no-op when the user set --dir or $TREECORE_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in TREECORE_HOME.
    if is_termux; then
        INSTALL_DIR="$TREECORE_HOME/treecore-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$TREECORE_HOME/treecore-agent/.git" ]; then
            INSTALL_DIR="$TREECORE_HOME/treecore-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/treecore-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/treecore-agent"
        ...
        # /usr/local/bin/treecore_cli wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/treecore_cli"
        log_info "  Data:    $TREECORE_HOME (unchanged)"
...
# so global package binaries land in $TREECORE_HOME/node/bin — which is NOT on
...
configure_managed_node_npm_prefix() {
    [ -x "$TREECORE_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$TREECORE_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$TREECORE_HOME/node/etc/npmrc"
}

get_treecore_cli_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/treecore_cli" ]; then
        echo "$link_dir/treecore_cli"
    else
        echo "treecore_cli"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/Icarus-B4/treecore-agents/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hermes owns its own uv at $TREECORE_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (treecore_cli/managed_uv.py) looks in the same
    # place, so install.sh and `treecore_cli update` stay in sync.
    local _managed_uv="$TREECORE_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $TREECORE_HOME/bin ..."
    mkdir -p "$TREECORE_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/treecore_cli-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/treecore_cli-uv-installer.$$.sh")"
    ...
    # directly into $TREECORE_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$TREECORE_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    if [ -x "$TREECORE_HOME/node/bin/node" ] && node_satisfies_build "$("$TREECORE_HOME/node/bin/node" --version)"; then
        export PATH="$TREECORE_HOME/node/bin:$PATH"
        log_success "Node.js $("$TREECORE_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.treecore/node/..."
    ...
    # Place into ~/.treecore/node/ and symlink binaries into the same bin dir
    # the treecore_cli command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$TREECORE_HOME/node"
    mkdir -p "$TREECORE_HOME"
    mv "$extracted_dir" "$TREECORE_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$TREECORE_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$TREECORE_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$TREECORE_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$TREECORE_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.treecore/node/"
    HAS_NODE=true
}
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `treecore_cli update` path
                # (#4735).
                ...
                local stash_name
                stash_name="treecore_cli-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``treecore_cli update`` and reset to the
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"treecore-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up treecore_cli command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/python"
        HERMES_ENTRYPOINT="$INSTALL_DIR/treecore_cli"
    else
        HERMES_BIN="$(which treecore_cli 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "treecore_cli not found on PATH after install"
            return 0
        fi
    fi
    ...
    # Create a user-facing shim for the treecore_cli command.
    ...
    rm -f "$command_link_dir/treecore_cli"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/treecore_cli" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
...
EOF
    ...
    chmod +x "$command_link_dir/treecore_cli"
    log_success "Installed treecore_cli launcher → $command_link_display_dir/treecore_cli"

    # Also expose `treecore-agent`. The `treecore-agent` console script declared in
    ...
    rm -f "$command_link_dir/treecore-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/treecore-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
...
EOF
    ...
    chmod +x "$command_link_dir/treecore-agent"
    log_success "Installed treecore-agent launcher → $command_link_display_dir/treecore-agent"

    # Also expose `treecore_cli-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `treecore_cli-acp`
    ...
    rm -f "$command_link_dir/treecore_cli-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/treecore_cli-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
...
EOF
    ...
    chmod +x "$command_link_dir/treecore_cli-acp"
    log_success "Installed treecore_cli-acp launcher → $command_link_display_dir/treecore_cli-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "treecore_cli command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v treecore_cli' \
                >/dev/null 2>&1; then
        ...
        log_info "treecore_cli not on PATH in non-login shells (common on RHEL-family)"
    ...
    # Export for current session so treecore_cli works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.treecore directory structure (config at top level, code in subdir)
    mkdir -p "$TREECORE_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.treecore/.env (top level, easy to find)
    if [ ! -f "$TREECORE_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$TREECORE_HOME/.env"
            log_success "Created ~/.treecore/.env from template"
        else
            touch "$TREECORE_HOME/.env"
            log_success "Created ~/.treecore/.env"
        fi
    else
        log_info "~/.treecore/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$TREECORE_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.treecore/config.yaml (top level, easy to find)
    if [ ! -f "$TREECORE_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$TREECORE_HOME/config.yaml"
            log_success "Created ~/.treecore/config.yaml from template"
        fi
    else
        log_info "~/.treecore/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in treecore_cli/default_soul.py — the
    ...
    if [ ! -f "$TREECORE_HOME/SOUL.md" ]; then
        cat > "$TREECORE_HOME/SOUL.md" << 'SOUL_EOF'
You are Hermes Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.treecore/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.treecore/"

    # Seed bundled skills into ~/.treecore/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `treecore_cli update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'treecore_cli update'." \
            > "$TREECORE_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $TREECORE_HOME/.no-bundled-skills"
        log_info "  Future 'treecore_cli update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.treecore/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.treecore/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$TREECORE_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$TREECORE_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.treecore/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$TREECORE_HOME/.env"
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (treecore_cli --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `treecore_cli update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'treecore_cli setup' after install."
        return 0
    fi
    ...
    # Run treecore_cli setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m treecore_cli.main setup < /dev/tty
    else
        python -m treecore_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$TREECORE_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    WHATSAPP_SESSION="$TREECORE_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'treecore_cli whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HERMES_CMD="$(get_treecore_cli_command_path)"
                $HERMES_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'treecore_cli whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'treecore_cli gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: treecore_cli gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: treecore_cli gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HERMES_CMD gateway > "$TREECORE_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.treecore/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: treecore_cli gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: treecore_cli gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.treecore-bootstrap-complete, which tells the Hermes
    ...
    local marker_path="$INSTALL_DIR/.treecore-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $TREECORE_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $TREECORE_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $TREECORE_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}treecore_cli${NC}              Start chatting"
    echo -e "   ${GREEN}treecore_cli setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}treecore_cli config${NC}       View/edit configuration"
    echo -e "   ${GREEN}treecore_cli config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}treecore_cli gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}treecore_cli update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'treecore_cli' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'treecore_cli' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'treecore_cli' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$TREECORE_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$TREECORE_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$TREECORE_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$TREECORE_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$TREECORE_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
# _purge_electron_build_cache() used by `treecore_cli desktop`; install.sh was the only
...
install_desktop() {
    ...
    #         retry (matches install.ps1 / `treecore_cli desktop`),
    ...
    if [ "$OS" = "linux" ]; then
        ...
        elif [ -x "$desktop_dir/release/linux-unpacked/treecore_cli" ]; then
            app="$desktop_dir/release/linux-unpacked/treecore_cli"
        fi
    else
        local cand
        for cand in \
    ...
    # `treecore_cli desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading TREECORE_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if TREECORE_HOME="$TREECORE_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from treecore_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `treecore_cli update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            # provisioned during prerequisites/node-deps (at $TREECORE_HOME/node/bin)
        ...
        complete)
            ...
            # $TREECORE_HOME. $TREECORE_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'treecore_cli update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $TREECORE_HOME.
    # $TREECORE_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'treecore_cli update' on this host install.
