...
Resolution order:
  1. ``TREECORE_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.treecore/config.yaml``
...
from treecore_constants import get_config_path
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("TREECORE_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``TREECORE_TIMEZONE`` update) to force ``get_timezone()`` /
