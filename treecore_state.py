...
from treecore_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from treecore_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from treecore_state_portability import SessionPortabilityMixin
from treecore_state_schema import SessionSchemaMixin
from treecore_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``treecore -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_treecore_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.treecore`` even when a test fixture later
    redirects ``TREECORE_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_treecore_home()`` fresh so a runtime
       ``TREECORE_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_treecore_home() / "state.db"
...
# ~/.treecore/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects TREECORE_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without TREECORE_HOME, a stale worktree
# without the re-pin, or a developer shell that exports TREECORE_HOME to the
...
#: ``~/.treecore``).  The test conftest injects the pre-sandbox production
#: root here so custom-``TREECORE_HOME`` deployments are covered too.
...
def _real_platform_state_root() -> Optional[Path]:
    ...
    Deliberately avoids ``Path.home()`` / ``treecore_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``treecore_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "treecore"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "treecore"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".treecore"
        return root.resolve()
    except Exception:
        return None
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    ...
    under ``~/.treecore/treecore-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``TREECORE_HOME``) paths.
    """
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                "Tests must run against a temporary TREECORE_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "TREECORE_HOME. If this test genuinely needs the live "
...
# treecore_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``treecore_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``treecore update`` can actually do for this install (#75153).
    """
    try:
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    logger.warning(
        ...
        "%s. See `treecore doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see treecore_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with treecore_cli.config.
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``treecore_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Hermes home tree (``get_treecore_home()``) — the safe repair scope:
    ...
    to check). Shared by :class:`SessionDB` and ``treecore_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_treecore_home()).resolve()
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (treecore_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_treecore_state.py:600-632)
        ...
        probe_session_id = f"_treecore_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.treecore/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``treecore sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("TREECORE_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_treecore_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("TREECORE_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship treecore_state without treecore_cli). A
    ...
    try:
        ...
        logger.debug(
            "treecore_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch treecore_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``treecore_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from treecore_cli.backup import is_zeroed_sqlite_file
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `treecore snapshot list` / "
                "`treecore snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple treecore processes (gateway + CLI sessions + worktree agents)
    ...
    # `treecore update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `treecore snapshot list` / "
                    f"`treecore snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`treecore db optimize`),
            ...
            # ``treecore_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `treecore update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `treecore sessions optimize-storage` on a host "
    ...
    #
    # `optimize_fts_storage()` (the `treecore sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`treecore sessions optimize-storage`) ──
    #
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``treecore --resume`` into a fresh
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: treecore_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``treecore -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``treecore sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``TREECORE_SESSION_SOURCE``, so their runs
