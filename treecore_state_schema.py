...
``treecore_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import treecore_state (cycle) — shared
module-level constants live in treecore_state_common.
"""
...
from treecore_state_common import (
    DEFERRED_INDEX_SQL,
    FTS_CJK_STALE_KEY,
    FTS_SQL,
    FTS_STORAGE_VERSION,
...
# Moved methods logged under the "treecore_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("treecore_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``treecore_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `treecore update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._treecore_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._treecore_fts5_probe")
            return True
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `treecore sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `treecore db optimize`) must keep its EXISTING
