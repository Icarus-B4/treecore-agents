...
export const $treecoreHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.treecore/logs/bootstrap-installer.log')
    $treecoreHome.set('~/.treecore')
  ...
  try {
    const [logPath, treecoreHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_treecore_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $treecoreHome.set(treecoreHome)
    $mode.set(mode)
  } catch (err) {
