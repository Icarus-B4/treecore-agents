...
///
/// `treecore_home_override` propagates to the child as $TREECORE_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, treecore_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = treecore_home_override {
        cmd.env("TREECORE_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, treecore_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = treecore_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_treecore_home() {
