...
//!
//! Mirrors `treecore_constants.get_treecore_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\treecore_cli
//!   macOS:   ~/.treecore
//!   Linux:   ~/.treecore  (override via $TREECORE_HOME)
//!
//! NOTE (macOS): Python's get_treecore_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.treecore on macOS — there
...
/// Returns the canonical Hermes home directory, respecting $TREECORE_HOME if set.
pub fn treecore_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("TREECORE_HOME") {
    ...
    {
        // %LOCALAPPDATA%\treecore — matches scripts/install.ps1's $TreecoreHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("treecore");
        }
    }

    // macOS + Linux + fallback: ~/.treecore (matches Python get_treecore_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".treecore");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".treecore")
}

pub fn log_dir() -> PathBuf {
    treecore_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    treecore_home().join("bootstrap-cache")
}
...
/// TREECORE_HOME so it survives repo checkout deletion (unlike anything under
/// treecore-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\treecore_cli\treecore-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_treecore_cli`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under TREECORE_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves TREECORE_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    treecore_home().join(".treecore-update-in-progress")
}
...
pub fn copy_self_to_treecore_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to TREECORE_HOME");
    Ok(())
}
...
///   const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_HERMES_ROOT, '.treecore-bootstrap-complete')
/// We don't always know ACTIVE_HERMES_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".treecore-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under TREECORE_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[treecore-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("TREECORE_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_treecore_home() -> String {
    treecore_home().to_string_lossy().into_owned()
}
