...
//!      venv shim and packaged app.asar are free; otherwise `treecore_cli update`
//!      or repair bootstrap can race locked files),
//!   2. run `treecore_cli update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in treecore_cli/main.py),
//!   3. run `treecore_cli desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `treecore_cli update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_treecore_cli) and
...
/// `treecore_cli update` exit code meaning "another treecore_cli process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// treecore_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `treecore_cli update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `treecore_cli update` claims
/// the same file (see `treecore_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `treecore_cli update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in treecore_cli/update_lock.py — all three read
...
/// pid (e.g. a dashboard-spawned `treecore_cli update`) still blocks.
fn live_marker_owner(path: &Path) -> Option<MarkerOwner> {
    ...
    // Desktop `writeUpdateMarker(treecoreHome, child.pid)` races ahead of us;
...
async fn run_update(app: AppHandle) -> Result<()> {
    let treecore_home = crate::paths::treecore_home();
    let install_root = treecore_home.join("treecore-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_treecore_cli`
    ...
    //
    // The same marker is the cross-process update lock (treecore_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `treecore_cli update` — is already
    ...
    let treecore_cli = resolve_treecore_cli(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the treecore_cli CLI under {}. Is Hermes installed? \
    ...
    // async on Windows. If it still holds the venv shim, `treecore_cli update`
    ...
    // ---- stage 2: treecore_cli update -----------------------------------------
    // Pass --branch so `treecore_cli update` targets the branch this installer was
    ...
    // without --branch, `treecore_cli update` switches the checkout to `main` (a
    ...
    // --force skips `treecore_cli update`'s Windows running-exe guard (which would
    ...
    // time `treecore_cli update` runs there is no legitimate treecore_cli.exe to protect,
    ...
    // probe only checks the treecore_cli.exe shim and app.asar, so an external venv
    ...
    // Retry-once for the update-boundary crash. `treecore_cli update` lazily imports
    ...
    // second `treecore_cli update` runs clean because the now-current module is loaded
    ...
    match update.exit_code {
        ...
        other => {
            let msg = format!(
                "treecore_cli update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::treecore_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: treecore_cli desktop --build-only ----------------------------
    // `treecore_cli update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `treecore_cli update` already does above, and `treecore_cli update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `treecore_cli desktop` \
    ...
    if let Some(target_app) = launch_target {
        ...
        crate::bootstrap::launch_treecore_cli_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend treecore_cli.exe (or the desktop Hermes.exe
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_treecore_cli(install_root)];
...
/// Force-kill any `treecore_cli.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `treecore_cli.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\treecore_cli.exe update`. And a
...
/// treecore_cli.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named treecore_cli.exe.)
fn force_kill_other_treecore_cli() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `treecore_cli <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv treecore_cli shim under an install root, regardless of existence.
fn venv_treecore_cli(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("treecore_cli.exe")
    } else {
        install_root.join("venv").join("bin").join("treecore_cli")
    }
}

/// Resolve the treecore_cli CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `treecore` on PATH.
fn resolve_treecore_cli(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_treecore_cli(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "treecore_cli.exe" } else { "treecore_cli" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let treecore_home = crate::paths::treecore_home();
    let mut envs = vec![(
        "TREECORE_HOME".to_string(),
        treecore_home.as_os_str().to_os_string(),
    )];
    // `treecore_cli update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `treecore_cli update` child claims that SAME lock (treecore_cli/update_lock.py).
    ...
    // sync with HANDOFF_PID_ENV in treecore_cli/update_lock.py.
    envs.push((
        "HERMES_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        treecore_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_treecore_cli_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
    ...
    let tmp = PathBuf::from(format!("{}.treecore-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.treecore-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::treecore_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_treecore_cli_is_under_install_root() {
        let root = Path::new("/x/treecore-agent");
        let shim = venv_treecore_cli(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/treecore-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        ...
        assert!(
            ...
            "the treecore_cli update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_treecore_cli(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/treecore-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".treecore-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `treecore_cli update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(treecoreHome, child.pid) races ahead
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let tmp = base.join("Hermes.app.treecore-update-new");
        let old = base.join("Hermes.app.treecore-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Hermes.app");
        let tmp = base.join("Hermes.app.treecore-update-new"); // intentionally absent
        let old = base.join("Hermes.app.treecore-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Hermes.app");
        let tmp = base.join("Hermes.app.treecore-update-new"); // absent
        let old = base.join("Hermes.app.treecore-update-old");
        write_marker(&target, "OLD");
