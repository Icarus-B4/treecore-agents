...
//! TREECORE_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for TREECORE_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub treecore_home: Option<String>,
}
...
#[tauri::command]
pub async fn launch_treecore_cli_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_treecore_cli_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Hermes desktop at {}. The desktop build step \
             may have been skipped or failed. Run `treecore_cli desktop` from a \
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_treecore_cli_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
...
pub(crate) fn resolve_treecore_cli_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_treecore_cli_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
...
pub(crate) fn treecore_cli_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".treecore-bootstrap-complete").exists()
        && resolve_treecore_cli_desktop_exe(install_root).is_some()
}
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    // writeFileAtomic(). treecore_cli_is_installed() only checks existence, so a
    // partial direct write would incorrectly enable the launcher fast path.
    let tmp_path = install_root.join(".treecore-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_treecore_cli_desktop_exe(install_root).ok_or_else(|| {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_treecore_cli_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::treecore_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // the repo INTO at $HermesHome\treecore-agent. Mirrors treecore_constants.
    let treecore_home = args
        .treecore_home
        .clone()
        .unwrap_or_else(|| crate::paths::treecore_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&treecore_home).join("treecore-agent");
    ...
    // Copy ourselves to TREECORE_HOME/treecore-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_treecore_home() {
        tracing::warn!(?err, "failed to copy installer into TREECORE_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    treecore_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, treecore_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn unique_tmp_dir(tag: &str) -> PathBuf {
        let base = std::env::temp_dir().join(format!(
            "treecore_cli-bootstrap-test-{tag}-{}-{}",
            std::process::id(),
    ...
    #[test]
    fn resolve_treecore_cli_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_treecore_cli_desktop_app(&root)
    ...
    #[test]
    fn resolve_treecore_cli_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_treecore_cli_desktop_app(&root).is_none(),
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".treecore-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".treecore-bootstrap-complete");
        let tmp_path = root.join(".treecore-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
        ...
        assert!(
            treecore_cli_is_installed(&root),
    ...
    #[test]
    fn treecore_cli_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".treecore-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".treecore-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".treecore-bootstrap-complete.tmp").exists(),
