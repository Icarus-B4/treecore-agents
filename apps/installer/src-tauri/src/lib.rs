...
pub fn run() {
    // Tracing → bootstrap-installer.log under TREECORE_HOME/logs/ so install
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // startup-grace handling used by launch_treecore_cli_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::treecore_home().join("treecore-agent");
                if bootstrap::treecore_cli_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_treecore_cli_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "treecore_cli already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_treecore_cli_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_treecore_home,
            paths::open_log_dir,
        ])
