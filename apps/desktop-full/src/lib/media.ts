...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.treecoreDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.treecoreDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.treecoreDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.treecoreDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
// (workspace, skills, ~/.treecore/cache, etc.); /api/media is intentionally
