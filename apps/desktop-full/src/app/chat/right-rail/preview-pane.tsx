        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.treecoreDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  useEffect(() => {
    if (
      ...
      !window.treecoreDesktop?.watchPreviewFile ||
      !window.treecoreDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.treecoreDesktop.onPreviewFileChanged(payload => {
    ...
    void window.treecoreDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.treecoreDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.treecoreDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
