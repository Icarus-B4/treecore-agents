...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { hermesDesktop: { readDir: typeof readDir } }).treecoreDesktop = { readDir }
})
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { hermesDesktop: unknown }).treecoreDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { hermesDesktop: unknown }).treecoreDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.treecoreDesktop is missing', async () => {
    delete (window as unknown as { hermesDesktop?: unknown }).treecoreDesktop
