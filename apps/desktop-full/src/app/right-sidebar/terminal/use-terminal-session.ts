...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.treecoreDesktop?.terminal
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.treecoreDesktop?.readClipboard?.()) ?? ''
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.treecoreDesktop?.terminal?.write(sessionId, `${command}\r`)
