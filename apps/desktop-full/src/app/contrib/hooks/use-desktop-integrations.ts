...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    window.treecoreDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.treecoreDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.treecoreDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  useEffect(() => {
    const unsubscribe = window.treecoreDesktop?.onDeepLink?.(payload => {
    ...
    void window.treecoreDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.treecoreDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.treecoreDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
