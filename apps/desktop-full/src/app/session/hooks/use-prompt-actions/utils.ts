...
export async function readImageForRemoteAttach(
  ...
  const dataUrl = await window.treecoreDesktop?.readFileDataUrl(filePath)
...
export async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.treecoreDesktop?.readFileDataUrlForAttach ?? window.treecoreDesktop?.readFileDataUrl
