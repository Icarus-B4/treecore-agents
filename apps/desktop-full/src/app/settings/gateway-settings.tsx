...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.treecoreDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.treecoreDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.treecoreDesktop.applyConnectionConfig(payload())
        : await window.treecoreDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.treecoreDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.treecoreDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.treecoreDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.treecoreDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.treecoreDesktop.getConnectionConfig(scope)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.treecoreDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.treecoreDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.treecoreDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.treecoreDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.treecoreDesktop?.getConnectionConfig) {
    ...
    <SettingsContent bare={embedded}>
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.treecoreDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
