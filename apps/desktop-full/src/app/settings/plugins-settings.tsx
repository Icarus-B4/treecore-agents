...
function reveal(file: string) {
  void window.treecoreDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // treecore_home breaks against a remote backend (#66899).
    const dir = await window.treecoreDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.treecoreDesktop?.openDir?.(dir)
