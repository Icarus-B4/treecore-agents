...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'win32', activeRoot: 'C:\\Users\\me\\AppData\\Local\\hermes\\treecore-agent' }
      })
    )
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    act(() => {
      desktop.emitBootstrapEvent({
        ...
        active: false,
        platform: 'win32',
        activeRoot: 'C:\\Users\\me\\AppData\\Local\\hermes\\treecore-agent-repaired'
      })
    })
  ...
  it('opens the remote connection form from the first-run choice', async () => {
    installDesktopMock(
      bootstrapState({
        setupChoice: { platform: 'linux', activeRoot: '/home/me/.treecore/treecore-agent' }
      })
    )
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    const desktop = installDesktopMock(
      bootstrapState({
        unsupportedPlatform: {
          platform: 'darwin',
          activeRoot: '/Users/me/.treecore/treecore-agent',
