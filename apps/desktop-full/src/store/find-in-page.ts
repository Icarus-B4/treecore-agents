...
export function closeFindBar(): void {
  ...
  void window.treecoreDesktop?.stopFindInPage()
}
...
export function setFindQuery(query: string): void {
  ...
  void window.treecoreDesktop?.findInPage(query, { forward: true, findNext: false })
}
...
export function findNext(): void {
  ...
  if (query) {
    void window.treecoreDesktop?.findInPage(query, { forward: true, findNext: true })
  }
}
...
export function findPrevious(): void {
  ...
  if (query) {
    void window.treecoreDesktop?.findInPage(query, { forward: false, findNext: true })
  }
}
...
export function initFindInPageListener(): () => void {
  ...
  if (listenerRefs === 1) {
    detachListener = window.treecoreDesktop?.onFoundInPage?.(result => {
