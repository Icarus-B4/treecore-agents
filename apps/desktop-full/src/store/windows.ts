...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.treecoreDesktop?.openSessionWindow === 'function'
}

// True when the shell can open a full peer app window (⌘⇧N / "New Window").
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.treecoreDesktop?.openWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.treecoreDesktop.openSessionWindow(sessionId, opts),
...
export async function openNewWindow(): Promise<void> {
  ...
  await runWindowOpen(() => window.treecoreDesktop.openWindow(), 'Could not open a new window')
}
