...
// /api/profiles/active `current`). "default" is the root ~/.treecore. This is the
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.treecoreDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new TREECORE_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.treecoreDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.treecoreDesktop?.getConnection
...
// Switch to the default (root ~/.treecore) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.treecoreDesktop?.touchBackend?.(target).catch(() => undefined)
}
