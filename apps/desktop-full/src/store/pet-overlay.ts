...
function pushNow(): void {
  window.treecoreDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.treecoreDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.treecoreDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.treecoreDesktop?.petOverlay?.close()
}
