...
// reaches the gateway); everything else routes through window.treecoreDesktop.git,
...
// Install a review bridge on window.treecoreDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    ...
    expect(
      (window.treecoreDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
