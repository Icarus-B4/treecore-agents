...
in the companion
[`treecore_cli-example-plugins`](https://github.com/NousResearch/treecore_cli-example-plugins)
repo instead.

User- and agent-authored plugins load at runtime from
`$TREECORE_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`treecore_cli-desktop-plugins` skill.
