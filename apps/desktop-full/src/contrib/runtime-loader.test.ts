...
// getStatus would supply the connected backend's treecore_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ treecore_home: '/remote/box/.treecore' }))
...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local root, never the backend treecore_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.treecore/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.treecore/desktop-plugins')
    // The remote backend's treecore_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.treecore/desktop-plugins')
  })
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved local root, never the backend treecore_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.treecore/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.treecore/desktop-plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.treecore/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
