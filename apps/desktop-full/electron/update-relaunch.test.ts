 ...
 *      internals; TREECORE_HOME / HERMES_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.treecore/treecore-agent'
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  ...
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.treecore/treecore-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
...
test('collectRelaunchEnv preserves TREECORE_HOME + HERMES_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    TREECORE_HOME: '/home/u/.treecore',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.treecore/treecore-agent/apps/desktop/release/linux-unpacked/Hermes',
    args: ['hermes://open/agent/42', "--note=it's fine"],
    env: { TREECORE_HOME: '/home/u/.treecore', HERMES_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export TREECORE_HOME='\/home\/u\/\.treecore'/)
