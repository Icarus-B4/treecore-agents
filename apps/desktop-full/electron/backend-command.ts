...
//
// The desktop app launches its own headless backend via `treecore serve` — it
...
// app hasn't updated yet, or an older `treecore` resolved from PATH) only knows
 ...
 * `-m treecore_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `treecore_cli/subcommands/dashboard.py` source registers
