...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'treecore_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'treecore_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'treecore_cli.uninstall', '--mode', 'full'])
})
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.treecore/treecore-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.treecore/treecore-agent',
    uninstallArgs: ['-m', 'treecore_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/hermes/linux-unpacked',
    treecoreHome: '/home/x/.treecore'
  })
  ...
  assert.match(script, /'-m' 'treecore_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/hermes\/linux-unpacked'/)
  assert.match(script, /export TREECORE_HOME='\/home\/x\/\.treecore'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.treecore/treecore-agent',
    agentRoot: '/home/x/.treecore/treecore-agent',
    uninstallArgs: ['-m', 'treecore_cli.uninstall', '--mode', 'full'],
    appPath: null,
    treecoreHome: '/home/x/.treecore'
  })

  // System python + source on PYTHONPATH so import treecore_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.treecore\/treecore-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'treecore_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'treecore_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    treecoreHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'treecore_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  ...
  // PYTHONPATH set so a system python can import treecore_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\hermes;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "treecore_cli\.uninstall" "--mode" "full"/)
