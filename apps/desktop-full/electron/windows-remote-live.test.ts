...
test.skipIf(!liveHost || !liveUser || !configuredHermes)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredHermes.replace('hermes.exe', 'python.exe')}' -m treecore_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        ...
        const terminate = `& '${python}' -m treecore_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.treecorePath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m treecore_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m treecore_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
