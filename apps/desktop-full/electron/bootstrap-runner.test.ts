...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    ...
    sourceRepoRoot: null,
    treecoreHome: '/tmp/hermes-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'treecore-agents', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'treecore-agents')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/treecore-agents',
      treecoreHome: '/tmp/hermes'
    }),
    ['--dir', '/tmp/treecore-agents', '--treecore-home', '/tmp/hermes', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/treecore-agents', '--treecore-home', '/tmp/hermes', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      ...
      treecoreHome: '/tmp/home'
    }),
    ['--dir', '/tmp/hermes', '--treecore-home', '/tmp/home', '--branch', 'main']
  )
})
