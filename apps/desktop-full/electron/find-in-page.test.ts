...
describe('installFoundInPageForwarder', () => {
  test('forwards found-in-page to the sender as a formatted payload', () => {
    ...
    assert.deepEqual(wc.calls.send, [{ channel: 'treecore:found-in-page', payload: { activeMatchOrdinal: 2, count: 5 } }])
  })
  ...
  test('handles missing fields without throwing', () => {
    ...
    assert.deepEqual(wc.calls.send, [{ channel: 'treecore:found-in-page', payload: { activeMatchOrdinal: 0, count: 0 } }])
  })
  ...
  test('two forwarders installed on distinct webContents do not cross-fire', () => {
    ...
    assert.deepEqual(wcA.calls.send, [
      { channel: 'treecore:found-in-page', payload: { activeMatchOrdinal: 1, count: 1 } }
    ])
