...
//   3. resolveVenvTreecoreCommand() — must probe the venv python via
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvTreecoreCommand>[2]> = {}) {
  return {
    isWindows: true,
...
test('resolveVenvHermesCommand: returns null off Windows', () => {
  ...
  assert.equal(resolveVenvTreecoreCommand('/root/venv/Scripts/hermes.exe', [], deps), null)
})

test('resolveVenvHermesCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvTreecoreCommand('/root/venv/Scripts/hermes.cmd', [], deps), null)
})

test('resolveVenvHermesCommand: returns null when the basename is not hermes/hermes.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvTreecoreCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvHermesCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvTreecoreCommand('/root/venv/bin/hermes.exe', [], deps), null)
})
...
test('resolveVenvHermesCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  ...
  const deps = makeDeps({
    canImportTreecoreCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvTreecoreCommand('/root/venv/Scripts/hermes.exe', ['serve'], deps)
...
test('resolveVenvHermesCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvTreecoreCommand('/root/venv/Scripts/hermes.exe', ['serve', '--port', '0'], deps)
...
test('resolveVenvHermesCommand: is case-insensitive on hermes.exe and the Scripts dir name', () => {
  ...
  assert.ok(resolveVenvTreecoreCommand('/root/venv/Scripts/HERMES.EXE', [], deps))
  assert.ok(resolveVenvTreecoreCommand('/root/venv/SCRIPTS/hermes.exe', [], deps))
})
