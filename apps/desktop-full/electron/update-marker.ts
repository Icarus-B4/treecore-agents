...
export function markerPath(treecoreHome) {
  return path.join(treecoreHome, '.treecore-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(treecoreHome)
  let raw
 ...
 *
 * The Tauri-based treecore-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(treecoreHome, pid, { now = Date.now } = {}) {
  const file = markerPath(treecoreHome)
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(treecoreHome, opts)
