...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `treecore_cli-dev-mock-${Date.now()}`))
  const treecoreHome = path.join(root, 'treecore_cli-home')
  const userDataDir = path.join(root, 'electron-user-data')
  fs.mkdirSync(treecoreHome, { recursive: true })
  fs.mkdirSync(userDataDir, { recursive: true })
  return { root, treecoreHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(treecoreHome, mockUrl) {
  fs.writeFileSync(
    path.join(treecoreHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(treecoreHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.treecoreHome, mock.url)
  console.log(`  TREECORE_HOME: ${sandbox.treecoreHome}`)

  const electronBin = findElectron()

  const env = {
    ...process.env,
    TREECORE_HOME: sandbox.treecoreHome,
