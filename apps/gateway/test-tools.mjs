...
ws.on('open', async () => {
  ...
  console.log('--- /write ---')
  const r1 = await send('prompt.submit', { session_id: 't1', text: '/write D:/local/treecore_cli/apps/gateway/test-out.txt hello-world' })
  console.log('RESP r1:', JSON.stringify(r1).slice(0, 100))
  console.log('--- /read ---')
  const r2 = await send('prompt.submit', { session_id: 't1', text: '/read D:/local/treecore_cli/apps/gateway/test-out.txt' })
