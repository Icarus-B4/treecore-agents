...
build sandbox. ``nix/python.nix`` sets ``TREECORE_NIX_BUILD=1`` on the
...
_IN_NIX_BUILD = os.environ.get("TREECORE_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for treecore-agent is not supported.\n"
    "Hermes is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://treecore-agent.nousresearch.com/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Hermes Nix derivation sets TREECORE_NIX_BUILD=1. If it does, file a bug."
)
