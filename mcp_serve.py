...
logger = logging.getLogger("treecore.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using TREECORE_HOME."""
    try:
        from treecore_constants import get_treecore_home
        return get_treecore_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("TREECORE_HOME", Path.home() / ".treecore")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from treecore_constants import get_treecore_home
        directory_file = get_treecore_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("TREECORE_HOME", Path.home() / ".treecore")
...
class EventBridge:
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            from treecore_constants import get_treecore_home
            db_file = get_treecore_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("TREECORE_HOME", Path.home() / ".treecore")) / "state.db"
        try:
