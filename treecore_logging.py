...
``~/.treecore/logs/`` (profile-aware via ``get_treecore_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``treecore_cli.web_server``,
    ``treecore_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from treecore_constants import get_config_path, get_treecore_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_treecore_session_injector", False):
    ...
    _session_record_factory._treecore_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``treecore logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``treecore logs --component gateway``.
    "gateway": ("gateway", "treecore_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("treecore_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "treecore_cli.web_server",
        "treecore_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    treecore_home
        ...
        ``get_treecore_home()`` (profile-aware).
    log_level
    ...
    home = treecore_home or get_treecore_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_treecore_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._treecore_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the treecore group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from treecore_cli.config import is_managed
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._treecore_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="treecore-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_treecore_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse treecore_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when treecore_cli.config isn't importable (bare
        # treecore_logging consumers).
        try:
            from treecore_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
