"""CLI entry point for the treecore-agent ACP adapter.

Loads environment variables from ``~/.treecore/.env``, configures logging
...
try:
    ...
    # shadowing Hermes's own modules — ``treecore acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from TREECORE_HOME (default ``~/.treecore``)."""
    from treecore_cli.env_loader import load_treecore_dotenv

    treecore_home = get_treecore_home()
    loaded = load_treecore_dotenv(treecore_home=treecore_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", treecore_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.treecore/node/ "
...
def _run_setup() -> None:
    from treecore_cli.main import main as treecore_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "treecore", "model"]
        treecore_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting treecore-agent ACP adapter")
    ...
    if os.environ.get("TREECORE_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
