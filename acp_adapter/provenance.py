...
This is an additive Hermes extension surfaced under ACP ``_meta.treecore`` so
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.treecore.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_treecore_session_id: The live internal Hermes DB session id
            (``state.agent.session_id``).
        previous_treecore_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"treecore": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_treecore_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_treecore_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_treecore_session_id}
    ...
    rotated = bool(
        ...
        and previous_treecore_session_id != current_treecore_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentHermesSessionId": current_treecore_session_id,
    ...
    if previous_treecore_session_id:
        provenance["previousHermesSessionId"] = previous_treecore_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"treecore": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"treecore": {"sessionProvenance": prov}}
