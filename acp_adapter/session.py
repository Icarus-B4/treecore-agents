...
Sessions are persisted to the shared SessionDB (``~/.treecore/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    Windows ACP clients can launch ``treecore acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from treecore_constants import windows_path_to_wsl
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["treecore-acp"]):
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            ...
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.treecore/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``TREECORE_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_treecore_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=treecore_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see treecore_state.get_messages_as_conversation).
        try:
