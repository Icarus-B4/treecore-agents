...
from tools.approval import (
    reset_treecore_interactive_context,
    set_treecore_interactive_context,
)
...
try:
    from treecore_cli import __version__ as TREECORE_VERSION
except Exception:
    TREECORE_VERSION = "0.0.0"
...
# already uses (`treecore_cli/moa_cmd.py`). This bounds each provider's row, not
...
class HermesACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Hermes inventory is also used by ``treecore model``, the TUI,
        ...
        try:
            ...
            from treecore_cli.models import normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from treecore_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_treecore_session_id: str,
        previous_treecore_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.treecore.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_treecore_session_id,
                previous_treecore_session_id=previous_treecore_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_treecore_session_id: Optional[str] = None,
        previous_treecore_session_id: Optional[str] = None,
    ) -> None:
        ...
        split during a turn), pass ``previous_treecore_session_id`` so the
        attached ``_meta.treecore.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # treecore_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_treecore_session_id or session_id,
            previous_treecore_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["treecore-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from treecore_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from treecore_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="treecore-agent", version=TREECORE_VERSION),
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.treecore`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"treecore": {"compactionSummary": True}}
        if kind == "merged":
            return {"treecore": {"containsCompactionSummary": True}}
        return None
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # tools.approval (set_treecore_interactive_context) rather than
        # os.environ["TREECORE_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue TREECORE_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind TREECORE_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # model emits absolute paths under ~/.treecore/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global TREECORE_CRON_SESSION (#37968).
            ...
            interactive_token = set_treecore_interactive_context(True)
            ...
            previous_session_id = os.environ.get("TREECORE_SESSION_ID")
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_treecore_interactive_context(interactive_token)
                # Restore TREECORE_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("TREECORE_SESSION_ID", None)
                else:
                    os.environ["TREECORE_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            pre_turn_treecore_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (TREECORE_SESSION_KEY in
        ...
        # _meta.treecore.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_treecore_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_treecore_id
            and pre_turn_treecore_id
            and post_turn_treecore_id != pre_turn_treecore_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_treecore_session_id=post_turn_treecore_id,
                    previous_treecore_session_id=pre_turn_treecore_id,
                )
            except Exception:
                logger.debug(
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Hermes Agent v{TREECORE_VERSION}"
