...
Uses a file-based lock (~/.treecore/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `treecore update` reloads
# the module) fail with ModuleNotFoundError for treecore_time et al.
...
from treecore_time import now as _treecore_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy treecore_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from treecore_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``treecore tools`` config for the ``cron`` platform.
    ...
    try:
        from treecore_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
# (TREECORE_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse TREECORE_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("TREECORE_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid TREECORE_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_treecore_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_treecore_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.treecore/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``treecore update`` / ``treecore gateway stop`` / systemd restart, or an
...
_treecore_home: Path | None = None


def _get_treecore_home() -> Path:
    """Resolve Hermes home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active TREECORE_HOME at call time
    ...
    """
    return _treecore_home or get_treecore_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    treecore_home = _get_treecore_home()
    lock_dir = treecore_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("TREECORE_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid TREECORE_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within TREECORE_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against TREECORE_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_treecore_home() / "scripts"
    ...
    # escape — scripts MUST reside within TREECORE_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("TREECORE_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `treecore setup`), or pin a "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`treecore setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _treecore_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _treecore_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    #
    # Bounded with its own timeout (separate from TREECORE_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("TREECORE_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid TREECORE_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_treecore_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_treecore_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed TREECORE_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads TREECORE_SESSION_PLATFORM
    #     and TREECORE_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via TREECORE_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     TREECORE_SESSION_PLATFORM == "telegram".
    ...
    #     TREECORE_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read TREECORE_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the TREECORE_CRON_AUTO_DELIVER_* vars set
    # below, so clearing TREECORE_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # TREECORE_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ TREECORE_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "TREECORE_CRON_AUTO_DELIVER_PLATFORM",
        "TREECORE_CRON_AUTO_DELIVER_CHAT_ID",
        "TREECORE_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["TREECORE_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `treecore chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with TREECORE_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $TREECORE_KANBAN_TASK -- letting
        ...
        # load_treecore_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this TREECORE_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_treecore_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from treecore_cli.env_loader import (
            load_treecore_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_treecore_dotenv(treecore_home=_get_treecore_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["TREECORE_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["TREECORE_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["TREECORE_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > TREECORE_MODEL env > config.yaml ``model:``
        ...
        try:
            ...
            _cfg_path = str(_get_treecore_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"TREECORE_MODEL={os.getenv('TREECORE_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `treecore model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from treecore_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("TREECORE_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_treecore_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject TREECORE_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # TREECORE_HOME. When a workdir is configured, also inject project
        ...
        # override via TREECORE_CRON_TIMEOUT env var.  0 = unlimited.
        #
...
**Job ID:** {job_id}
**Run Time:** {_treecore_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_treecore_now().strftime('%b %d %H:%M')}"
...
def run_one_job(
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_treecore_home())
        )
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`treecore pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `treecore resume`. Logged once per
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _treecore_now().strftime('%H:%M:%S'))
            try:
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _treecore_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set TREECORE_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("TREECORE_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid TREECORE_CRON_MAX_PARALLEL value; defaulting to unbounded")
