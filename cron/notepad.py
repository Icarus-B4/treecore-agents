...
Write path is the CLI (``treecore cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_treecore_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`treecore cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `treecore cron notepad {job_id} delete "
