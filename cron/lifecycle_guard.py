...
``treecore gateway restart`` (or ``launchctl kickstart ai.treecore.gateway``
or ``systemctl restart treecore-gateway``).  When the cron fires, the
...
``treecore cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``treecore gateway``, ``launchctl ... treecore-gateway``,
``systemctl ... treecore-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_TREECORE_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``treecore gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `treecore gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:treecore\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a treecore-gateway label. macOS launchd
    # labels look like `ai.treecore.gateway` / `treecore-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated treecore services (e.g.
    # `launchctl unload ai.treecore.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.treecore.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a treecore-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\btreecore[.\-]?gateway)"
    # Branch D: pkill / kill targeting the treecore gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.treecore.gateway-... \` / `  -- ...`,
...
# for `systemctl restart treecore-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart treecore-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.treecore.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart treecore-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        grep -c 'systemctl restart treecore-gateway' /var/log/syslog
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart treecore-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".treecore"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<TREECORE_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.treecore/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_treecore_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_treecore_home() falls back to Path.home(), which raises when
        # neither TREECORE_HOME nor HOME is resolvable (launchd/systemd
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "treecore gateway restart"
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `treecore gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `treecore gateway restart` from a shell outside "
