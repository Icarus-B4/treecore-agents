...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``treecore://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``treecore://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"treecore://blueprint/{quote(blueprint.key)}{qs}"
