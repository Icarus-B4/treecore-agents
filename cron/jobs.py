...
Jobs are stored in ~/.treecore/cron/jobs.json
Output is saved to ~/.treecore/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own TREECORE_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same TREECORE_HOME — so a job authored in profile
# `coder` lives in `~/.treecore/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_treecore_home()` (the active profile home), NOT `get_default_treecore_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever TREECORE_HOME the
...
# `_get_treecore_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
TREECORE_DIR = get_treecore_home().resolve()
...
CRON_DIR = TREECORE_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``treecore cron status`` process share it
...
# threshold in `treecore cron status` (treecore_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_treecore_home()
       (context-local override, then the TREECORE_HOME env var) — so a test
       or embedder that re-points TREECORE_HOME after this module was
...
# the cron inactivity timeout is disabled (TREECORE_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. TREECORE_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``TREECORE_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _jobs_lock():
    ...
    and standalone ``treecore`` CLI invocations, which previously shared no lock
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec treecore treecore cron create ...`` — ``docker exec``
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # against `treecore_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                treecore_tz = _treecore_now().tzinfo
                dt = dt.replace(tzinfo=treecore_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _treecore_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    ...
    """
    target_tz = _treecore_now().tzinfo
    if dt.tzinfo is None:
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall treecore-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `treecore cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``treecore cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `treecore cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``treecore cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``treecore cron
    ...
    try:
        for _attempt in range(5):
            ...
            try:
                with os.fdopen(fd, "w", encoding="utf-8") as f:
                    json.dump(
                        {"jobs": jobs, "updated_at": _treecore_now().isoformat()},
                        f,
                        indent=2,
                    )
                    f.flush()
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_treecore_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.treecore/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.treecore/scripts/, .sh/.bash via bash, else Python). Each
    ...
    # covered, not just `treecore cron create`.
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _treecore_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _treecore_now().isoformat(),
        },
    )
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = (
            ...
            f"- removed at: {_treecore_now().isoformat()}\n\n"
    ...
    with _jobs_lock():
        ...
        for job in jobs:
            ...
            if not isinstance(claim, dict) or claim.get("by") != expected_owner:
                return False
            claim["at"] = _treecore_now().isoformat()
            save_jobs(jobs)
            return True
    return False
...
def _machine_id() -> str:
    ...
    Uses ``TREECORE_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("TREECORE_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from TREECORE_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one TREECORE_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from TREECORE_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`treecore_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``treecore_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _treecore_now().strftime("%Y-%m-%d_%H-%M-%S")
