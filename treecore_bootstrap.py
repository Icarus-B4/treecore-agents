...
(``treecore``, ``treecore-agent``, ``treecore-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``treecore_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``treecore_bootstrap``, never ``treecore_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    """
    root = src_root or os.environ.get("TREECORE_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``TREECORE_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("TREECORE_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import treecore_bootstrap``
# (or ``from treecore_bootstrap import apply_windows_utf8_bootstrap``) at
