# Treecore Agent — Web UI

Browser-based dashboard for managing Treecore Agent configuration, API keys, and monitoring active sessions.
...
`treecore dashboard` on port 9120 serves the **built** bundle from `treecore_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Treecore Agent” — mixed case, no Mondwest, no `text-display` |
