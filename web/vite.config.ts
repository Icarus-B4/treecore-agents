...
const BACKEND = process.env.HERMES_DASHBOARD_URL ?? "http://127.0.0.1:9120";

/**
 * In production the Python `treecore dashboard` server injects a one-shot
      ...
      try {
        ...
        if (!match) {
          console.warn(
            `[hermes] Could not find session token in ${BACKEND} — ` +
              `is \`treecore dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        console.warn(
          `[hermes] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`treecore dashboard\` or set HERMES_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      // Same host as `treecore dashboard` must serve these; Vite has no
