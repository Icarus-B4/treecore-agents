...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'treecore update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "treecore update";
    return (
      t.status.updateHermesConfirmMessage ??
      `This will run 'treecore update' (${cmd}) and restart the gateway when it finishes.`
    );
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Treecore gateway process. Connected channels and active sessions will reconnect afterward."
      }
