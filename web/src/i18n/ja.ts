...
export const ja: Translations = {
  ...
  status: {
    ...
    stopped: "停止",
    updateHermes: "Treecore を更新",
    updatingHermes: "Treecore を更新しています…",
  ...
  pluginsPage: {
    ...
    headline:
      "Treecore プラグインを発見、インストール、有効化、更新します (`treecore plugins` 相当)。",
  ...
  env: {
    ...
    hideValue: "値を非表示",
    customTitle: "カスタムキー",
    customHint: "Treecore が認識しない、.env に保存された任意の環境変数。スキル、MCP サーバー、または独自のツール用に環境変数を注入するために使用します。",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "実際のセッション履歴から獲得できる Treecore のコレクタブル バッジです。既知の未達成の実績は「Discovered」として表示され、Secret 実績は最初の該当する挙動が検出されるまで非表示のままです。",
      scan_subtitle:
        "Treecore のセッション履歴をスキャンしています。履歴が大きい場合、初回スキャンには 5～10 秒かかることがあります。",
    },
    actions: {
      rescan: "再スキャン",
    },
    stats: {
      ...
      latest: "最新",
      latest_hint_empty: "Treecore をもっと使ってみてください",
      none_yet: "まだありません",
    },
    state: {
      unlocked: "解除済み",
    ...
    guide: {
      ...
      secret_body:
        "シークレットはトリガー条件を隠しています。Treecore が関連するシグナルを検出すると、カードは「Discovered」になり、要件が表示されます。",
      scan_status_header: "スキャン状況",
      scan_status_body:
        "Treecore はローカル履歴を一度スキャンし、その後カードが自動的に表示されます。数秒かかってもスタックしているわけではありません。",
