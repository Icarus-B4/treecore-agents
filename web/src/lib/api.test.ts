...
import { api, dashboardPath, fetchJSON } from "./api";
...
describe("dashboardPath", () => {
  it("keeps dashboard navigation inside a reverse-proxy prefix", () => {
    expect(dashboardPath("/login", "/treecore")).toBe("/treecore/login");
    expect(dashboardPath("config", "/treecore")).toBe("/treecore/config");
    expect(dashboardPath("/config", "")).toBe("/config");
  });
});
