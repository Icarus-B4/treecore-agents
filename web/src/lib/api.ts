import { buildHermesWebSocketUrl } from "@treecore/shared";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``treecore update``, ``treecore gateway restart``, etc.). A tab kept
...
export const api = {
  ...
  logout: () =>
    fetch(`${BASE}/auth/logout`, {
      ...
      window.location.assign(dashboardPath("/login"));
      return r;
    }),
  getSessions: (
    limit = 20,
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``treecore update``. */
