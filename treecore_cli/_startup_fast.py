...
This module is imported by ``treecore_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no treecore_cli.config,
...
``treecore_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(treecore_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect TREECORE_HOME?"""
    active_profile = os.path.join(treecore_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    treecore_home = os.environ.get("TREECORE_HOME", "").strip()
    if treecore_home:
        return treecore_home
    return os.path.join(os.path.expanduser("~"), ".treecore")
...
def container_mode_may_be_active() -> bool:
    ...
    treecore_home = os.environ.get("TREECORE_HOME", "").strip()
    if treecore_home:
        if os.path.exists(os.path.join(treecore_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(treecore_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(treecore_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".treecore")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect TREECORE_HOME.
    """
...
def print_fast_version_info() -> None:
    from treecore_cli import __release_date__, __version__
    ...
    print("Run 'treecore version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``treecore --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the TREECORE_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("TREECORE_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
