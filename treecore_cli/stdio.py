...
Entry points (``cli.py`` ``main``, ``treecore_cli/main.py`` CLI dispatch,
...
def configure_windows_stdio() -> bool:
    ...
    Set ``TREECORE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("TREECORE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # already-running shells don't see the change — which means treecore
...
def _default_windows_editor() -> str:
    ...
    ``treecore config edit`` both honour ``$EDITOR``.  Users who prefer a
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\treecore\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``treecore`` from right after install) retain their old PATH.
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "treecore", "git", "cmd"),
        os.path.join(local_appdata, "treecore", "git", "bin"),
        os.path.join(local_appdata, "treecore", "git", "usr", "bin"),
        # Hermes venv Scripts directory — host of the treecore.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes treecore, but harmless to include.
        os.path.join(local_appdata, "treecore", "treecore-agent", "venv", "Scripts"),
