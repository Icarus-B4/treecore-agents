...
This module backs ``treecore console`` and is intentionally narrower than the
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'treecore doctor'")
        and last.startswith("Run 'treecore setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="treecore", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `treecore "):
        return ""
    return summary
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("treecore_cli.main")
    builder = getattr(module, builder_name)
...
def _register_command_family(
    ...
    *,
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `treecore {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `treecore {usage}`?",
        )
...
class HermesConsoleEngine:
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "treecore":
                tokens = tokens[1:]
            if not tokens:
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        extracted = {
            "version": (
                "treecore_cli.subcommands.version",
            ...
            "debug": (
                "treecore_cli.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "treecore_cli.subcommands.prompt_size",
            ...
            "insights": (
                "treecore_cli.subcommands.insights",
            ...
            "security": (
                "treecore_cli.subcommands.security",
            ...
            "backup": (
                "treecore_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "treecore_cli.subcommands.import_cmd",
            ...
            "config": (
                "treecore_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "treecore_cli.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "treecore_cli.subcommands.plugins",
            ...
            "skills": (
                "treecore_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "treecore_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "treecore_cli.subcommands.pairing",
            ...
            "webhook": (
                "treecore_cli.subcommands.webhook",
            ...
            "hooks": (
                "treecore_cli.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "treecore_cli.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "treecore_cli.subcommands.profile",
        ...
        self.register(
            ...
            _adder_handler("send", (), "treecore_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("treecore_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "treecore_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("treecore_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("treecore_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        ...
        if first in blocked_top:
            return f"`treecore {first}` is not available in Hermes Console."
        blocked_pairs = {
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from treecore_cli.logs import list_logs, tail_log
...
def _config_show(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from treecore_cli.config import show_config
...
def _config_set(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    key = args[0]
    value = " ".join(args[1:])
    from treecore_cli.config import set_config_value
...
def _config_migrate(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from treecore_cli.config import migrate_config
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from treecore_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="treecore_cli.subcommands.profile",
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from treecore_cli.cron import cron_list
...
def _cron_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from treecore_cli.cron import cron_status
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from treecore console")
...
def run_console_repl(
    *,
    ...
    """Run the local ``treecore console`` REPL."""
    ...
    while True:
        if interactive:
            print("treecore> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
