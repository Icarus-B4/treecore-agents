...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``treecore desktop`` / ``treecore gui``)
     Built inside the agent checkout under ``$TREECORE_HOME/treecore-agent/``:
       ...
       - ``$TREECORE_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$TREECORE_HOME``:
...
``treecore uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(treecore_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return treecore_home / "treecore-agent"
...
def source_built_gui_artifacts(treecore_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``treecore desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``treecore-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(treecore_home)
    ...
    return [
        ...
        treecore_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        ...
        # ``uninstall_gui``.
        from treecore_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `treecore desktop` installs. Its icon lives
...
def agent_is_installed(treecore_home: Path) -> bool:
    """Return True when a usable Python agent install exists under TREECORE_HOME.
    ...
    if (agent_root / "treecore_cli").is_dir():
        return True
...
def gui_is_installed(treecore_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(treecore_home):
        if p.exists():
            return True
...
def gui_install_summary(treecore_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = treecore_home if treecore_home is not None else get_treecore_home()
...
def uninstall_gui(treecore_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``treecore-agent/treecore_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$TREECORE_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from treecore_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove treecore' or "
            "'sudo dnf remove treecore'). AppImage builds are a single file you "
