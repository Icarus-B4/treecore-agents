"""Dashboard process-hygiene helpers — extracted from ``treecore_cli/main.py``.
...
``_detect_concurrent_treecore_instances``) are lifted verbatim. References to
helpers that STAY in ``treecore_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``treecore_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``treecore_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``treecore_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``treecore dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``treecore update`` replaces files on disk, the running
    ...
    ``treecore-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``treecore serve`` as
    ...
    environment variable ``TREECORE_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`treecore serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "treecore.*dashboard"`) keeps us consistent
            # with `treecore_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``treecore dashboard`` / ``treecore serve`` processes.

    Called at the end of ``treecore update`` (default ``reason``) and also
    from ``treecore dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``treecore update`` path), a detected
    ``treecore-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``treecore-serve.service``) has its owning unit
    ...
    # backend child, it sets TREECORE_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("TREECORE_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `treecore update` path) — `--stop` must
    ...
    #    remote backend (treecore serve) under Restart=on-failure never comes
...
def _detect_concurrent_treecore_instances(
    ...
    The Hermes Desktop Electron app spawns ``treecore.EXE`` as a backend child,
    so during ``treecore update`` the user-invoked process and the desktop's
    ...
    shims (``treecore.exe`` / ``treecore-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``treecore update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``treecore.exe``) is a separate process from the Python
    ...
    for shim in _m()._treecore_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated treecore.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``treecore update``
    ...
    #      treecore.exe sitting *under* a non-Hermes parent (e.g. a Hermes
