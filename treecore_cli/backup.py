"""
Backup and import commands for treecore CLI.

`treecore backup` creates a zip archive of the entire ~/.treecore/ directory
(excluding the treecore-agent repo and transient files).

`treecore import` restores from a backup zip, overlaying onto the current
TREECORE_HOME root.
"""
...
from treecore_constants import get_default_treecore_root, get_treecore_home, display_treecore_home
...
# ``treecore-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/treecore-agent/``
...
# TREECORE_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "treecore-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under TREECORE_HOME) —
...
# File names that ``treecore import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE TREECORE_HOME
...
@contextmanager
def _backup_operation_lock(treecore_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = treecore_home / ".backup.lock"
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside TREECORE_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to treecore root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``treecore-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/treecore-agent/`` — must be preserved.
        if part == "treecore-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside TREECORE_HOME.
...
# to the user as a hung `treecore update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # treecore_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    ...
    treecore_root = get_default_treecore_root()

    if not treecore_root.is_dir():
        print(f"Error: Hermes home directory not found at {treecore_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(treecore_root):
            _run_backup_locked(args, treecore_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, treecore_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"treecore-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"treecore-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_treecore_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(treecore_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(treecore_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``treecore-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "treecore-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(treecore_root)
    ...
    # outside TREECORE_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_treecore_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: treecore import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a treecore home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.treecore/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a treecore dir name
        if prefix in {".treecore", "treecore"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_treecore_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (treecore_root / "config.yaml").exists()
        has_env = (treecore_root / ".env").exists()
        ...
        treecore_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under TREECORE_HOME.
            ...
            target = treecore_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(treecore_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_treecore_home()}."
            )
        ...
        profiles_dir = treecore_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # treecore_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: treecore profile list  (after installing treecore)")

        # Guidance
        print()
        if not (treecore_root / "treecore-agent").is_dir():
            print("Note: The treecore-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: treecore update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  treecore -p {pname} gateway install")

        print("Done. Your Hermes configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and treecore backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to TREECORE_HOME).
...
# — `treecore update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $TREECORE_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside TREECORE_HOME and the entry is silently
...
def _quick_snapshot_root(treecore_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: treecore snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(treecore_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside TREECORE_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``treecore update``.
    ...
    Args:
        ...
        treecore_home: Override for the Hermes home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(treecore_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for treecore backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_treecore_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, treecore_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(treecore_root):
            return _write_full_zip_backup_locked(out_path, treecore_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, treecore_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``treecore_root`` to ``out_path``.
...
def _pre_update_backup_dir(treecore_home: Optional[Path] = None) -> Path:
    home = treecore_home or get_treecore_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of TREECORE_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<TREECORE_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``treecore update``) should continue even if the backup fails.
    """
    treecore_root = treecore_home or get_default_treecore_root()
    if not treecore_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(treecore_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, treecore_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `treecore claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of TREECORE_HOME under ``backups/`` before a
    ``treecore claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``treecore import <archive>``.  Writes to
    ``<TREECORE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `treecore import` and the
