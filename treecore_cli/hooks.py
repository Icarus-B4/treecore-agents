"""treecore hooks — inspect and manage shell-script hooks.

Usage::

    treecore hooks list
    treecore hooks test <event> [--for-tool X] [--payload-file F]
    treecore hooks revoke <command>
    treecore hooks doctor

Consent records live under ``~/.treecore/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.treecore/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``treecore hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: treecore hooks {list|test|revoke|doctor}")
        print("Run 'treecore hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.treecore/config.yaml.")
        print("See `treecore hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.treecore/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `treecore hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `treecore hooks test` and `treecore hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `treecore hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `treecore hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `treecore hooks doctor`.")
