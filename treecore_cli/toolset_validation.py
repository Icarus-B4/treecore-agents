...
``treecore-cli`` to the non-existent ``treecore``. ``resolve_toolset('treecore')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``treecore-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``treecore`` instead of ``treecore-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"treecore-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `treecore tools` to reconfigure."
        )
    return warnings
