...
``treecore -p <profile>`` joins the same board as the dispatcher that
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``TREECORE_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``TREECORE_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``treecore kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.treecore``. In Docker / custom
deployments where ``TREECORE_HOME`` points outside ``~/.treecore`` (e.g.
``/opt/treecore``), ``<root>`` is ``TREECORE_HOME``. Legacy env-var
overrides still work:

* ``TREECORE_KANBAN_DB`` — pin the database file path directly.
* ``TREECORE_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``TREECORE_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``TREECORE_KANBAN_DB``,
``TREECORE_KANBAN_WORKSPACES_ROOT``, and ``TREECORE_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/treecore-kanban-v1-spec.pdf`` for the full
...
from treecore_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``treecore_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from treecore_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("TREECORE_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active TREECORE_HOME so dispatcher- and
...
# ``TREECORE_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``TREECORE_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("TREECORE_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``TREECORE_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("TREECORE_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``TREECORE_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "TREECORE_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "treecore_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``treecore-agent``
...
def kanban_home() -> Path:
    ...
    1. ``TREECORE_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_treecore_root()``, which already returns ``<root>``
       when ``TREECORE_HOME`` is ``<root>/profiles/<name>``, and returns
       ``TREECORE_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``TREECORE_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("TREECORE_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from treecore_constants import get_default_treecore_root
    return get_default_treecore_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``treecore kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``TREECORE_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``treecore kanban boards
    ...
    env = os.environ.get("TREECORE_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``treecore kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``TREECORE_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("TREECORE_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``TREECORE_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("TREECORE_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``TREECORE_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("TREECORE_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``treecore kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``treecore_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``TREECORE_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (treecore_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``TREECORE_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("TREECORE_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``treecore_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from treecore_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``treecore gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from treecore_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see treecore_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from treecore_cli.sqlite_safe_read import has_live_connection
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``treecore kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``TREECORE_KANBAN_DB`` env → ``TREECORE_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `treecore kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from treecore_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        from treecore_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See treecore_state._WAL_INCOMPAT_MARKERS for detection logic.
                from treecore_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``treecore kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``TREECORE_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from treecore_cli.sqlite_safe_read import file_length_matches_header
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from treecore_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``treecore --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from treecore_cli import projects_db as _pdb
    ...
    # `treecore --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``treecore kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `treecore kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``treecore kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``treecore kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``treecore kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``treecore kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``treecore kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``TREECORE_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
...
#      via ``treecore kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``TREECORE_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``treecore kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``treecore kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from treecore_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from treecore_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        # `_default_spawn` invokes ``treecore -p <assignee>`` which fails
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `treecore kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from treecore_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_treecore_argv() -> list[str]:
    """Return the interpreter-bound Hermes CLI invocation."""
    # ``treecore_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``treecore`` package — there is no
    # ``treecore`` package to import.
    return [sys.executable, "-m", "treecore_cli.main"]


def _absolute_treecore_path(path: str) -> str:
...
def _treecore_path_argv(path: str) -> list[str]:
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_treecore_argv()
    return [_absolute_treecore_path(path)]


def _resolve_treecore_argv() -> list[str]:
    """Resolve the ``treecore`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$TREECORE_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("treecore")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\treecore.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m treecore_cli.main`` — fallback for setups where
       Hermes is launched from a venv and the ``treecore`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_treecore_bin`` for the same reason. Kept
    local (not imported from gateway) because ``treecore_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("TREECORE_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _treecore_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _treecore_path_argv(resolved_env_bin)
        return _module_treecore_argv()

    treecore_bin = _safe_which_no_cwd("treecore") if _IS_WINDOWS else shutil.which("treecore")
    if treecore_bin:
        return _treecore_path_argv(treecore_bin)
    return _module_treecore_argv()
...
def _resolve_worker_cli_toolsets(treecore_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``TREECORE_KANBAN_TASK`` is set.
    """
    ...
    try:
        ...
        token = set_treecore_home_override(treecore_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for TREECORE_HOME=%r (%s)",
            treecore_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``treecore -p <profile> chat -q ...`` subprocess.
    ...
    ``TREECORE_KANBAN_DB`` / ``TREECORE_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject TREECORE_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `treecore -p <name>` the
    # _apply_profile_override() runs *before* treecore_constants is imported.
    # If TREECORE_HOME is absent from the child's env, get_treecore_home() falls
    # back to Path.home() / ".treecore" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from treecore_cli.profiles import resolve_profile_env
    try:
        env["TREECORE_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via TREECORE_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # TREECORE_HOME never had profiles created.
        pass
    if task.tenant:
        env["TREECORE_TENANT"] = task.tenant
    env["TREECORE_KANBAN_TASK"] = task.id
    env["TREECORE_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `treecore kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["TREECORE_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["TREECORE_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["TREECORE_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["TREECORE_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["TREECORE_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["TREECORE_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`treecore -p <name>`
    # rewrites TREECORE_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_treecore_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["TREECORE_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["TREECORE_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["TREECORE_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["TREECORE_KANBAN_BOARD"] = resolved_board
    # TREECORE_PROFILE is the author the kanban_comment tool defaults to.
    # `treecore -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["TREECORE_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited TREECORE_TUI=1
    ...
    # older treecore builds on PATH that predate the flag's precedence.
    env.pop("TREECORE_TUI", None)

    cmd = [
        *_resolve_treecore_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped TREECORE_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("TREECORE_HOME"))
    if worker_toolsets:
    ...
    # `treecore kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`treecore` executable not found on PATH. "
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``treecore kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled TREECORE_PROFILE values like "treecore-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``treecore_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from treecore_constants import get_default_treecore_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``treecore kanban assignees`` for the terminal.
