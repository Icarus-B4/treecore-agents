"""Windows ConPTY bridge for the `treecore dashboard` chat tab.

Drop-in counterpart to ``treecore_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``treecore_cli.web_server`` consumes: ``spawn``, ``read``,
