...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or treecore -c.",
    ...
    # --- CLI Flags ---
    "treecore -c resumes your most recent CLI session. treecore -c \"project name\" resumes by title.",
    "treecore -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "treecore -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "treecore chat -t web,terminal enables only specific toolsets for a focused session.",
    "treecore chat -s github-pr-workflow preloads a skill at launch.",
    "treecore chat -q \"query\" runs a single non-interactive query and exits.",
    "treecore chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "treecore chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "treecore --yolo bypasses all dangerous command approval prompts for the entire session.",
    "treecore chat --source telegram tags the session for filtering in treecore sessions list.",
    "treecore -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "treecore doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "treecore dump outputs a compact setup summary — great for bug reports.",
    "treecore config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "treecore config edit opens config.yaml in your default editor.",
    "treecore config check scans for missing or stale configuration options.",
    "treecore sessions browse opens an interactive session picker with search.",
    "treecore sessions stats shows session counts by platform and database size.",
    "treecore sessions prune --older-than 30 cleans up old sessions.",
    "treecore skills search react --source skills-sh searches the skills.sh public directory.",
    "treecore skills check scans installed hub skills for upstream updates.",
    "treecore skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "treecore skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "treecore mcp add github --command npx adds MCP servers from the command line.",
    "treecore mcp serve runs Hermes itself as an MCP server for other agents.",
    "treecore auth add lets you add multiple API keys for credential pool rotation.",
    "treecore completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "treecore logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "treecore backup creates a zip backup of your entire Hermes home directory.",
    "treecore profile create coder creates an isolated profile that becomes its own command.",
    "treecore profile create work --clone copies your current config and keys to a new profile.",
    "treecore update syncs new bundled skills to ALL profiles automatically.",
    "treecore gateway install sets up Hermes as a system service (systemd/launchd).",
    "treecore memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "treecore webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: treecore tools disables unused tools, treecore skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "treecore models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as treecore -w).",
    ...
    "TREECORE_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'treecore profile create coder' creates the 'coder' command.",
    "treecore profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "treecore sessions export backup.jsonl exports all sessions for backup or analysis.",
    "treecore -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "treecore skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .treecore/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: treecore cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "treecore gateway install sets it up as a system service that starts on boot.",
    ...
    "Context files (.treecore.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Hermes loads project context from .treecore.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "treecore mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "treecore mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "treecore mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — treecore tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.treecore/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "treecore chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "treecore plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "treecore acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "TREECORE_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "treecore auth add nous or treecore auth add openai-codex sets up OAuth-based providers.",
    ...
    "treecore status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.treecore/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.treecore/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Hermes state via TREECORE_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "TREECORE_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in TREECORE_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.treecore/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'treecore profile create ls' would be rejected.",
    "treecore profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "treecore claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "TREECORE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via treecore tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.treecore/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.treecore/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.treecore/.env into the running session — pick up new API keys without restarting.',
    ...
    "TREECORE_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.treecore/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.treecore/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'treecore curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "treecore curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'treecore curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'treecore auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'TREECORE_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "TREECORE_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'treecore dashboard launches a local web UI at 127.0.0.1:9120 — zero data leaves localhost.',
    'treecore dashboard embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.treecore/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.treecore/dashboard-plugins/ — no npm build required.',
    ...
    'TREECORE_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'TREECORE_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'TREECORE_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'TREECORE_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.treecore/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.treecore/skills/ so drop-in skills appear without restarting the session.',
    ...
    'treecore -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'treecore chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'treecore chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'treecore chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "treecore chat --source tool tags programmatic chats so they don't clutter treecore sessions list.",
    'treecore dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'treecore sessions rename <ID> "new title" renames any past session; treecore sessions delete <ID> removes one.',
    'treecore import restores a full Hermes backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'treecore fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'treecore pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'treecore setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'treecore status --deep runs the full health sweep across every component; plain treecore status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'TREECORE_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'TREECORE_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.treecore/plugins/ — trust-gated by design.',
    "TREECORE_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'TREECORE_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'TREECORE_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'TREECORE_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'TREECORE_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'TREECORE_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'TREECORE_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'TREECORE_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'TREECORE_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'TREECORE_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'TREECORE_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    '--source tool chats are excluded from treecore sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.treecore/dashboard-plugins/.',
]
