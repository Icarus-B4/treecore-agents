"""MCP picker — interactive `treecore mcp picker` (also the default `treecore mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``treecore mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `treecore plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from treecore_cli.cli_output import prompt_yes_no
from treecore_cli.curses_ui import curses_single_select
from treecore_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from treecore_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `treecore mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: treecore mcp install <name>    Picker: treecore mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hermes — run `treecore update` "
...
def show_catalog() -> None:
    """`treecore mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`treecore mcp picker` (and default `treecore mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`treecore mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from treecore_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `treecore mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
