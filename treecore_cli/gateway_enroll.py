"""``treecore gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.treecore/auth.json``) — the same path ``treecore dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.treecore/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `treecore auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from treecore_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `treecore gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.treecore/.env (0600 dir) and never logs.
    to_write = {
