...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``treecore_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``treecore_state._backup_db_file``.
    """
    from treecore_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="treecore-session-recovery-",
        dir=str(work_root),
    )
