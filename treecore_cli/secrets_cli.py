"""CLI handlers for ``treecore secrets bitwarden ...``.
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Status:  [cyan]treecore secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]treecore secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]treecore secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]treecore secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `treecore secrets bitwarden setup` to pick a "
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "run `treecore secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`treecore secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]treecore[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]treecore secrets bitwarden setup[/cyan] and pick EU or "
