"""`treecore checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.treecore/checkpoints/``.  Actions:

    treecore checkpoints               # same as `status`
    treecore checkpoints status        # total size, project count, breakdown
    treecore checkpoints list          # per-project checkpoint counts + workdir
    treecore checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    treecore checkpoints clear [-f]    # nuke the entire base (asks first)
    treecore checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    treecore checkpoints
    treecore checkpoints prune --retention-days 3 --max-size-mb 200
    treecore checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: treecore checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``treecore checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `treecore checkpoints` → status
