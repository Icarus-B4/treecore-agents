...
* ``treecore update`` from a terminal,
* the dashboard's Update button (``POST /api/treecore/update`` →
  ``_spawn_treecore_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``treecore-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``treecore update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <TREECORE_HOME>/.treecore-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``treecore update`` as a child stage. Without a handoff the child
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``treecore-setup`` binary under ``~/.treecore`` is only refreshed by a full
  installer run (``copy_self_to_treecore_home`` deliberately no-ops during
...
MARKER_NAME = ".treecore-update-in-progress"

# Set by an orchestrating updater (the Tauri `treecore-setup --update` flow) to
# its own pid before spawning `treecore update` as a child stage. The parent
...
HANDOFF_PID_ENV = "TREECORE_UPDATE_HANDOFF_PID"
...
def update_marker_path() -> Path:
    ...
    the Rust updater resolves ``$TREECORE_HOME`` or the platform default, and the
    ...
    """
    from treecore_constants import get_process_treecore_home

    return get_process_treecore_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``treecore update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``treecore-setup`` binaries
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `treecore update` as a stage): we run under ITS claim
