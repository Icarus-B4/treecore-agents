...
SCHEMA_KEY = "treecore.metrics.schema_version"
SCHEMA_VERSION = "treecore.metrics.event.v2"
MODEL_CALL_SCOPE = "treecore.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "treecore.task_run"
TOOL_CALL_SCOPE = "treecore.tool_call"
CLIENT_ACTIVE_MARK = "treecore.client.active"
TOOL_APPROVAL_MARK = "treecore.tool_approval"
SKILL_LIFECYCLE_MARK = "treecore.skill.lifecycle"
SKILL_LOAD_MARK = "treecore.skill.load"
SUBSCRIBER_NAME = "treecore.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "treecore.client.active"
LEGACY_MODEL_CALL_METRIC = "treecore.model_call.count"
MODEL_ROUTE_METRIC = "treecore.model_route.count"
TASK_STARTED_METRIC = "treecore.task_run.started"
TASK_FINISHED_METRIC = "treecore.task_run.finished"
TOOL_CALL_METRIC = "treecore.tool_call.count"
TOOL_APPROVAL_METRIC = "treecore.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "treecore.skill.lifecycle.count"
SKILL_LOAD_METRIC = "treecore.skill.load.count"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(treecore_version or "").strip()
    ...
    return {
        ...
        "treecore_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("treecore_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("treecore.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from treecore_cli.platforms import get_all_platforms
...
def tool_category(kwargs: dict[str, Any]) -> str:
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "treecore-yuanbao", "slack", "sms")
    ):
