...
Config files are stored in ~/.treecore/ for easy access:
- ~/.treecore/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.treecore/.env         - API keys and secrets

This module provides:
- treecore config          - Show current configuration
- treecore config edit     - Open config in editor
- treecore config get      - Print a resolved configuration value
- treecore config set      - Set a specific value
- treecore config unset    - Remove a user configuration value
- treecore config wizard   - Re-run setup wizard
"""
...
from treecore_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``treecore config set`` (which rewrites
    ...
    ``config.yaml`` in place: treecore never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.treecore/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``treecore logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  treecore config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   on every plugin install / ``treecore update``.
...
# * ``TREECORE_HOME`` / ``TREECORE_PROFILE`` / ``TREECORE_CONFIG`` /
#   ``TREECORE_ENV`` — Hermes runtime location flags. Writing these into
...
#
# IMPORTANT: ``TREECORE_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (TREECORE_LANGFUSE_PUBLIC_KEY,
# TREECORE_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # NOT a TREECORE_* blanket: integration credentials (TREECORE_GEMINI_*,
    # TREECORE_LANGFUSE_*, TREECORE_SPOTIFY_*, ...) ARE allowed.
    "TREECORE_HOME", "TREECORE_PROFILE", "TREECORE_CONFIG", "TREECORE_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(TREECORE_HOME, TREECORE_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.treecore/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # TREECORE_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `treecore plugins enable
    # observability/langfuse` or `treecore tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "TREECORE_LANGFUSE_ENV",
    "TREECORE_LANGFUSE_RELEASE",
    "TREECORE_LANGFUSE_SAMPLE_RATE",
    "TREECORE_LANGFUSE_MAX_CHARS",
    "TREECORE_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "TREECORE_ACP_AUTH_METHOD",
    "TREECORE_ACP_AUTO_APPROVE",
    "TREECORE_COPILOT_ACP_COMMAND",
    "TREECORE_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from treecore_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set TREECORE_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("TREECORE_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_treecore_home() / ".managed"
...
def is_managed() -> bool:
    ...
    Two signals: the TREECORE_MANAGED env var (set by the systemd service),
    or a .managed marker file in TREECORE_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``treecore_cli/`` — i.e. the git checkout for source
    installs, ``/opt/treecore`` inside the published image. It is a property of
    the running interpreter, NOT of ``$TREECORE_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$TREECORE_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. TREECORE_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.treecore``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$TREECORE_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.treecore:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$TREECORE_HOME``, a
    ...
    ``docker`` and ``treecore update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/treecore-agent`` image bakes a ``docker``
        stamp into ``/opt/treecore`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $TREECORE_HOME.
    try:
    ...
    #    host install whose shared $TREECORE_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``treecore update``.
    try:
        method = (
            (get_treecore_home() / ".install_method")
    ...
    # detect Nix installs that don't set TREECORE_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``treecore_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$TREECORE_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/treecore`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nousresearch/treecore-agent:latest"
    return "treecore update"
...
# Long-form text for ``treecore update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# treecore_cli/main.py; lives here so the wording stays consistent and we
...
✗ ``treecore update`` doesn't apply inside the Docker container.

Hermes Agent runs as a published image (nousresearch/treecore-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/treecore-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate treecore-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/treecore-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/treecore-agent/tags
  • Your config and session history live under ``$TREECORE_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``treecore update`` inside Docker.
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    raw = os.getenv("TREECORE_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hermes installation is managed by NixOS "
            f"(TREECORE_MANAGED={env_hint}).\n"
            "Edit services.treecore-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from TREECORE_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, treecore_bin
    or None if container mode is not active, we're already inside the
    container, or TREECORE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("TREECORE_DEV") == "1":
        return None
    ...
    container_mode_file = get_treecore_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "treecore-agent")
    exec_user = info.get("exec_user", "treecore")
    treecore_bin = info.get("treecore_bin", "/data/current-package/bin/treecore")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "treecore_bin": treecore_bin,
    }
...
# Re-export from treecore_constants — canonical definition lives there.
from treecore_constants import get_treecore_home, get_process_treecore_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_treecore_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_treecore_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_treecore_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the TREECORE_UID / TREECORE_GID env vars set by Docker deployments.
    ...
    ownership. The entrypoint chowns the top-level TREECORE_HOME once, but
    subdirectories created at runtime by ``ensure_treecore_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("TREECORE_UID", "").strip()
    gid_str = os.environ.get("TREECORE_GID", "").strip()
    try:
...
def _chown_to_treecore_uid(path) -> None:
    """Chown ``path`` to ``TREECORE_UID:TREECORE_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_treecore_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_treecore_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the treecore group can
    share state with the gateway service.

    The mode can be overridden via the TREECORE_HOME_MODE environment variable
    (e.g. TREECORE_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse TREECORE_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``TREECORE_UID``/``TREECORE_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("TREECORE_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("TREECORE_CONTAINER") or os.environ.get("TREECORE_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set TREECORE_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into TREECORE_HOME, upgrading legacy empty templates.
...
# ensure_treecore_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_TREECORE_HOME_ENSURED: set = set()


def ensure_treecore_home():
    """Ensure ~/.treecore directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``TREECORE_HOME`` we only re-run
    ...
    ``get_treecore_home()`` and therefore re-run for the new path.
    """
    home = get_treecore_home()
    key = str(home)

    if key in _TREECORE_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``treecore profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old TREECORE_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_treecore_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _TREECORE_HOME_ENSURED.add(key)


def _ensure_treecore_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"TREECORE_HOME {home} does not exist. "
...
from treecore_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `treecore config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.treecore.config`` entries, then checks
    ...
    try:
        ...
        # should never break `treecore update`.  Skill-config prompting is a
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``treecore_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (treecore_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (treecore_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (treecore_cli/setup.py)
    "known_plugin_toolsets", # written/read by treecore_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'treecore setup' to create a valid config")]
    ...
    # scalars are bridged into os.environ (gateway/run.py, treecore send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'treecore doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_treecore_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "treecore update / treecore -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from treecore_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ treecore config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in treecore_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from treecore_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from treecore_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: treecore config set <key> <value>")
    ...
    # `treecore update`, but only the version bump is persisted.
    ...
    # metadata.treecore.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.treecore/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_treecore_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``treecore config set``
    ...
    backends) — while display paths (``treecore status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _treecore_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.treecore/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_treecore_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.treecore/config.yaml.
    ...
    (which change ``TREECORE_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/treecore/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_treecore_dotenv()
...
#   openai-codex (OAuth — treecore auth) — OpenAI Codex
#   nous         (OAuth — treecore auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.treecore/config.yaml.\n
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.treecore/.env.
    ...
    interactive menu render (`treecore tools`, `treecore setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `treecore tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.treecore/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.treecore/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``TREECORE_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"TREECORE_CUSTOM_{slug}_API_KEY" if slug else "TREECORE_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.treecore/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.treecore/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.treecore/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.treecore/.env`` over ``os.environ``.
...
def show_config():
    ...
    from treecore_cli.auth import get_anthropic_key
    ...
    # Warn on stale TREECORE_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("TREECORE_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale TREECORE_MAX_ITERATIONS={_env_ghost} "
                f"(run 'treecore doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  treecore config edit     # Edit config file", Colors.DIM))
    print(color("  treecore config set <key> <value>", Colors.DIM))
    print(color("  treecore setup           # Run setup wizard", Colors.DIM))
    print()
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `treecore cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``treecore config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``treecore config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``treecore config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (treecore config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``treecore config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``treecore config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    treecore config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    treecore config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `treecore config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_treecore_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `treecore config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `treecore config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: treecore config get <key> [--json]")
            print()
            print("Examples:")
            print("  treecore config get model")
            print("  treecore config get terminal.backend")
            print("  treecore config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: treecore config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  treecore config set model anthropic/claude-sonnet-4")
            print("  treecore config set terminal.backend docker")
            print("  treecore config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: treecore config unset <key>")
            print()
            print("Examples:")
            print("  treecore config unset model")
            print("  treecore config unset terminal.backend")
            print("  treecore config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'treecore config migrate' to add them")
        ...
        print("  treecore config           Show current configuration")
        print("  treecore config edit      Open config in editor")
        print("  treecore config get <key>          Print a resolved config value")
        print("  treecore config set <key> <value>   Set a config value")
        print("  treecore config unset <key>        Remove a config value")
        print("  treecore config check     Check for missing/outdated config")
        print("  treecore config migrate   Update config with new options")
        print("  treecore config path      Show config file path")
        print("  treecore config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``treecore config`` UI so users
