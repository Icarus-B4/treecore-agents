...
MIGRATION_MARKER = (
    "# managed by treecore-agent — `treecore codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end treecore-agent managed section"
)
...
def _translate_one_server(
    ...
    if not isinstance(treecore_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(treecore_cfg.get("command"))
    has_url = bool(treecore_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (TREECORE_HOME, PYTHONPATH) could
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="treecore-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a TREECORE_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed treecore-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.treecore`` symlink that happens to live under ``/private/var/folders``
...
def _build_treecore_tools_mcp_entry() -> dict:
    ...
    so a treecore installed under /opt/, /usr/local/, or a venv all work.
    TREECORE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # TREECORE_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_treecore_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever TREECORE_HOME its launcher sets
    ...
    # — that would override the launcher's TREECORE_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("TREECORE_HOME", tmp_path) would
    ...
    treecore_home = os.environ.get("TREECORE_HOME") or ""
    if treecore_home and _looks_like_test_tempdir(treecore_home):
        treecore_home = ""
    if treecore_home:
        env["TREECORE_HOME"] = treecore_home
    # PYTHONPATH passes through so a worktree-launched treecore finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["TREECORE_QUIET"] = "1"
    env["TREECORE_REDACT_SECRETS"] = env.get("TREECORE_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.treecore_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    Args:
        ...
        expose_treecore_tools: when True (default), register Hermes' own
    ...
    treecore_servers = (treecore_config or {}).get("mcp_servers") or {}
    if not isinstance(treecore_servers, dict):
    ...
    for name, cfg in treecore_servers.items():
    ...
    # The server itself is agent/transports/treecore_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_treecore_tools:
        translated["treecore-tools"] = _build_treecore_tools_mcp_entry()
        if "treecore-tools" not in report.migrated:
            report.migrated.append("treecore-tools")
