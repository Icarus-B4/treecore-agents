...
Profile-aware location: ``$TREECORE_HOME/logs/dashboard-auth.log``.
...
from ``treecore_constants`` or other treecore_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$TREECORE_HOME/logs/dashboard-auth.log``.

    Uses ``treecore_constants.get_treecore_home()`` (a leaf module — no import
    ...
    return get_treecore_home() / "logs" / "dashboard-auth.log"
