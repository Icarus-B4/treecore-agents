...
from treecore_cli.dashboard_auth.base import (
    InvalidCodeError,
    InvalidCredentialsError,
    ProviderError,
)
from treecore_cli.dashboard_auth.cookies import (
    clear_pkce_cookie,
...
from treecore_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``TREECORE_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /treecore`` (Mission Control deploys) — we
    ...
    from treecore_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``treecore_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from treecore_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("treecore_session_pkce", "")
...
async def auth_native_authorize(
    ...
    from treecore_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from treecore_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from treecore_cli.dashboard_auth import list_session_providers
    from treecore_cli.dashboard_auth.base import RefreshExpiredError
