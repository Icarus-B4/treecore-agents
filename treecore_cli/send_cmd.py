"""CLI subcommand: ``treecore send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"treecore send: {file_path} is not a text file. --file reads the "
                ...
                f'  treecore send --to telegram "MEDIA:{file_path}"\n'
                f'  treecore send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  treecore send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"treecore send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"treecore send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"treecore send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"treecore send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `treecore send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"treecore send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `treecore gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.treecore/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_treecore_env() -> None:
    """Populate ``os.environ`` from ``~/.treecore/.env`` AND bridge top-level
    ...
    startup that ``treecore send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.treecore/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.treecore/config.yaml`` into
       ...
       via ``treecore config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``treecore send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.treecore/.env and ~/.treecore/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "treecore send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  treecore send --to telegram \"hello\"\n"
            "  treecore send --to discord:#ops --file report.md\n"
            "  treecore send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "treecore send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `treecore send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.treecore/.env + ~/.treecore/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  treecore send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | treecore send --to telegram:-1001234567890\n"
            "  treecore send --to discord:#ops --file /tmp/report.md\n"
            "  treecore send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  treecore send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  treecore send --list                  # all platforms\n"
            "  treecore send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `treecore send --list telegram`.",
    )
