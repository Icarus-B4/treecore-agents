"""
Status command for treecore CLI.
...
from treecore_cli.auth import AuthError, resolve_provider
from treecore_cli.colors import Colors, color
from treecore_cli.config import get_env_path, get_env_value, get_treecore_home, load_config
from treecore_cli.models import provider_label
...
from treecore_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``treecore config``'s
...
def _estop_status_line():
    """One-line pause banner for `treecore status`, or None when not paused.

    Cheap: a single stat on $TREECORE_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `treecore resume` to lift)"
...
def show_status(args):
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `treecore status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: treecore portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: treecore model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: treecore auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: treecore auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: treecore model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from treecore_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'treecore-agent[vercel]')"
    ...
    try:
        from treecore_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   treecore gateway")
    ...
    jobs_file = get_treecore_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_treecore_home() / "sessions" / "sessions.json"
    ...
    try:
        from treecore_cli.active_sessions import (
    ...
    print(color("  Run 'treecore doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'treecore setup' to configure", Colors.DIM))
    print()
