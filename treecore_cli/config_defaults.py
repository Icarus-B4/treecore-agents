...
verbatim from treecore_cli/config.py. Must not import from treecore_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # identity slot (SOUL.md). Empty by default. The TREECORE_ENVIRONMENT_HINT
        ...
        # ``TREECORE_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             TREECORE_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force TREECORE_HOME/home when it exists (old strict
        ...
        #  "/home/user/.treecore/cache/documents:/output"]
        ...
        # bundled Hermes image, which drops to the `treecore` user via
    ...
    # Opt in via ``treecore chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.treecore/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: treecore sweeps the checkpoint base at startup
        ...
        # ``treecore checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .treecore.md, or .cursorrules before Hermes applies
    ...
    # Single-query (``treecore -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # to compact its own thread (default); treecore =
    ...
    "auxiliary": {
        ...
        # Invoked by ``treecore kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``treecore kanban decompose`` and the kanban
        ...
        # ``treecore profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `treecore model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `treecore` (and `treecore chat`) launches by default:
        ...
        # REPL and `--tui` (or TREECORE_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `treecore --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `treecore -c` muscle memory.  Default off so existing
        # users aren't surprised.  TREECORE_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `treecore --tui` drops a one-time hint
        ...
        # model — see treecore_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to TREECORE_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/treecore`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `treecore pets`. Disabled until
        ...
        "pet": {
            ...
            # get_treecore_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``TREECORE_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``TREECORE_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``TREECORE_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``TREECORE_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``TREECORE_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/treecore``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.treecore/cache/piper-voices/
    ...
    "wake_word": {
        ...
        "phrase": "hey treecore",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_treecore" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.treecore/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.treecore/cache/delegation/
    ...
    "moa": {
        ...
        # <treecore_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.treecore/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${TREECORE_SKILL_DIR} and ${TREECORE_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `treecore curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `treecore curator run --consolidate` overrides
        ...
        # normally restored on every `treecore update`, so pruning them only
        ...
        # skipped), snapshot ~/.treecore/skills/ into
        # ~/.treecore/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `treecore curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for treecore-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (TREECORE_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.treecore/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or TREECORE_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .treecore config) always require human
        ...
        # longer trigger the startup banner. Add via `treecore doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``treecore_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # $TREECORE_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via TREECORE_CRON_MAX_PARALLEL env var.
        ...
        # TREECORE_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `treecore -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `treecore` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `treecore kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     treecore-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.treecore/logs/.
    ...
    # update model picker lists without shipping a treecore-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://treecore-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `treecore model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal TREECORE_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.treecore/sessions/sessions.json entirely.
        ...
        # the TREECORE_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.treecore.gateway` that defenses 1-2 don't
        ...
        # ``TREECORE_GATEWAY_MAX_STARTS`` / ``TREECORE_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.treecore/.env,
        ...
        # user. Bridged to TREECORE_MEDIA_DELIVERY_STRICT.
        "strict": False,
        ...
        # cache (~/.treecore/cache/{documents,images,audio,video,screenshots})
        ...
        # to TREECORE_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # TREECORE_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to TREECORE_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.treecore/state.db.
    ...
    "sessions": {
        ...
        # the default of ``treecore sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.treecore/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `treecore sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `treecore update` prints a one-line notice with
        ...
        # ~/.treecore/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to TREECORE_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # TREECORE_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    # ``treecore doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `treecore doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``treecore update`` behaviour.
    "updates": {
        ...
        #     DBs) into <TREECORE_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``treecore backup``-style zip
        #     of TREECORE_HOME into <TREECORE_HOME>/backups/, restorable with
        #     ``treecore import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `treecore update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<TREECORE_HOME>/lsp/bin/`` on first use.
    ...
    # `treecore tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.treecore/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.treecore/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.treecore/cache/. If a later startup cannot reach
            ...
            # ~/.treecore/bin/ on first use.  When False you must install
            ...
            # `treecore secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.treecore/.env (or the shell)
    ...
    #
    # Configure with `treecore egress setup`.  Disabled by default — the rest of
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.treecore/bin/ on
    ...
    # `treecore desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the TREECORE_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'treecore model' for endpoint-specific config)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @treecore:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. TREECORE_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #treecore)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: treecore-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'treecore-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # TREECORE_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to TREECORE_TOOL_PROGRESS_MODE for backward
    ...
    "TREECORE_PREFILL_MESSAGES_FILE": {
    ...
    "TREECORE_EPHEMERAL_SYSTEM_PROMPT": {
