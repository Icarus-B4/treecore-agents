...
2. Python deps declared by user-installed plugins under ``~/.treecore/plugins``
...
def _discover_plugins(treecore_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.treecore/plugins``.
    ...
    """
    plugins_dir = treecore_home / "plugins"
...
def _discover_components(
    *,
    ...
    home = treecore_home or Path(get_treecore_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `treecore security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, treecore_home=home
    )
