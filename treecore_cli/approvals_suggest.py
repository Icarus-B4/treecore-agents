"""``treecore approvals suggest`` — mine approval history into allowlist proposals.
...
is the session DB (``~/.treecore/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, treecore self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/treecore home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_treecore_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_treecore_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  treecore approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.treecore/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``treecore approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``treecore approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from treecore_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: treecore approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `treecore approvals <subcommand> -h` for details."
    )
    return 1
