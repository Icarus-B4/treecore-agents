"""``treecore debug`` debug tools for Hermes Agent.

Currently supports:
    treecore debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.treecore/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[treecore debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.treecore/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``treecore debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``treecore debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `treecore debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "treecore-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``treecore debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.treecore/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``treecore debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "treecore-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from treecore_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_treecore_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `treecore debug share` runs
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``treecore debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``treecore dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``treecore dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "treecore-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``treecore debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "treecore debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `treecore debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  treecore debug delete <url>")
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``treecore debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from treecore_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "treecore debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `treecore debug share --local` to print the report instead, "
            "or `treecore debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: treecore debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'treecore debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``treecore debug`` call.
    ...
    # best-effort — any failure is swallowed so ``treecore debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: treecore debug <command>")
        print()
        print("Commands:")
