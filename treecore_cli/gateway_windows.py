...
def _preserve_treecore_home_path(path: str | Path) -> str:
    """Render Hermes-owned paths under the configured TREECORE_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\treecore`` as a symlink/junction to
    ...
    path lives under TREECORE_HOME.
    """
    candidate = Path(path)
    try:
        from treecore_cli.config import get_treecore_home

        home = Path(get_treecore_home())
...
def _current_profile_cli_args() -> list[str]:
    ...
    from treecore_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "treecore_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("TREECORE_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("TREECORE_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("TREECORE_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["TREECORE_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["TREECORE_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["TREECORE_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("TREECORE_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("TREECORE_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("TREECORE_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during treecore_cli boot.
    from treecore_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\treecore\\gateway-service\\<task_name>.cmd``
    (or ``<TREECORE_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Hermes installs stay self-contained).
    """
    _assert_windows()
    from treecore_cli.config import get_treecore_home

    script_dir = Path(get_treecore_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``TREECORE_HOME`` whenever it
    ...
    source checkout only if ``TREECORE_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports TREECORE_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m treecore_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``treecore gateway run``.
    ...
    lines.append(f'set "TREECORE_HOME={treecore_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "TREECORE_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports treecore_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_treecore_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_treecore_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_treecore_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [python_exe_path, "-m", "treecore_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_treecore_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_treecore_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('TREECORE_HOME')}) = {_quote_vbs_string(treecore_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('TREECORE_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_treecore_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_treecore_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    treecore_home = str(Path(get_treecore_home()))
    profile_arg = _profile_arg(treecore_home)

    content = _build_gateway_cmd_script(python_path, working_dir, treecore_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, treecore_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_treecore_home_path(get_python_path())
    )
    project_root = _preserve_treecore_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    treecore_home = str(Path(get_treecore_home()))
    profile_arg = _profile_arg(treecore_home)

    argv = [python_exe, "-m", "treecore_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "TREECORE_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_treecore_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_treecore_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (TREECORE_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m treecore_cli.main
    ...
    from treecore_cli.config import get_treecore_home
    from treecore_cli.gateway import PROJECT_ROOT
    ...
    try:
        treecore_home = str(Path(get_treecore_home()).resolve())
    except Exception:
        treecore_home = ""
    ...
    if treecore_home:
        env_overlay["TREECORE_HOME"] = treecore_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m treecore_cli.main gateway run`` directly — NOT
    ...
    log_dir = Path(get_treecore_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("TREECORE_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("TREECORE_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from treecore_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `treecore -p <profile> gateway install` must be safe.
    ...
    from treecore_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"treecore {profile_arg} gateway start" if profile_arg else "treecore gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: treecore gateway start")
        return
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                ...
                if start_now:
                    ...
                    print("  Approve the Windows UAC prompt, then run: treecore gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: treecore gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from treecore_cli.config import get_treecore_home
        print(f"    type {Path(get_treecore_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_treecore_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from treecore_cli.config import get_treecore_home

    treecore_home = Path(get_treecore_home())
    print()
    print("Next steps:")
    print("  treecore gateway status                      # Check status")
    print(f"  type {treecore_home}\\logs\\gateway.log       # View logs")
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: treecore gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: treecore gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `treecore gateway status`."
        )
