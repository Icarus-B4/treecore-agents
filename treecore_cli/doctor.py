"""
Doctor command for treecore CLI.
...
from treecore_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
TREECORE_HOME = get_treecore_home()
_DHH = display_treecore_home()  # user-facing display path (e.g. ~/.treecore or ~/.treecore/profiles/coder)

# Load environment variables from ~/.treecore/.env so API key checks work
_env_path = get_env_path()
load_treecore_dotenv(treecore_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from treecore_cli.colors import Colors, color
from treecore_cli.models import _TREECORE_USER_AGENT
from treecore_cli.vercel_auth import describe_vercel_auth
from treecore_constants import OPENROUTER_MODELS_URL
...
from treecore_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.treecore/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("TREECORE_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from treecore_cli.auth import get_minimax_oauth_auth_status
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    # TREECORE_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor removed its only consumer (the v3→4 migration) — it is silently
    # ignored. TREECORE_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("TREECORE_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("TREECORE_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches treecore_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``treecore --version`` reporting a stale version while
    ...
    """
    try:
        from treecore_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != treecore_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'treecore update', or set "
            "treecore_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-treecore and dashboard static services are up
    ...
    for static in ("main-treecore", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `treecore profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `treecore doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from treecore_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the TREECORE_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("TREECORE_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via TREECORE_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `treecore`.
    os.environ.setdefault("TREECORE_INTERACTIVE", "1")

    # Handle `treecore doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.treecore/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`treecore doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from treecore_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
    ...
    # Detect drift between pyproject.toml and treecore_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.treecore/.env (primary location for user config)
    env_path = TREECORE_HOME / '.env'
    if env_path.exists():
        ...
        # matches treecore_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'treecore setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'treecore setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'treecore setup' to create one")
                issues.append("Run 'treecore setup' to create .env")
    
    # Check ~/.treecore/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = TREECORE_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from treecore_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from treecore_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'treecore config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from treecore_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.treecore/.env or run 'treecore setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'treecore setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'treecore config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from treecore_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from treecore_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'treecore setup' to migrate config")
                else:
                    issues.append("Run 'treecore doctor --fix' or 'treecore setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'treecore doctor --fix'")
        except Exception:
            pass

        # Detect stale TREECORE_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives TREECORE_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from treecore_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("TREECORE_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"TREECORE_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `treecore setup` run)",
                )
                if should_fix:
                    if remove_env_value("TREECORE_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale TREECORE_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove TREECORE_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the TREECORE_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale TREECORE_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'treecore doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from treecore_cli.config import load_env as _load_env_depr
            from treecore_cli.config import read_user_config_raw as _read_raw_depr
    ...
    if treecore_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        treecore_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = treecore_home / subdir_name
    ...
    soul_path = treecore_home / "SOUL.md"
    if soul_path.exists():
    ...
    memories_dir = treecore_home / "memories"
    ...
    state_db_path = treecore_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from treecore_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'treecore doctor --fix' "
                        "(or 'treecore sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from treecore_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'treecore doctor --fix' "
                        "(or 'treecore sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = treecore_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'treecore doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "treecore"
        ...
        _cmd_link = _cmd_link_dir / "treecore"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(treecore not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/treecore → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/treecore points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/treecore → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/treecore — run 'treecore doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/treecore exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/treecore not found",
                    "(treecore command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/treecore → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/treecore symlink — run 'treecore doctor --fix'")
    ...
    try:
        from treecore_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'treecore-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'treecore-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `treecore acp --setup-browser` installs agent-browser into the
        # Hermes-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_treecore_agents_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(TREECORE_HOME / "node" / "bin")
            or _which_in(TREECORE_HOME / "node")
        )
        _legacy_ab = _which_in(TREECORE_HOME / "node_modules" / ".bin")
        if agent_browser_path.exists():
            ...
            # `treecore update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `treecore doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable TREECORE_HOME mirror
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'treecore config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _TREECORE_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``treecore doctor``.
    ...
    # never the legitimate source for `treecore doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'treecore setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = TREECORE_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: treecore skills list)")
    ...
    try:
        from treecore_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = TREECORE_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: treecore memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: treecore memory setup",
                    "No Honcho API key — run 'treecore memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run treecore memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: treecore memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: treecore memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from treecore_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "treecore -p" in content:
                            _m = _re.search(r"treecore -p (\S+)", content)
    ...
    # `treecore doctor --live` (real network calls; bounded + read-only).
    try:
        from treecore_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'treecore doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
