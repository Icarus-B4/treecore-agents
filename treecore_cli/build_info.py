...
``treecore_cli/dump.py`` and ``treecore_cli/banner.py``).  That doesn't work inside
...
To make ``treecore dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$TREECORE_GIT_SHA``
arg into ``<project_root>/.treecore_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``TREECORE_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".treecore_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.treecore_build_sha`` if present.  The file is
    written by the Dockerfile's ``TREECORE_GIT_SHA`` build-arg and contains
