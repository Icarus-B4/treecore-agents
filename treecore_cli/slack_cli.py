"""``treecore slack ...`` CLI subcommands.

Today only ``treecore slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ treecore slack manifest > slack-manifest.json
    # or:
    $ treecore slack manifest --write
...
def _build_full_manifest(
    ...
    """
    from treecore_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``treecore_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$TREECORE_HOME/slack-manifest.json``)
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "treecore slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"treecore slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "treecore slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "treecore slack manifest: long description must be at most "
