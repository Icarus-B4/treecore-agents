...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_treecore_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:treecore_pkce",
    ...
    if provider == "nous":
        ...
        # <treecore-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `treecore --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``treecore auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``treecore auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`treecore auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `treecore auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `treecore auth` is called bare."""
