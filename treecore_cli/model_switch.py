...
- ``treecore_cli.providers``        -- canonical provider identity + overlays
- ``treecore_cli.model_normalize``  -- per-provider name formatting
...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``treecore model`` wizard — not a
    ...
    while ``treecore model`` still live-probed the full ``/v1/models`` list.
...
# The previous substring check (`"treecore" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``treecore-brain:qwen3-14b-ctx16k`` that just
# happen to carry "treecore" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, treecore-4-405b, openrouter/treecore3:70b
# Negative examples it must NOT match:
#   treecore-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_TREECORE_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])treecore[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_treecore_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_TREECORE_NON_AGENTIC_RE.search(model_name))


def _check_treecore_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_treecore_non_agentic(model_name):
        return _TREECORE_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``treecore config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from treecore_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``treecore_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'treecore model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'treecore doctor' — config issues detected:"
        ...
        from treecore_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from treecore_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from treecore_cli.config import load_config
            from treecore_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from treecore_cli.providers import resolve_user_provider as _ruser
    ...
    if target_provider in {"nous", "nous-portal", "nousresearch"}:
        from treecore_cli.providers import nous_api_mode
    ...
    # treecore_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from treecore_cli.models import normalize_opencode_base_url
    ...
    treecore_warn = _check_treecore_model_warning(new_model)
    if treecore_warn:
        warnings.append(treecore_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from treecore_cli.config import normalize_extra_headers
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from treecore_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against treecore_id / mdev_id (section 1), pid / treecore_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from treecore_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by treecore provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://treecore-agent.nousresearch.com/docs/api/model-catalog.json so
    ...
    if "ollama-cloud" not in curated:
        from treecore_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from treecore_cli.models import fetch_lmstudio_models
    ...
    from treecore_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from treecore_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for treecore_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # same treecore_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if treecore_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(treecore_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (TREECORE_OVERLAYS) with proper auth store checking.
        ...
        from treecore_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(treecore_id):
            continue
        ...
        if not has_creds:
            try:
                from treecore_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(treecore_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        treecore_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `treecore model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(treecore_id)
        if not model_ids:
            model_ids = curated.get(treecore_id, [])
            if treecore_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(treecore_id, model_ids)
        ...
        results.append({
            ...
            "is_current": (
                ...
                or treecore_id == current_provider
    ...
    from treecore_cli.providers import TREECORE_OVERLAYS
    from treecore_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Hermes provider ID.
    # TREECORE_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hermes IDs ("copilot").
    _mdev_to_treecore = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in TREECORE_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Hermes slug — e.g. "github-copilot" → "copilot"
        treecore_slug = _mdev_to_treecore.get(pid, pid)
        if treecore_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or treecore_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(treecore_slug)
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or treecore_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(treecore_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(treecore_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", treecore_slug, exc)
        ...
        # configured.
        if not has_creds and treecore_slug == "anthropic":
            try:
                ...
                if (treecore_creds and treecore_creds.get("accessToken")) or \
        ...
        if treecore_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(treecore_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(treecore_slug)
                model_ids = _ids if _ids else (curated.get(treecore_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(treecore_slug, []) or curated.get(pid, [])
        elif treecore_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `treecore model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from treecore_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(treecore_slug, []) or curated.get(pid, [])
                if treecore_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(treecore_slug, model_ids)
        total = len(model_ids)
        if treecore_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": treecore_slug,
            "name": get_label(treecore_slug),
            "is_current": treecore_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "treecore",
        ...
        seen_slugs.add(treecore_slug.lower())
        _record_builtin_endpoint(treecore_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or TREECORE_OVERLAYS (keeps /model in sync
    # with `treecore model`).
    try:
        from treecore_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``treecore model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``treecore model``. Pin with
            ...
            if should_probe:
                try:
                    from treecore_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, treecore_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``treecore model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from treecore_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`treecore_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from treecore_cli.models import fetch_openrouter_models
