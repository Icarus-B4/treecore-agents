...
Used by ``treecore kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``treecore_cli/goals.py`` — same aux
...
TREECORE_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("TREECORE_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)
...
def _profile_author() -> str:
    """Mirror of ``treecore_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("TREECORE_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=TREECORE_KANBAN_SPECIFY_MAX_TOKENS,
