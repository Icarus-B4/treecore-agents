"""treecore webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    treecore webhook subscribe <name> [options]
    treecore webhook list
    treecore webhook remove <name>
    treecore webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.treecore/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _treecore_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_treecore_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     treecore gateway setup
  ...
  Then start the gateway: treecore gateway run
"""
...
def webhook_command(args):
    """Entry point for 'treecore webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: treecore webhook {subscribe|list|remove|test}")
        print("Run 'treecore webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (treecore gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: treecore webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from treecore webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (treecore gateway run)")
