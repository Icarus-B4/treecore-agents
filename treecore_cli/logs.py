"""``treecore logs`` — view and filter Hermes log files.
...
Usage examples::
    ...
    treecore logs                    # last 50 lines of agent.log
    treecore logs -f                 # follow agent.log in real time
    treecore logs errors             # last 50 lines of errors.log
    treecore logs gateway -n 100    # last 100 lines of gateway.log
    treecore logs gui -f            # follow gui.log (dashboard/pty/ws)
    treecore logs desktop -f        # follow desktop.log (Electron app boot/backend)
    treecore logs --level WARNING    # only WARNING+ lines
    treecore logs --session abc123   # filter by session ID substring
    treecore logs --component tools  # only tool-related lines
    treecore logs --since 1h         # lines from the last hour
    treecore logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_treecore_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Hermes runs — try 'treecore chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_treecore_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_treecore_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_treecore_home()}/logs/")
        return

    print(f"Log files in {display_treecore_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'treecore chat' to generate logs)")
