...
Why this exists
---------------
``treecore_cli/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``treecore_cli/web_routers/`` need those helpers, but
...
thin proxy that resolves ``treecore_cli.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``treecore_cli.web_server`` module (imported on demand)."""
    mod = sys.modules.get("treecore_cli.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import treecore_cli.web_server as mod  # type: ignore[no-redef]
    return mod
