"""Implementation of the ``treecore bundles`` CLI subcommand.

Mirrors the structure of ``treecore_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `treecore bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  treecore bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``treecore bundles`` argparse tree.

    Called from ``treecore_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `treecore bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``treecore bundles <subcommand>`` to the right handler."""
