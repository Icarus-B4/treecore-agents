"""
Top-level argparse construction for the treecore CLI.
...
# argparse runs (it sets ``TREECORE_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``treecore_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    treecore                        Start interactive chat
    treecore chat -q "Hello"        Single query mode
    treecore --tui                  Launch the modern TUI (or set display.interface: tui)
    treecore --cli                  Force the classic REPL (overrides display.interface: tui)
    treecore -c                     Resume the most recent session
    treecore -c "my project"        Resume a session by name (latest in lineage)
    treecore --resume <session_id>  Resume a specific session by ID
    treecore setup                  Run setup wizard
    treecore logout                 Clear stored authentication
    treecore auth add <provider>    Add a pooled credential
    treecore auth list              List pooled credentials
    treecore auth remove <p> <t>    Remove pooled credential by index, id, or label
    treecore auth reset <provider>  Clear exhaustion status for a provider
    treecore model                  Select default model
    treecore fallback [list]        Show fallback provider chain
    treecore fallback add           Add a fallback provider (same picker as `treecore model`)
    treecore fallback remove        Remove a fallback provider from the chain
    treecore config                 View configuration
    treecore config edit            Edit config in $EDITOR
    treecore config set model gpt-4 Set a config value
    treecore gateway                Run messaging gateway
    treecore -s treecore-agent-dev,github-auth
    treecore -w                     Start in isolated git worktree
    treecore gateway install        Install gateway background service
    treecore sessions list          List past sessions
    treecore sessions browse        Interactive session picker
    treecore sessions rename ID T   Rename/title a session
    treecore logs                   View agent.log (last 50 lines)
    treecore logs -f                Follow agent.log in real time
    treecore logs errors            View errors.log
    treecore logs --since 1h        Lines from the last hour
...
def build_top_level_parser():
    ...
    # Mirrors `treecore chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via TREECORE_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `treecore setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to TREECORE_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.treecore/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    # top-level parser: when the user writes `treecore -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/treecore_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also TREECORE_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.treecore/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
