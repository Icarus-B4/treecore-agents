...
def _root() -> Path:
    ...
    # helper's token, including custom TREECORE_HOME layouts.
    return get_default_treecore_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, treecore_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(treecore_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the treecore exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs treecore_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "treecore_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "treecore_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, treecore_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, treecore_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves treecore_cli's own location and prepends its parent, because the
    ...
    # the treecore_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the treecore_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["treecorePath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Hermes path must be absolute")
    treecore_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(treecore_path)
    ...
    bootstrap = (
        ...
        "runpy.run_module('treecore_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_treecore(treecore_path: str) -> dict[str, Any]:
    path = os.path.abspath(treecore_path)
    if not os.path.isabs(treecore_path) or not os.path.isfile(path):
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "treecoreHome": str(get_default_treecore_root()), "python": sys.executable}
    ...
    if operation == "inspect" and len(argv) == 2:
        return inspect_treecore(argv[1])
