...
from treecore_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
...
class XAIGrokAdapter(UpstreamAdapter):
    ...
    auth_hint = "treecore auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`treecore auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`treecore auth reset xai-oauth` or re-authenticate with "
                    "`treecore auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `treecore auth add xai-oauth --type oauth`."
            )
