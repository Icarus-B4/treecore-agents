"""CLI handlers for the ``treecore proxy`` subcommand."""
...
from treecore_cli.proxy.adapters import ADAPTERS, get_adapter
from treecore_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "treecore proxy requires aiohttp. Run `treecore setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"treecore auth add {adapter.name}")
        print(
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print(
        "\nStart the proxy with: treecore proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``treecore proxy <subcommand>``."""
    ...
    print(
        "treecore proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  treecore proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  treecore proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  treecore proxy providers\n"
