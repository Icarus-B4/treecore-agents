...
configured for "cli" in `treecore tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (TREECORE_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `treecore chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"treecore -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "treecore -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"treecore -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "treecore -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "treecore -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to TREECORE_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("TREECORE_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "treecore -z: --provider requires --model (or TREECORE_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"treecore -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("treecore -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``treecore -z`` / oneshot mode.
...
def _run_agent(
    ...
    # Imports are local so they don't run when treecore is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from treecore_cli.config import load_config
    from treecore_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("TREECORE_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from treecore_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("TREECORE_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    from treecore_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                TREECORE_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via TREECORE_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via TREECORE_YOLO_MODE=1
