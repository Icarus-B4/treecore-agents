...
Extracted verbatim from ``treecore_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from treecore_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``treecore model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `treecore import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`treecore -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the TREECORE_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under TREECORE_HOME.
    output: str = ""
