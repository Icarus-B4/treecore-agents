...
The actual runtime resolution happens in treecore_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "treecore"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, treecore-tools
    ...
    if new_value == "codex_app_server":
        ...
        try:
            from treecore_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the treecore-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "treecore-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            if "treecore-tools" in mig_report.migrated:
                msg_lines.append(
