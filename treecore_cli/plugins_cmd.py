"""``treecore plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.treecore/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    plugins = get_treecore_home() / "plugins"
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.treecore/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from treecore_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_treecore_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_treecore_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_treecore_home()}/.env later)[/dim]")
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.treecore/plugins``.
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `treecore plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `treecore plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  treecore gateway restart[/dim]")
    console.print()
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``treecore setup`` / ``treecore plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``treecore plugins enable`` /
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`treecore plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``treecore_agents.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.treecore/plugins``. Include package metadata here
    so ``treecore plugins list`` can show and enable them.
    """
    from treecore_cli.plugins import ENTRY_POINTS_GROUP
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``treecore plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] treecore plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] treecore plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] treecore plugins")
    console.print("[dim]Enable/disable:[/dim] treecore plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        from treecore_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``treecore plugins enable web/firecrawl`` cleared
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.treecore/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.treecore/plugins/<name>``."""
    ...
    # Sibling of the CLI ``treecore plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``treecore_cli.main`` never covers them. After a
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.treecore/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    """Dispatch treecore plugins subcommands."""
