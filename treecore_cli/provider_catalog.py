...
The provider list shown by ``treecore model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`treecore_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``treecore model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`treecore_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``treecore model`` shows.
"""
...
# treecore_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `treecore model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``treecore model`` universe.
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = TREECORE_OVERLAYS.get(slug)
