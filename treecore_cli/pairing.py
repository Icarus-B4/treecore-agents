...
Usage:
    treecore pairing list              # Show all pending + approved users
    treecore pairing approve <platform> <request-id|code>  # Approve a pairing request
    treecore pairing revoke <platform> <user_id> # Revoke user access
    treecore pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle treecore pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: treecore pairing {list|approve|revoke|clear-pending}")
        print("Run 'treecore pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: treecore pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.treecore/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'treecore pairing list' to see pending requests.\n")
