...
from treecore_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://treecore-agent.nousresearch.com/docs"
...
# display_treecore_home imported lazily at call sites (stale-module safety during treecore update)
...
from treecore_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from treecore_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  treecore config set model.provider custom")
    print_info("  treecore config set model.base_url http://localhost:8080/v1")
    print_info("  treecore config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'treecore setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``TREECORE_NONINTERACTIVE=1`` (see ``treecore_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("TREECORE_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``TREECORE_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from treecore_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'treecore setup')")


def _print_setup_summary(config: dict, treecore_home):
    ...
    try:
        from treecore_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        ...
        print_info("    treecore model            (pick any provider/model)")
        print_info("    treecore setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'treecore setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from treecore_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `treecore tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from treecore_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'treecore setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'treecore setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'treecore tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'treecore setup terminal'"))
    ...
    # Spotify (OAuth via treecore auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'treecore setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {treecore_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('treecore setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('treecore setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('treecore setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('treecore setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('treecore setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('treecore config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('treecore config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('treecore config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('treecore', Colors.GREEN)}              Start chatting")
    print(f"   {color('treecore gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('treecore doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `treecore tools` and `treecore setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``treecore model``)
    ...
    provider added to ``treecore model`` is automatically available here.
    ...
    # Delegate to the shared treecore model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from treecore_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: treecore model")
    ...
    # on demand via `treecore auth add`, `treecore setup` vision, and
    # `treecore setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from treecore_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``treecore model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from treecore_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.treecore/.env."
            )
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via treecore setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'treecore setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `treecore setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `treecore setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `treecore egress setup` then `treecore egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `treecore egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'treecore-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $TREECORE_HOME/bin is never on PATH, so a bare
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'treecore-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into TREECORE_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("TREECORE_MAX_ITERATIONS")
    ...
    print_info("  Run `treecore setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # TREECORE_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.treecore/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from treecore_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_treecore_home(Path(get_treecore_home()))
    except Exception:
        pass
...
def _profile_name_from_treecore_home(treecore_home) -> str | None:
    """Return the active profile name when TREECORE_HOME is a profile dir."""
    if treecore_home.parent.name == "profiles":
        return treecore_home.name
    return None
...
def _setup_qqbot():
    ...
    from treecore_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://treecore-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://treecore-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  treecore config edit")
    print_info("   Open config in your editor:  treecore config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from treecore_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if not selected:
        print_info("No platforms selected. Run 'treecore setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     treecore config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_treecore_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: treecore gateway install --system")
                print_info("  Or run in foreground:  treecore gateway")
        else:
            from treecore_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   treecore gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   treecore gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `treecore setup tools` and `treecore tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``treecore_cli.auth`` — lists every supported
    ...
    """
    try:
        from treecore_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from treecore_cli.config
    ...
    if section_key == "model":
        ...
        from treecore_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_treecore.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_treecore migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_treecore", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(treecore_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: treecore claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=treecore_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: treecore claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use treecore claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``treecore setup --portal`` and ``treecore portal``. This is the
    ...
    ``treecore setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``treecore model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``treecore portal`` always offers a model picker,
    ...
    # setup calls, so `treecore portal` == quick setup's Nous step.
    try:
        from treecore_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `treecore portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `treecore portal`: %s", exc)
        print()
    ...
    print_info("  Run `treecore portal info` to inspect routing.")
    print_info("  Run `treecore` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      treecore setup           — full or quick (auto-detected)
      treecore setup model     — just model/provider
      treecore setup tts       — just text-to-speech
      treecore setup terminal  — just terminal backend
      treecore setup gateway   — just messaging platforms
      treecore setup tools     — just tool configuration
      treecore setup telemetry — just local shared metrics
      treecore setup agent     — just agent settings
    """
    from treecore_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, treecore_home)
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'treecore setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(treecore_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, treecore_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, treecore_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {treecore_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'treecore config edit'")
    ...
    # Tune later with `treecore setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, treecore_home)


def _run_first_time_quick_setup(config: dict, treecore_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``treecore setup <section>``
    or switch providers with ``treecore model``.
    """
    from treecore_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'treecore setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    treecore setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  treecore setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``treecore tools`` (which
    ...
    try:
        ...
        from treecore_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``treecore setup agent`` /
    ``treecore config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, treecore_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `treecore update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        treecore tools")
        print_info("  Seed skills:         treecore skills opt-in --sync")
        print_info("  Add MCP servers:     treecore mcp add")
        print_info("  Enable plugins:      treecore plugins")
        print_info("  Tune agent settings: treecore setup agent")
        print()
        _print_setup_summary(config, treecore_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, treecore_home)


def _blank_slate_walkthrough(config: dict, treecore_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`treecore update` runs from re-injecting them. Opt back in any")
            print_info("time with `treecore skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `treecore tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `treecore plugins list` / `treecore plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `treecore plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `treecore mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `treecore mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   treecore tools")
    print_info("  Seed skills:         treecore skills opt-in --sync")
    print_info("  Add MCP servers:     treecore mcp add")
    print_info("  Tune agent settings: treecore setup agent")
    print()
...
def _run_quick_setup(config: dict, treecore_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'treecore setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'treecore setup gateway'.")
