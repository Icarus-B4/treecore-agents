...
entries via ``treecore mcp catalog`` or the interactive ``treecore mcp picker``,
and install them with ``treecore mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into treecore-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``treecore mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.treecore/.env`` (the
...
from treecore_constants import get_treecore_home, get_optional_mcps_dir
...
from treecore_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.treecore/.env.
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if t_type == "http" and a_type == "api_key":
        ...
        # at connect time.
        from treecore_cli.mcp_config import _env_key_for_server
...
def _install_root() -> Path:
    ...
    root = get_treecore_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.treecore/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.treecore/.env via save_env_value()."""
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from treecore_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from treecore_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``treecore mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `treecore mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `treecore mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `treecore mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `treecore mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `treecore auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`treecore auth {entry.auth.provider}` if you have not "
    ...
    from treecore_cli.mcp_config import _save_mcp_server
