...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with treecore-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.treecore/plugins/<name>/``
3. **Project plugins** – ``./.treecore/plugins/<name>/`` (opt-in via
   ``TREECORE_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``treecore_agents.plugins``
   entry-point group.
...
from treecore_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``TREECORE_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("TREECORE_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``TREECORE_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.treecore/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("TREECORE_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When TREECORE_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "TREECORE_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by treecore_cli.kanban_db when a task
    ...
    # separate `treecore -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `treecore kanban dispatch`),
...
ENTRY_POINTS_GROUP = "treecore_agents.plugins"

_NS_PARENT = "treecore_plugins"
...
class PluginManifest:
    ...
    #              in ~/.treecore/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``treecore plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``TREECORE_HOME`` via
        :func:`treecore_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.treecore/profiles/<name>``, or ``"custom"`` when
        ``TREECORE_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``treecore honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``treecore <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`treecore_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_treecore_dotenv()`` startup — after
        ``~/.treecore/.env`` loads, before Hermes reads credentials — when
        ...
        the first ``load_treecore_dotenv()`` call, so a plugin-registered
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``treecore model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: treecore_cli.main imports plugins indirectly
        from treecore_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.treecore/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("TREECORE_SAFE_MODE"):
            logger.info("TREECORE_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with treecore and must
            ...
            # all ~20 of them added several seconds to every `treecore`
            # invocation — including plain `treecore chat`, which never touches a
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `treecore plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.treecore/plugins/)
        user_dir = get_treecore_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.treecore/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("TREECORE_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".treecore" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set TREECORE_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("TREECORE_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from treecore_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from treecore_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``treecore plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`treecore plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `treecore plugins list`.
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from treecore_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_treecore_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``treecore_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``treecore_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def _get_pre_tool_call_directive_details(
    ...
    from treecore_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="treecore-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``treecore tools`` TUI so plugin-provided toolsets appear
