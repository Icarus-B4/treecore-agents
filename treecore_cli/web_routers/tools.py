...
late-binding seam in :mod:`treecore_cli.web_deps` (``late`` for callables,
...
from treecore_cli.web_deps import late, LateState
from treecore_cli.web_models import (
...
_log = logging.getLogger("treecore_cli.web_server")
...
_spawn_treecore_action = late("_spawn_treecore_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from treecore_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``treecore tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `treecore tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``treecore tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.treecore/.env`` via ``save_env_value`` —
    the same store ``treecore tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``treecore tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``treecore -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same TREECORE_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``treecore computer-use permissions grant`` as a background action.
