...
from treecore_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``treecore_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``treecore_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.treecore/skills/ for newly installed or removed skills",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`treecore_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/treecore`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``treecore skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``treecore skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``treecore skills list`` and
...
# unrelated command landed. These claim their slots right after /treecore,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /treecore <command>).
...
# native slot, the alias spelling stays reachable via /treecore reset).
...
# ``/treecore <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/treecore" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /treecore topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /treecore moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /treecore debug on Slack.
#   - egress: Docker-only proxy status; reachable as /treecore egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /treecore init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /treecore version on
...
#   - diff: git working-tree diff; reached via /treecore diff on Slack so it
...
#     /treecore update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /treecore heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /treecore refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
_SLACK_VIA_TREECORE_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/treecore <verb>`` subcommand.
    ...
    can still reach them via ``/treecore <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/treecore`` is always reserved as the first entry so the
    legacy ``/treecore <subcommand>`` form keeps working for anything that
    ...
    # Reserve /treecore as the catch-all top-level command.
    entries.append(("treecore", "Talk to Hermes or run a subcommand", "[subcommand] [args]"))
    seen.add("treecore")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_TREECORE_ONLY:
            # Intentionally Slack-via-/treecore only (see _SLACK_VIA_TREECORE_ONLY).
            return
    ...
    # everything except /treecore, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://treecore-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /treecore handler.

    Maps both canonical names and aliases so /treecore bg do stuff works
    the same as /treecore background do stuff.

    Plugin-registered slash commands are included so ``/treecore <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from treecore_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
