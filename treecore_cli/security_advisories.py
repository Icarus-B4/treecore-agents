...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``treecore doctor --ack <id>``; the ack is persisted to
...
1. ``treecore doctor`` (and ``treecore doctor --ack <id>``)
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including treecore when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.treecore/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: treecore doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'treecore doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.treecore/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_treecore_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``treecore doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `treecore doctor` on the gateway host for details.")
