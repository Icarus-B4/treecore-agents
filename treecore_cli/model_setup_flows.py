"""Per-provider model-selection wizard flows for ``treecore setup`` / ``treecore model``.

Extracted from ``treecore_cli/main.py`` as part of the god-file decomposition
campaign (``~/.treecore/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from treecore_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``treecore_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from treecore_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``treecore_cli.main`` at import time -> no import cycle.
"""
...
from treecore_cli.providers import custom_provider_slug
...
def _existing_api_key_for_model_flow(provider_id: str, pconfig) -> tuple[str, str]:
    ...
    from treecore_cli.auth import _resolve_api_key_provider_secret
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from treecore_cli.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from treecore_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.treecore/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `treecore setup` from scratch. OpenRouter
    ...
    from treecore_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from treecore_constants import AI_GATEWAY_BASE_URL
    from treecore_cli.main import _prompt_api_key
    from treecore_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.treecore/.env by hand.
    ...
    from treecore_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from treecore_cli.auth import _save_model_choice, deactivate_provider
    from treecore_cli.config import load_config, save_config
    ...
    if not presets:
        print("No MoA presets configured. Run `treecore moa configure <name>` first.")
        return
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from treecore_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from treecore_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `treecore auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``treecore auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from treecore_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from treecore_cli.auth import (
    ...
    from treecore_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from treecore_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from treecore_cli.auth import _save_model_choice, deactivate_provider
    from treecore_cli.config import (
    ...
    from treecore_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `treecore model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from treecore_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from treecore_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from treecore_cli import azure_detect
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from treecore_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from treecore_cli.auth import _save_model_choice, deactivate_provider
    from treecore_cli.config import load_config, save_config
    from treecore_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from treecore_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from treecore_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from treecore_cli.config import save_env_value, load_config, save_config
    from treecore_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from treecore_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from treecore_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set TREECORE_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from treecore_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from treecore_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from treecore_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `treecore auth`)
    from treecore_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
    ...
    if selected:
        ...
        # Save the API key env var name so treecore knows where to find it
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from treecore_cli.config import load_config, save_config, get_env_value
    from treecore_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.treecore/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``treecore_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from treecore_cli.main import _prompt_provider_choice
    from treecore_cli.auth import ZAI_ENDPOINTS
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from treecore_cli.main import _run_anthropic_oauth_flow
    from treecore_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
    ...
    if selected:
        ...
        # providers if the user switches without running 'treecore model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
