...
Each profile is a fully independent TREECORE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.treecore/profiles/<name>/`` by default.

The "default" profile is ``~/.treecore`` itself — backward compatible,
zero migration needed.

Usage::

    treecore profile create coder          # fresh profile + bundled skills
    treecore profile create coder --clone  # also copy config, .env, SOUL.md, skills
    treecore profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    treecore -p coder chat                 # or via flag
    treecore profile use coder             # set as sticky default
    treecore profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See treecore_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.treecore``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   treecore-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `treecore backup` archives
...
# Marker file written by `treecore profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `treecore
...
# `treecore skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.treecore``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_treecore_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.treecore) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "treecore-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when TREECORE_HOME equals the
...
# artifacts* at the root of TREECORE_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``treecore_constants``.
...
_RESERVED_NAMES = frozenset({
    "treecore", "default", "test", "tmp", "root", "sudo",
})

# Hermes subcommands that cannot be used as profile names/aliases
_TREECORE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the treecore root, NOT to the current TREECORE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where TREECORE_HOME points outside
    ``~/.treecore``, profiles live under ``TREECORE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_treecore_home() / "profiles"


def _get_default_treecore_home() -> Path:
    """Return the default (pre-profile) TREECORE_HOME path.

    In standard deployments this is ``~/.treecore``.
    In Docker/custom deployments where TREECORE_HOME is outside ``~/.treecore``
    (e.g. ``/opt/data``), returns TREECORE_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_treecore_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its TREECORE_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_treecore_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, treecore subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _TREECORE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a treecore subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            treecore_exe = shutil.which("treecore") or "treecore"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(treecore_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        # not be migrated. The next `treecore doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``treecore -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``treecore profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``treecore -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``treecore profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``TREECORE_HOME``.
    """
    try:
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_treecore_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, treecore_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own TREECORE_HOME.
    ...
    The returned ``treecore_home`` is the path to pass to
    ``set_treecore_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_treecore_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``treecore update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.treecore)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.treecore/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `treecore -p <name> auth add`) had no file until first
    ...
    if not soul_path.exists():
        try:
            from treecore_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `treecore update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`treecore profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `treecore update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `treecore profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `treecore -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches TREECORE_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``treecore profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "TREECORE_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_treecore_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``TREECORE_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `treecore -p <canon> profile
    ...
    treecore_markers = ("treecore_cli.main", "treecore-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # a resolved executable named `treecore`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_treecore = (
                any(marker in joined for marker in treecore_markers)
                or exe_name == "treecore"
                or exe_name.startswith("treecore")
            )
            if not is_treecore:
                continue
            ...
            # ...or by TREECORE_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("TREECORE_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.treecore).\n"
            "To remove everything, use: treecore uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_treecore_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``TREECORE_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from treecore_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set TREECORE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("TREECORE_HOME")
    try:
        os.environ["TREECORE_HOME"] = str(profile_dir)
        from treecore_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["TREECORE_HOME"] = old_home
        elif "TREECORE_HOME" in os.environ:
            del os.environ["TREECORE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.treecore/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: treecore profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from TREECORE_HOME.

    Returns ``"default"`` if TREECORE_HOME is not set or points to ``~/.treecore``.
    Returns the profile name if TREECORE_HOME points into ``~/.treecore/profiles/<name>``.
    Returns ``"custom"`` if TREECORE_HOME is set to an unrecognized path.
    """
    from treecore_constants import get_treecore_home
    treecore_home = get_treecore_home()
    resolved = treecore_home.resolve()

    default_resolved = _get_default_treecore_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      TREECORE_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Hermes file the
      user may have lying alongside TREECORE_HOME (#58394).
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.treecore itself — its parent is ~/ and its
        # directory name is ".treecore", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: treecore profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.treecore itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.treecore). "
            "Specify a different name: treecore profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="treecore_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"treecore_{old_name}"
    legacy_old_host = f"treecore.{old_name}"
    new_host = f"treecore_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_treecore_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("treecore_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a TREECORE_HOME path string.

    Called early in the CLI entry point, before any treecore modules
    are imported, to set the TREECORE_HOME environment variable.
    """
