...
and reusable from both `treecore doctor` and a future `treecore migrate xai`.
"""
...
def apply_migration(
    ...
    from treecore_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `treecore migrate xai` reports nothing to migrate
