"""treecore claw — OpenClaw migration commands.

Usage:
    treecore claw migrate              # Preview then migrate (always shows preview first)
    treecore claw migrate --dry-run    # Preview only, no changes
    treecore claw migrate --yes        # Skip confirmation prompt
    treecore claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    treecore claw migrate --no-backup  # Skip pre-migration snapshot
    treecore claw cleanup              # Archive leftover OpenClaw directories
    treecore claw cleanup --dry-run    # Preview what would be archived
"""
...
from treecore_cli.config import get_treecore_home, get_config_path, load_config, save_config
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'treecore gateway stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_treecore.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_treecore", script_path)
...
def claw_command(args):
    """Route treecore claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: treecore claw <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print("Run 'treecore claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    # migrate-treecore posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: treecore claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.treecore/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: treecore claw migrate --yes")
            return
    ...
    if not no_backup:
        try:
            from treecore_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(treecore_home=treecore_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: treecore import {backup_archive.name}")
        except Exception as e:
            print()
    ...
    # 'treecore claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: treecore claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: treecore claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  treecore claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  treecore claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  treecore config set OPENROUTER_API_KEY sk-or-v1-...")
