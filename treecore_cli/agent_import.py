"""treecore import-agent — import Claude Code / Codex CLI setups into Hermes.

Usage:
    treecore import-agent                       # auto-detect ~/.claude or ~/.codex
    treecore import-agent claude-code           # import from ~/.claude
    treecore import-agent codex                 # import from ~/.codex
    treecore import-agent claude-code --dry-run # preview only, no changes
    treecore import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``treecore claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_treecore.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in TREECORE_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → TREECORE_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in TREECORE_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in TREECORE_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → TREECORE_HOME/skills/codex-imports/<name>/
...
deliberately via ``treecore setup`` or config.yaml.
"""
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `treecore config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`treecore config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_treecore.py)
# ---------------------------------------------------------------------------
...
class AgentImporter:
    ...
    def import_mcp_servers(self, servers: Dict[str, Any], kind: str) -> None:
        ...
        for name, srv in servers.items():
            ...
            treecore_srv: Dict[str, Any] = {}
            if srv.get("command"):
                treecore_srv["command"] = srv["command"]
                if srv.get("args"):
                    treecore_srv["args"] = srv["args"]
                env_kept, env_stripped = sanitize_mcp_env(srv.get("env"))
                if env_kept:
                    treecore_srv["env"] = env_kept
                if env_stripped:
                    self.stripped_secrets.extend(
                        f"mcp_servers.{name}.env.{k}" for k in env_stripped
                    )
                if srv.get("cwd"):
                    treecore_srv["cwd"] = srv["cwd"]
            if srv.get("url"):
                treecore_srv["url"] = srv["url"]
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        treecore_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → TREECORE_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``treecore import-agent`` (invoked from treecore_cli.main)."""
    from treecore_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: treecore import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: treecore import-agent claude-code   or   treecore import-agent codex")
            return
        agent = detected[0]
    ...
    if not source_dir.is_dir():
        ...
        print_info("Specify a custom path: treecore import-agent "
    ...
    print_info(f"Target:      {treecore_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'treecore setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: treecore import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'treecore setup' "
               "to configure providers, or add them to ~/.treecore/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from treecore_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'treecore setup' or ~/.treecore/.env.")
        print()
