...
process replacement so that ``treecore sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``treecore`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from treecore_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched treecore."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_treecore_bin() -> Optional[str]:
    """Find the treecore entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("treecore")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m treecore_cli.main``.
    ...
    ``treecore.exe`` on PATH or the ``sys.executable -m treecore_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("treecore")
    if path_bin:
        return path_bin
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with treecore.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_treecore_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "treecore_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh treecore invocation.
    ...
    relaunch contract wants: "run treecore again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``treecore.exe`` (a Python console-script shim that wraps
    ``python -m treecore_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "treecore exited, then
    new treecore started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``treecore`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nHermes relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run treecore.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
