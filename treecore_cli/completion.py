"""Shell completion script generation for treecore CLI.
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_treecore_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
...
# Add to ~/.bashrc:
#   eval "$(treecore completion bash)"

_treecore_profiles() {{
    local profiles_dir="$HOME/.treecore/profiles"
...
_treecore_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_treecore_profiles)" -- "$cur"))
        return
    fi
...
complete -F _treecore_completion treecore
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef treecore
# Hermes Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(treecore completion zsh)"

_treecore_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.treecore/profiles" ]]; then
        profiles+=($HOME/.treecore/profiles/*(N/:t))
    fi
...
_treecore() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_treecore_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'treecore command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "# Add to your config:",
        "#   treecore completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __treecore_profiles",
        "    echo default",
        "    if test -d $HOME/.treecore/profiles",
        "        for d in $HOME/.treecore/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c treecore -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c treecore -f -s p -l profile"
        " -d 'Profile name' -xa '(__treecore_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__treecore_profiles)' -d 'Profile name'"
                )
