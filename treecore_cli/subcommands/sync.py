"""``treecore sync`` subcommand parser — Skill Sync.

Cloned from ``treecore_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    treecore sync status                 show gate/opt-in/head state
    treecore sync pull                   pull and materialize opted-in skills
    treecore sync push                   push opted-in skills
    treecore sync now                    reconcile: pull then push
    treecore sync enable <skill>         opt a skill into sync
    treecore sync disable <skill>        opt a skill out of sync
    treecore sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    treecore sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  treecore sync status            what is synced, and from where\n"
            "  treecore sync enable my-skill   include a skill in your sync\n"
            "  treecore sync now               pull, then push\n"
            "  treecore sync propose my-skill  share a skill with your team\n"
        ),
