"""``treecore debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hermes Agent. Use 'treecore debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    treecore debug share              Upload debug report (asks for confirmation)
    treecore debug share --yes        Skip confirmation (for scripts/CI)
    treecore debug share --lines 500  Include more log lines
    treecore debug share --expire 30  Keep paste for 30 days
    treecore debug share --local      Print report locally (no upload)
    treecore debug share --no-redact  Disable upload-time secret redaction
    treecore debug share --nous       Upload to Nous-internal storage (private)
    treecore debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'treecore debug share'",
    )
