"""``treecore login`` subcommand parser.

Extracted verbatim from ``treecore_cli/main.py:main()`` (god-file Phase 2).
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``treecore login`` was removed in favor of ``treecore auth`` / ``treecore model``
    (the runtime handler in ``treecore_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``treecore login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``treecore --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``treecore login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``treecore model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `treecore auth` to manage credentials, "
            "`treecore model` to select a provider, or `treecore setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `treecore model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: treecore-cli)"
    )
