"""``treecore claw`` subcommand parser.
...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.treecore/ (by default a "
        "single restore-point archive is written to ~/.treecore/backups/ "
        "before apply; restorable with 'treecore import').",
    )
    claw_migrate.add_argument(
