"""``treecore dashboard`` / ``treecore serve`` subcommand parsers.
...
``treecore_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    """
    parser.add_argument(
        "--port", type=int, default=9120, help="Port (Treecore default 9120, 0 for auto-assign by OS)"
    )
    parser.add_argument(
    ...
    # `treecore dashboard` / `treecore serve` cmdlines and SIGTERM them directly —
    # the same path `treecore update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Treecore web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Treecore web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Treecore Agents web dashboard for managing config, API keys, and sessions",
    )
    ...
    # backend as `treecore dashboard --no-open --tui --host ... --port ...`. The
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Treecore backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Treecore backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports TREECORE_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `treecore dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.treecore/.env.
    # Nested subparser so bare `treecore dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "TREECORE_DASHBOARD_OAUTH_CLIENT_ID into ~/.treecore/.env, and prints "
            "how to engage the login gate. Requires being logged in (treecore setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://treecore.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via TREECORE_DASHBOARD_PORTAL_URL. Mainly for "
