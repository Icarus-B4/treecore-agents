"""``treecore pause`` / ``treecore resume`` — the global emergency stop.

``treecore pause`` writes the ESTOP sentinel at ``$TREECORE_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``treecore resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    print(
        ...
        "    In-flight work keeps running. Run `treecore resume` to lift the pause."
    )
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`treecore resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `treecore pause`",
