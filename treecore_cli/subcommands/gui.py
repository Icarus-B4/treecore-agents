"""``treecore gui`` subcommand parser.
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any treecore CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--treecore-root",
        help="Override the Hermes source root used by Desktop (sets TREECORE_DESKTOP_TREECORE_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets TREECORE_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
