...
3. Running inside a container with no persistent volume mount over the
   TREECORE_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("treecore.security_audit")
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("TREECORE_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(treecore_home: Optional[Path]) -> Optional[str]:
...
def run_security_audit(
    *, treecore_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(treecore_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(treecore_home=treecore_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
