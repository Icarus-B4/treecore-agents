...
* ``treecore profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``treecore skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``treecore profile``, not a parallel tree):

    treecore profile install <source> [--name N] [--alias] [--force] [--yes]
    treecore profile update  <name>  [--force-config] [--yes]
    treecore profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "treecore_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".treecore_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "treecore-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            treecore_requires=str(data.get("treecore_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.treecore_requires:
            out["treecore_requires"] = self.treecore_requires
        if self.author:
...
def check_treecore_requires(spec: str, current_version: str) -> None:
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``treecore profile update`` can
...
def plan_install(
    ...
    check_treecore_requires(manifest.treecore_requires, treecore_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.treecore).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="treecore_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `treecore profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`treecore profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="treecore_dist_update_") as tmp:
        plan = plan_install(
