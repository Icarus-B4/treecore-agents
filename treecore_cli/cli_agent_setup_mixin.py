...
(``~/.treecore/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
class CLIAgentSetupMixin:
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("   Run 'treecore model' to choose a provider, or "
                      "'treecore setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: treecore setup")
            return False
        ...
        # `treecore chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `treecore auth add openai-codex`
        # without `treecore model`), fall back to the provider's first catalog
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``treecore model`` (which fronts Quick Setup / Nous
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'treecore model' or 'treecore setup' any time.")
            return False
        ...
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'treecore model' any time.")
            return False
        except Exception as exc:
            logger.debug("first-run provider setup failed: %s", exc)
            _cprint(f"  ⚠️  Provider setup failed: {exc}")
            _cprint("  Run 'treecore model' to try again.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'treecore model' to retry.")
        return False
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (treecore sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (treecore sessions list).{_RST}")
                return False
        ...
        try:
            ...
            from treecore_constants import partial_update_hint
