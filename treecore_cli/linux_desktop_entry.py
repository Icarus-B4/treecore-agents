"""Install and remove the Linux desktop entry (``treecore.desktop``).

``treecore desktop`` builds and launches the Electron app. On Linux, a
...
``treecore uninstall --gui`` removes the entry again.

Two values must be absolute for the entry to work:

  - ``Exec`` — the launcher runs without shell ``PATH`` customizations, so
    a bare ``treecore desktop`` fails when treecore lives in ``~/.local/bin``
...
DESKTOP_ENTRY_NAME = "treecore.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``treecore.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``treecore desktop``.

    Prefer the real ``treecore`` executable (argv[0] or PATH). When Hermes
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from treecore_cli.relaunch import resolve_treecore_bin

    bin_path = resolve_treecore_bin()
    if bin_path:
        argv = [str(Path(bin_path).resolve()), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "treecore_cli.main", "desktop"]
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    ...
    icon_value = str(icon) if icon.is_file() else "treecore"
