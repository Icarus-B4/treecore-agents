    ...
    *,
    ...
    if account_info is None:
        return (
            ...
            f"is unavailable. Run `treecore model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                ...
                f"`treecore model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `treecore model`. "
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `treecore model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            ...
            f"login, so {capability} is unavailable. Run `treecore model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `treecore model` to refresh Hermes."
        return message
...
def _no_paid_access_message(
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f" member spend cap at {billing_url}, then run `treecore model`"
            f" to refresh."
        )
...
def _fresh_account_info(
    *,
    ...
    try:
        from treecore_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from treecore_cli.auth import _decode_jwt_claims
    except Exception:
        return None
