...
TREECORE_OVERLAYS: Dict[str, HermesOverlay] = {
    "moa": HermesOverlay(
    ...
    "qwen-oauth": HermesOverlay(
        ...
        base_url_env_var="TREECORE_QWEN_BASE_URL",
    ),
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "treecore", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    overlay = TREECORE_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + treecore extra
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # Direct provider checks for providers not in TREECORE_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    if canonical != raw:
        try:
            from treecore_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
