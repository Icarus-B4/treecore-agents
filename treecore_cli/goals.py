...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current TREECORE_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``treecore_home`` path so profile switches still pick up the right DB.
        ...
        if state.consecutive_transport_failures >= DEFAULT_MAX_CONSECUTIVE_TRANSPORT_FAILURES:
            ...
            return {
                ...
                "message": (
                    ...
                    "Check the goal_judge provider/key in ~/.treecore/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: deepseek\n"
...
def run_kanban_goal_loop(
    *,
    ...
    (``treecore -p <profile> chat -q "work kanban task <id>"``). The worker's
