"""Dependency-light venv recovery that runs BEFORE treecore_cli.main's imports.

The ``treecore`` console entry point is ``treecore_cli.main:main``.  Importing
``treecore_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``treecore_cli.env_loader``, ``yaml`` via ``treecore_cli.config``, ...).  In
...
a corrupted venv.  ``treecore_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``treecore_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``treecore acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``treecore_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``treecore_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``treecore update`` exists AND an import probe
