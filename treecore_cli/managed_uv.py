...
Hermes owns its own uv binary at ``$TREECORE_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$TREECORE_HOME/bin`` so the installer writes directly there — no PATH
...
uses an install-scoped store under ``<checkout>/.treecore-runtime/python``. A
...
from treecore_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".treecore-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    ...
    ``$TREECORE_HOME/bin/uv`` on POSIX, ``$TREECORE_HOME\\bin\\uv.exe`` on
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            ...
            "out of WAL mode on this SQLite build. The next `treecore update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``treecore update`` runs
    the call site from the *old*, already-imported ``treecore_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``treecore update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under TREECORE_HOME
    caches the last successful self-update time.
    """
    try:
        from treecore_constants import get_treecore_home

        stamp = get_treecore_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``treecore update`` so the managed copy stays current.
...
def _reload_treecore_constants():
    """Re-execute ``treecore_constants`` from disk and return the fresh module.

    ``treecore update`` imports ``treecore_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        cannot import name 'venv_python_path' from 'treecore_constants'
        (~/.treecore/treecore-agent/treecore_constants.py)
    ...
    """
    import treecore_constants

    return importlib.reload(treecore_constants)
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("treecore_cli.main")
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``treecore update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``treecore update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$TREECORE_HOME/bin/`` instead of ``~/.local/bin/``.
    """
