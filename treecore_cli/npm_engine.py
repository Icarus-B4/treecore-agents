...
lives inside its **own** managed Node tree (``$TREECORE_HOME/node``), installing
...
from treecore_constants import (
    bootstrap_treecore_managed_node,
    get_treecore_home,
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    ...
    ``$TREECORE_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$TREECORE_HOME/node/etc/npmrc`` so that global
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="treecore-npm-upgrade-") as tmp:
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    ...
    Installs the managed tree under ``$TREECORE_HOME/node`` (reusing a healthy
    ...
    managed_npm = bootstrap_treecore_managed_node()
    if not managed_npm:
        if not quiet:
