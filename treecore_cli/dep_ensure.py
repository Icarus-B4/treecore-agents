...
from treecore_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import treecore_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $TREECORE_HOME/node is
    ...
    "browser": lambda: (
        ...
        or _has_treecore_agents_browser()
    ),
...
def _has_treecore_agents_browser() -> bool:
    ...
    # install.sh installs globally into $TREECORE_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-HermesHome", str(get_treecore_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = treecore_subprocess_env(inherit_credentials=False)
