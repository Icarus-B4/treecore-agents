...
`treecore tools` and `treecore setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.treecore/config.yaml under
...
from treecore_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from treecore_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``treecore tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`treecore_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from treecore_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _TREECORE_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `treecore tools` → Video Generation, which walks
...
# `treecore tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `treecore tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from treecore_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `treecore tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``treecore tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from treecore_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``treecore tools`` checklist actually offers
    for ``platform``.
    ...
    so ``treecore tools`` never claims to add or remove a toolset the user was
...
from treecore_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set TREECORE_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "TREECORE_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "TREECORE_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "TREECORE_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "TREECORE_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "TREECORE_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `treecore model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("TREECORE_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $TREECORE_HOME/bin is never on PATH, so a bare which()
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``treecore update`` and
      by ``treecore computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``treecore update`` sets
    ...
    a further ~600s wait on Windows). ``treecore computer-use install
    ...
    ``treecore update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `treecore update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `treecore computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `treecore update` (require_confirmed_update=True) keeps the
    ...
    # `treecore computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: treecore computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `treecore update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `treecore update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.treecore/logs/update.log — write straight to it so
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $TREECORE_HOME/node is not on PATH, so a bare
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_treecore_home()}/treecore-agent && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in treecore-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/treecore-agent:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.treecore/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.treecore/config.yaml")
        ...
        # Run the full `treecore auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.treecore/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from treecore_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: treecore auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `treecore auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: treecore auth spotify")
        except Exception as exc:
        ...
        try:
            from treecore_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: treecore plugins enable observability/langfuse")
        _print_info("    Restart Hermes for tracing to take effect.")
        _print_info("    Verify: treecore plugins list")
        ...
        try:
            ...
            _print_info("    Run later: treecore auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `treecore auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: treecore auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: treecore auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``treecore tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``treecore tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: treecore tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``treecore-discord``, which contains those
...
#:
#: Saving ``treecore tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[treecore-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``treecore tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"treecore-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (treecore-acp, treecore-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``treecore-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"treecore-{platform}"
    ...
    # "treecore-cli" (which include all _TREECORE_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[treecore-cli, spotify]`` after enabling Spotify via ``treecore
        ...
        # (e.g. "treecore-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``treecore tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `treecore tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"treecore-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("treecore-")}
    ...
    # shipped as a bundled plugin but user must opt in via `treecore tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `treecore tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by treecore tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `treecore` instead of
    # `treecore-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `treecore update`/`treecore doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `treecore tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (treecore-cli, treecore-telegram, etc.)
    ...
    # Opening `treecore tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `treecore tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    from treecore_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``treecore tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from treecore_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (treecore_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `treecore model`).
    if managed_feature:
        from treecore_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``treecore model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``treecore model`` aux-task
    ...
    """
    try:
        from treecore_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `treecore model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `treecore model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `treecore tools` and `treecore setup tools`.
    ...
    print(color("  Guide: https://treecore-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from treecore_constants import display_treecore_home
    print(color(f"  Tool configuration saved to {display_treecore_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'treecore' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, treecore mcp
