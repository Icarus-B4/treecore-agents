...
file. Skins are defined as YAML files in ~/.treecore/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.treecore/skins/ therefore
...
.. code-block:: python
    ...
    from treecore_cli.skin_engine import get_active_skin, list_skins, set_active_skin
    ...
    set_active_skin("mytheme")            # Switch to user skin from ~/.treecore/skins/
...
Drop a YAML file in ``~/.treecore/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces TREECORE_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces TREECORE_CADUCEUS)
...
def _skins_dir() -> Path:
    ...
    return get_treecore_home() / "skins"
