...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `treecore ... > file`) raises
...
from treecore_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built treecore — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/treecore-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/treecore-agent"
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``TREECORE_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = treecore_home / ".update_check"
    embedded_rev = os.environ.get("TREECORE_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # TREECORE_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/treecore/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from treecore_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $TREECORE_HOME/treecore-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = treecore_home / "treecore-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$TREECORE_HOME/treecore-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``treecore_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from treecore_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/treecore-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/treecore-agent repo (forks don't get a link).
    """
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else TREECORE_CADUCEUS
    except Exception:
        _bskin = None
        _hero = TREECORE_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or treecore setup[/]"
            )
        else:
    ...
    if os.getenv("TREECORE_YOLO_MODE"):
    ...
    try:
        from treecore_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from treecore_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built treecore; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else TREECORE_AGENT_LOGO
        console.print(_logo)
        console.print()
