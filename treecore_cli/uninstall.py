...
- Keep data: Remove code but keep ~/.treecore/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing treecore-agent or treecore PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Hermes Agent" comment and following line
                if '# Hermes Agent' in line or '# treecore-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('treecore' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing treecore
                if 'treecore' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the treecore wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "treecore",
        Path.home() / ".local" / "bin" / "treecore-acp",
        Path.home() / ".local" / "bin" / "treecore-agent",
        Path("/usr/local/bin/treecore"),
        Path("/usr/local/bin/treecore-acp"),
        Path("/usr/local/bin/treecore-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains treecore_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'treecore_cli' in content or 'treecore-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(treecore_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``treecore`` command:
    ...
    """
    node_dir = (treecore_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``treecore gateway run`` processes
    ...
    try:
        from treecore_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from treecore_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``TREECORE_HOME`` and ``TREECORE_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\treecore\git\cmd``, ``%LOCALAPPDATA%\treecore\git\bin``,
#      ``%LOCALAPPDATA%\treecore\git\usr\bin``, ``%LOCALAPPDATA%\treecore\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\treecore\git\`` and Node to
#      ``%LOCALAPPDATA%\treecore\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``treecore dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\treecore\gateway-service\`` and sometimes
...
def _treecore_path_markers(treecore_home: Path) -> list[str]:
    """Path-entry substrings that identify Hermes-owned User-PATH entries."""
    root = str(treecore_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare treecore-agent install dir.
    markers = [root + "\\treecore-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if TREECORE_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "treecore".  We don't want to catch
    # unrelated entries like "ctreecore-foo" or "ephermeral", so we look for
    # backslash-treecore as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(treecore_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _treecore_path_markers(treecore_home)
            kept: list[str] = []
...
def remove_treecore_env_vars_windows() -> list[str]:
    """Delete TREECORE_HOME and TREECORE_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("TREECORE_HOME", "TREECORE_GIT_BASH_PATH"):
                try:
...
def _is_default_treecore_home(treecore_home: Path) -> bool:
    """Return True when ``treecore_home`` points at the default (non-profile) root."""
    try:
        from treecore_constants import get_default_treecore_root
        return treecore_home.resolve() == get_default_treecore_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its TREECORE_HOME directory.

    We shell out to ``treecore -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current TREECORE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m treecore_cli.main` so we don't depend on a `treecore`
    #    wrapper that may be half-removed mid-uninstall.
    treecore_invocation = [_sys.executable, "-m", "treecore_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                treecore_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's TREECORE_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``treecore uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$TREECORE_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from treecore_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(treecore_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {treecore_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(treecore_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Hermes agent at {treecore_home / 'treecore-agent'}")
        print(f"  • Your config, sessions, and secrets under {treecore_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Hermes agent is still installed. Run 'treecore' to use the CLI,")
    print("or 'treecore uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.treecore/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.treecore/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie TREECORE_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_treecore_home(treecore_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.treecore data); otherwise keep-data. Named profiles
    ...
    print(f"  Config:  {treecore_home / 'config.yaml'}")
    print(f"  Secrets: {treecore_home / '.env'}")
    print(f"  Data:    {treecore_home / 'cron/'}, {treecore_home / 'sessions/'}, {treecore_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their TREECORE_HOME dirs. Otherwise
...
def _print_uninstall_dry_run(*, project_root: Path, treecore_home: Path, full_uninstall: bool) -> None:
    ...
    if full_uninstall:
        print(f"  • Hermes config/data: {treecore_home}")
        if _is_default_treecore_home(treecore_home):
        ...
        print(f"  • Keep Hermes config/data: {treecore_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``treecore`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$TREECORE_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in treecore_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\treecore\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(treecore_home))))
        ...
        log_info("Removing TREECORE_HOME / TREECORE_GIT_BASH_PATH User env vars...")
        removed_env = remove_treecore_env_vars_windows()
        if removed_env:
    ...
    log_info("Removing treecore command...")
    ...
    removed_node_links = remove_node_symlinks(treecore_home)
    ...
    #     step-5 rmtree(treecore_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE TREECORE_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from treecore_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(treecore_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.treecore/, just remove the treecore-agent subdir
            if treecore_home in project_root.parents or project_root.parent == treecore_home:
    ...
    #     under TREECORE_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(treecore_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(treecore_home)
    ...
    # 5. Optionally remove ~/.treecore/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile TREECORE_HOME dirs live under
        ...
        try:
            if treecore_home.exists():
                shutil.rmtree(treecore_home)
                log_success(f"Removed {treecore_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {treecore_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {treecore_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {treecore_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://treecore-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://treecore-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m treecore_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import treecore_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``treecore_constants`` + ``treecore_cli.colors``
    (and lazily ``treecore_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m treecore_cli.uninstall")
    parser.add_argument(
        "--mode",
