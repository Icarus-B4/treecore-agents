"""Hermes update pipeline — extracted from ``treecore_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``treecore_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``treecore_cli.main`` reference — so existing call sites and test monkeypatches
that target ``treecore_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``treecore_cli.main``.
...
Imports are one-way: ``treecore_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``treecore_cli.main`` reference.

    Lets callers keep patching ``treecore_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``treecore_cli.main`` -> ``treecore_cli.update_cmd`` stays one-way
    at import time.
    """
    from treecore_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``treecore update`` keeps running in the pre-pull Python process. After a
...
_UPDATE_CRITICAL_FILES = (
    "treecore_cli/main.py",
    "treecore_cli/config.py",
    "treecore_cli/__init__.py",
    "treecore_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "treecore_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``treecore`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="treecore-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``treecore update`` can be driven by a
    ...
    """
    from treecore_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from treecore_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('treecore_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``treecore update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `treecore update`.
    ...
    print("  Preview now:  treecore curator run --dry-run")
    print("  Pause it:     treecore curator pause")
    print(
        "  Docs:         https://treecore-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `treecore update`.
    ...
    try:
        ...
        return
    db_path = get_treecore_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `treecore sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    treecore sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  treecore sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``treecore update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``treecore update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: treecore curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    treecore dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``treecore_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.treecore-update-staging"
    backup = f"{dst}.treecore-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `treecore update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``treecore_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("TREECORE_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== treecore-update completed {action_id} ===")
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `treecore update "
            f"--branch {branch}`, or update against main with `treecore update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/treecore-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="treecore-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"treecore-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for treecore-agent itself
        ...
        # GitHub ZIPs extract to treecore-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"treecore-agent-{branch}")
        ...
        print(
            "  Re-run `treecore update` to retry; if the agent won't start, "
            "reinstall from https://treecore-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from treecore_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `treecore update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: treecore skills list-modified  "
    ...
    try:
        from treecore_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from treecore_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_treecore_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_treecore_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "treecore-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes treecore completely unrunnable (SyntaxError on import).
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/treecore-agent.git",
    "git@github.com:NousResearch/treecore-agent.git",
    "https://github.com/NousResearch/treecore-agent",
    "git@github.com:NousResearch/treecore-agent",
}

OFFICIAL_REPO_URL = "https://github.com/NousResearch/treecore-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_treecore_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_treecore_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/treecore-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/treecore-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/treecore-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``treecore update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from treecore_constants import get_default_treecore_root

    default_home = get_default_treecore_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "treecore.exe"
    lines = ["✗ Another treecore.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Hermes Desktop, exit any open `treecore` REPLs, and")
    lines.append("  stop the gateway (`treecore gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `treecore update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``treecore update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `treecore update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `treecore update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from treecore_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from treecore_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$TREECORE_HOME/bin/uv``, but on Termux the official
    ...
    """
    from treecore_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `treecore update` is exactly the step
...
def _npm_lockfile_changed(treecore_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `treecore
    ...
    try:
        ...
        cache_file = treecore_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(treecore_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from treecore_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `treecore update`.")
            failed = ["repo root"]
            if any(
    ...
    shared_treecore_root = get_default_treecore_root()
    if not _m()._npm_lockfile_changed(shared_treecore_root):
    ...
    # need desktop during `treecore update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `treecore update`.")
        return list(labels)
    ...
    nixos_env = with_treecore_node_path(_m()._nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `treecore update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_treecore_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.treecore/logs/update.log`` only, never the terminal.

    During ``treecore update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``treecore update --check``: fetch and report without installing.
    ...
    """
    from treecore_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``treecore update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from treecore_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``treecore update`` without requiring a reinstall.  The
    ...
    ``treecore`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/treecore.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v treecore'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/treecore, code at /usr/local/lib/treecore-agent).
    fhs_link = Path("/usr/local/bin/treecore")
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``treecore-acp`` launcher next to the ``treecore`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``treecore update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``treecore-acp`` command name against the login-shell PATH; the console
    ...
    The shim simply delegates to the sibling ``treecore`` launcher with the
    ...
    ``treecore-acp.exe`` already resolves) and wherever a ``treecore-acp`` is
    already present next to the ``treecore`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        treecore_cmd = bin_dir / "treecore"
        acp_cmd = bin_dir / "treecore-acp"
        try:
            if not (treecore_cmd.is_file() or treecore_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of TREECORE_HOME under
      ``backups/`` (restorable via ``treecore import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from treecore_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from treecore_constants import
        # get_treecore_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from treecore_cli.config import get_treecore_home as _get_home
    ...
    try:
        from treecore_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_treecore_home so the user sees ~/.treecore/...
    try:
        from treecore_constants import get_treecore_home, display_treecore_home

        home = get_treecore_home()
        try:
            display_path = f"{display_treecore_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  treecore import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``treecore update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``treecore update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run treecore from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.treecore-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".treecore-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The treecore.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m treecore_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``treecore
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m treecore_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "treecore_cli.main" in cmdline_low:
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines.append("    treecore update")
    lines.append("  (or use `treecore update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``treecore update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from treecore_cli._scan_venv_blockers import _is_pausable_gateway
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    treecore.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        ...
        from treecore_cli.gateway import (
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m treecore_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: treecore gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``treecore gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from treecore_cli import gateway_windows
        from treecore_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``treecore-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("treecore-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``treecore update`` never touched them, so installs created before the
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``treecore update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``treecore update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another treecore.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_treecore_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `treecore serve` backend) will keep .pyd
    ...
    # the treecore.exe shim guard above, but its lock probe only checks the shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://treecore-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Hermes windows/gateways and re-run: treecore update")
            else:
        ...
        # orphan merge-conflict markers in treecore_cli/config.py bricked
        # every user who ran ``treecore update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``treecore
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``treecore update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_treecore_home added to treecore_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``treecore`` launch finishes the install
        ...
        # symbols in treecore_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `treecore` again "
        ...
        if not import_ok:
            ...
            print("    Run `treecore update` again — if it persists, reinstall:")
            print("    https://treecore-agent.nousresearch.com")

        node_failures = _update_node_dependencies()
        _m()._build_web_ui(_m().PROJECT_ROOT / "web")

        # Rebuild the desktop app if the source tree changed since the last
        # build.  ``treecore desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``treecore desktop`` shouldn't be forced into a full
        # Electron build by ``treecore update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `treecore desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "treecore_cli.main", "desktop", "--build-only"]
                ...
                # when `treecore update` runs inside the desktop updater chain
                # (Desktop → treecore-setup → treecore update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from treecore_constants import with_treecore_node_path

                _build_env = with_treecore_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `treecore desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_treecore_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.treecore/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit TREECORE_HOME so
        # it is not affected by sync_skills()'s module-level TREECORE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's TREECORE_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from treecore_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'treecore config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: treecore config migrate")
            else:
                print()
                print("Skipped. Run 'treecore config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from treecore_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `treecore curator status`. Self-stamps after printing so it
        ...
        # Self-heal the treecore-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Hermes on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("treecore-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``treecore update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `treecore update`
                # must stay fast; `treecore computer-use install --upgrade`
        ...
        # When running as ``treecore update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_treecore_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... treecore-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... treecore-gateway*)
            ...
            try:
                from treecore_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all treecore-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: treecore gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `treecore update` restart it automatically, allow\n"
                        ...
                        # path in `treecore gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: treecore gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: treecore -p <profile> gateway run  for each profile)"
                        )
        ...
        # When both treecore.service (from a pre-rename install) and the
        # current treecore-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `treecore update` surfaces the issue until the user migrates.
        try:
            from treecore_cli.gateway import (
                has_legacy_treecore_units,
                _find_legacy_treecore_units,
                supports_systemd_services,
            )
            ...
            if supports_systemd_services() and has_legacy_treecore_units():
                ...
                for name, path, is_sys in _find_legacy_treecore_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (treecore.service) fight the current")
                print("  treecore-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    treecore gateway migrate-legacy")
                print()
        ...
        print("  treecore model              # Select provider and model")
