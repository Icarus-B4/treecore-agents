...
Add, remove, or reorder entries here — both `treecore setup` and
`treecore` provider-selection will pick up the change automatically.
"""
...
from treecore_cli.urllib_security import open_credentialed_url

logger = logging.getLogger(__name__)

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_TREECORE_USER_AGENT = f"treecore-cli/{_TREECORE_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `treecore model`
    flow without maintaining a separate static list.
    """
    from treecore_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $TREECORE_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $TREECORE_HOME/models_dev_cache.json directly (no network) so this
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see treecore_cli/model_setup_flows.py).
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_treecore_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$TREECORE_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  treecore model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `treecore model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `treecore model` TUI
...
# slug as a top-level row in the interactive `treecore model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``treecore model``, the
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from treecore_cli.model_catalog import get_default_model_from_cache
...
# ``treecore model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in treecore_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``treecore auth add openai-codex`` without
    ``treecore model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``treecore model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    treecore-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from treecore_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — treecore-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from treecore_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return treecore-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; treecore's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``treecore model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from treecore_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:treecore-3                           →  ("nous", "treecore-3")
...
def _resolve_static_model_alias(
    ...
    try:
        from treecore_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``treecore_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``treecore auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.treecore/.env``.
    ...
    """
    try:
        from treecore_cli.auth import resolve_api_key_provider_credentials
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from treecore_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from treecore_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``treecore model`` shows.
            from treecore_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $TREECORE_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from treecore_constants import get_treecore_home
    return get_treecore_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$TREECORE_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `treecore config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _TREECORE_USER_AGENT}
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _TREECORE_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"treecore-agent/{_TREECORE_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=treecore"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from treecore_constants import get_treecore_home
    return get_treecore_home() / "ollama_cloud_models_cache.json"
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `treecore moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
