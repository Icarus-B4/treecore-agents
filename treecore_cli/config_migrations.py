...
``treecore_cli.config.migrate_config``. Each step is a function
...
``treecore_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``treecore_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``treecore_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``treecore_cli.config`` (e.g.
``patch("treecore_cli.config.read_raw_config", ...)``) keep working, because
...
#: TREECORE_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_treecore_home()}/config.yaml and run `treecore setup` to "
...
def _cfg():
    """Return the live ``treecore_cli.config`` module (lazy, cycle-free)."""
    from treecore_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_treecore_home = _c.get_treecore_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$TREECORE_HOME/plugins/`` for currently installed user plugins.
        grandfathered: List[str] = []
        try:
            user_plugins_dir = get_treecore_home() / "plugins"
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `treecore plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `treecore curator status` has no
    ...
    #   3. Creates `~/.treecore/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_treecore_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_treecore_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `treecore config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `treecore config set`"
                )
...
# behavior set it to true explicitly via `treecore config set`.
