...
# load_treecore_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_treecore_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_treecore_dotenv().
# Used by setup / `treecore model` flows to label detected credentials so
...
# TREECORE_HOME paths we've already pulled external secrets for during this
# process.  ``load_treecore_dotenv()`` is called at module-import time from
# several hot modules (cli.py, treecore_cli/main.py, run_agent.py,
...
def _known_treecore_env_keys() -> set[str]:
    ...
    ``load_treecore_dotenv()`` calls.
    """
    from treecore_cli.config import _EXTRA_ENV_KEYS
    from treecore_cli.config_defaults import OPTIONAL_ENV_VARS
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    ...
    ``tests/treecore_cli/test_dump_env_visibility.py``), and a startup scrub
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_treecore_dotenv()`` call.  Returns
...
def get_secret_source_values(
    treecore_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``treecore_home``."""
    home_key = str(Path(treecore_home).resolve())
...
def hydrate_profile_secret_sources(
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(treecore_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_treecore_dotenv()'s .op.env bootstrap: the 1Password
        ...
        local_env["TREECORE_HOME"] = str(home)
...
def reset_secret_source_cache() -> None:
    """Forget which TREECORE_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `treecore setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``treecore_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from treecore_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_treecore_dotenv(
    *,
    treecore_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Hermes environment files with user config taking precedence.

    Behavior:
    - `~/.treecore/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(treecore_home or os.getenv("TREECORE_HOME", Path.home() / ".treecore"))
    ...
    #   EnvironmentFile=-/path/to/.treecore/.op.env
    ...
    # TERMINAL_ENV=docker left in ~/.treecore/.env (e.g. written by an older
    # `treecore setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_treecore_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``treecore_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process TREECORE_HOME: the shared bridge reads the
    process-global config, so re-applying it for a *different* profile's
    ``load_treecore_dotenv(treecore_home=...)`` call would bridge the wrong
    ...
    """
    try:
        if Path(home_path).resolve() != _process_treecore_home().resolve():
            return
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of TREECORE_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    owns the once-per-TREECORE_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_treecore_dotenv()`` runs at import
    time from several hot modules (cli.py, treecore_cli/main.py,
    ...
    if not cfg:
        ...
        # on its next load_treecore_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_treecore_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `treecore model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `treecore` startup, so populating the
    # shared cache here lets main.py's early bridge and treecore_logging reuse
    ...
    if home_path == _process_treecore_home():
        try:
...
def _process_treecore_home() -> Path:
    """The TREECORE_HOME the shared config cache is keyed to."""
    try:
        from treecore_constants import get_treecore_home

        return get_treecore_home()
    except Exception:
