...
from treecore_cli.config import get_treecore_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.treecore/.env.
    ...
    exports. ``treecore debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.treecore_build_sha`` by the Dockerfile's
    ``TREECORE_GIT_SHA`` build-arg (see ``treecore_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``treecore dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from treecore_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(treecore_home: Path) -> int:
...
def _cron_summary(treecore_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = treecore_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"treecore_home:      {display_treecore_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.treecore/.env: a managed
        ...
        # A credential added via `treecore auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `treecore auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["treecore-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(treecore_home)}")
    lines.append(f"  skills:             {_count_skills(treecore_home)}")
