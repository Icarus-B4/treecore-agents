...
``treecore tools`` can prompt the user to install it.
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    """
    # Imported lazily so plugin import stays light for ``treecore tools`` probes.
    ...
    if _test_hook:
        env["TREECORE_DDGS_ALLOW_TEST_HOOKS"] = "1"
