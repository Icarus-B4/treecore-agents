...
```bash
treecore memory setup    # select "holographic"
```

Or manually:
```bash
treecore config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.treecore-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$TREECORE_HOME/memory_store.db` | SQLite database path |
