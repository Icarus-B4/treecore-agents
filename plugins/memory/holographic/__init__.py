"""treecore-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $TREECORE_HOME/config.yaml (profile-scoped):
  plugins:
    treecore-memory-store:
      db_path: $TREECORE_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "treecore-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, treecore_home):
        """Write config to config.yaml under plugins.treecore-memory-store."""
        from pathlib import Path
        config_path = Path(treecore_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["treecore-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_treecore_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _treecore_home = str(get_treecore_home())
        _default_db = _treecore_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $TREECORE_HOME in user-supplied paths so config values like
        # "$TREECORE_HOME/memory_store.db" or "~/.treecore/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$TREECORE_HOME", _treecore_home)
            db_path = db_path.replace("${TREECORE_HOME}", _treecore_home)
