...
Config: Uses the existing Honcho config chain:
  1. $TREECORE_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, treecore_home):
        """Write config to $TREECORE_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(treecore_home) / "honcho.json"
        existing = {}
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "treecore-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_treecore_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "treecore-default",
