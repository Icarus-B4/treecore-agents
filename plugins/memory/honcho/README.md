...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/treecore>
...
```bash
treecore memory setup honcho   # configure Honcho directly (works on a fresh install)
treecore memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
treecore config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.treecore/.env
```

> `treecore honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `treecore memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `treecore model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$TREECORE_HOME/honcho.json` | Profile-local (isolated Hermes instances) |
| 2 | `~/.treecore/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Hermes profile: `treecore` (default) or `treecore_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `treecore honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 5 | `per-repo` strategy | Git root directory name (`treecore-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`treecore`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-treecore-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening treecore in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Hermes session ID (timestamp + hex). Every `treecore` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
...
Both profiles see the same user (`yourname`) in the same shared environment (`treecore`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Hermes profile: `treecore` (default) or `treecore_<profile>` (e.g. `treecore -p coder` -> host key `treecore_coder`). Older `treecore.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `TREECORE_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `treecore-agent`) |
...
|---------|-------------|
| `treecore memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `treecore honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `treecore memory setup`) |
| `treecore honcho status` | Show resolved config for active profile |
| `treecore honcho enable` / `disable` | Toggle Honcho for active profile |
| `treecore honcho mode <mode>` | Change recall or observation mode |
| `treecore honcho peer --user <name>` | Update user peer name |
| `treecore honcho peer --ai <name>` | Update AI peer name |
| `treecore honcho tokens --context <N>` | Set context token budget |
| `treecore honcho tokens --dialectic <N>` | Set dialectic max chars |
| `treecore honcho map <name>` | Map current directory to a session name |
| `treecore honcho sync` | Create host blocks for all Hermes profiles |
