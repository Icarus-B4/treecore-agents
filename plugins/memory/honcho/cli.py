...
Handles: treecore honcho setup | status | sessions | map | peer
"""
...
from treecore_cli.config import cfg_get
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "treecore" else ""
...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'treecore honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'treecore honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `treecore update` -- no output, no exceptions.
    """
    try:
        from treecore_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _local_config_path() -> Path:
    ...
    Always returns $TREECORE_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_treecore_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        treecore_host.get("pinUserPeer"),
        treecore_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in treecore_host:
        aliases_src = treecore_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in treecore_host:
        prefix_src = treecore_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(treecore_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom treecore_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    treecore_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        treecore_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(treecore_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = treecore_host.get("userPeerAliases")
    prefix = treecore_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(treecore_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(treecore_host)
    treecore_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        treecore_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        treecore_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/treecore (NS-605).
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = treecore_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            treecore_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(treecore_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'treecore honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'treecore honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'treecore honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(treecore_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'treecore honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = treecore_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        treecore_host["peerName"] = new_peer

    current_ai = treecore_host.get("aiPeer") or cfg.get("aiPeer", "treecore")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        treecore_host["aiPeer"] = new_ai

    current_workspace = treecore_host.get("workspace") or cfg.get("workspace", "treecore")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        treecore_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, treecore_host)
    ...
    if run_mapping:
        peer_target = treecore_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(treecore_host)
            treecore_host["pinUserPeer"] = False
            if prior_aliases:
                treecore_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                treecore_host, current_prefix, prefix_from_root,
            ...
            if merged:
                treecore_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                treecore_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                treecore_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = treecore_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        treecore_host["observationMode"] = new_obs
    else:
        treecore_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(treecore_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        treecore_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        treecore_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = treecore_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        treecore_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = treecore_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        treecore_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                treecore_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(treecore_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            treecore_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        treecore_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        treecore_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        treecore_host["dialecticReasoningLevel"] = new_reasoning
    else:
        treecore_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = treecore_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        treecore_host["sessionStrategy"] = new_strat

    treecore_host["enabled"] = True
    treecore_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        treecore_config = load_config()
        treecore_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(treecore_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: treecore config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    treecore honcho status     -- show full config")
    print("    treecore honcho mode       -- change recall/observation mode")
    print("    treecore honcho tokens     -- tune context and dialectic budgets")
    print("    treecore honcho peer       -- update peer names")
    print("    treecore honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if treecore_cli internals move."""
    try:
        from treecore_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(treecore_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    treecore_host["apiKey"] = cred.access_token
    treecore_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        treecore_host["peerName"] = cred.consent_peer_name
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: treecore honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'treecore honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'treecore honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: treecore honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        treecore = hosts.get(_host_key(), {})
        user = treecore.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = treecore.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = treecore.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = treecore.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Hermes' identity in Honcho. Seed with 'treecore honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: treecore honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: treecore honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = treecore.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = treecore.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = treecore.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("\n  Set with: treecore honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'treecore honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'treecore honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    treecore honcho identity --show        — show both peer representations")
        print("    treecore honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  treecore honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'treecore honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'treecore honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  treecore honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'treecore'")
        ...
        for f in user_files:
            print("    treecore honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'treecore honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'treecore honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    treecore honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  treecore honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  treecore honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. treecore honcho setup              — configure API key (required)")
        print("  2. treecore honcho migrate            — re-run this walkthrough")
    else:
        print("  1. treecore honcho status             — verify Honcho connection")
        print("  2. treecore                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. treecore honcho identity --show    — verify AI peer representation")
        print("  4. treecore honcho tokens             — tune context and dialectic budgets")
        print("  5. treecore honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'treecore memory setup'...\n")
        from treecore_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``treecore honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``treecore honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to treecore memory setup)",
    )
