...
Resolution order for config file:
  1. $TREECORE_HOME/honcho.json  (instance-local, enables isolated Hermes instances)
...
from treecore_cli.profiles import _get_default_treecore_home
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      1. TREECORE_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``treecore_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"treecore"`` (default profile)
    """
    explicit = os.environ.get("TREECORE_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $TREECORE_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.treecore/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_treecore_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_treecore_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.treecore
    ...
    def from_global_config(
        ...
        Resolution: $TREECORE_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        ...
        # A hosts.treecore block or explicit enabled flag means the user
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'treecore honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `treecore honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                treecore_cfg = load_config()
                honcho_cfg = treecore_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
