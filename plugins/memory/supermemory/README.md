...
- Hosted: API key from [app.supermemory.ai/integrations?connect=treecore](http://app.supermemory.ai/integrations?connect=treecore)
...
```bash
treecore memory setup    # select "supermemory"
```

Or manually:

```bash
treecore config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.treecore/.env
```
...
Before running `treecore memory setup`, add the local endpoint to
`$TREECORE_HOME/supermemory.json`:
...
Then run `treecore memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$TREECORE_HOME/supermemory.json`
...
| `container_tag` | `treecore` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `treecore-{identity}` → `treecore-coder`). |
...
All Supermemory API calls send `x-sm-source: treecore`, and document writes stamp
`metadata.sm_source: treecore`. This is a **functional routing key, not telemetry**:
...
```json
{
  "container_tag": "treecore-{identity}"
}
```

For a profile named `coder`, this resolves to `treecore-coder`. The default profile resolves to `treecore-default`. Without `{identity}`, all profiles share the same container.
