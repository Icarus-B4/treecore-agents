...
_DEFAULT_CONTAINER_TAG = "treecore"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=treecore"
...
def _load_supermemory_config(treecore_home: str) -> dict:
    config = _default_config()
    config_path = Path(treecore_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, treecore_home: str) -> None:
    config_path = Path(treecore_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "treecore"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "treecore", **(metadata or {})}
...
def _resolve_container_tag_for_setup(treecore_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(treecore_home)
...
def _probe_supermemory_connection(api_key: str, treecore_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(treecore_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(treecore_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `treecore memory setup`.
        # All other options are documented for $TREECORE_HOME/supermemory.json
    ...
    def save_config(self, values, treecore_home):
        ...
        _save_supermemory_config(sanitized, treecore_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, treecore_home)
    ...
    def post_setup(self, treecore_home: str, config: dict) -> None:
        ...
        from treecore_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(treecore_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from treecore_constants import get_treecore_home
        self._treecore_home = kwargs.get("treecore_home") or str(get_treecore_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._treecore_home)
