...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # treecore_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(treecore_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(treecore_home) / "mem0.json"
    existing = {}
...
def _setup_platform(treecore_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "treecore-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "treecore"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.treecore/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.treecore/.env (or unset it) or Hermes will keep "
    ...
    provider.save_config(provider_config, treecore_home)

    if env_writes:
        _write_env(Path(treecore_home) / ".env", env_writes)
...
def _setup_selfhosted(treecore_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "treecore-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "treecore")
...
def _setup_oss(treecore_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(treecore_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "treecore-user")
    ...
    if env_writes:
        _write_env(Path(treecore_home) / ".env", env_writes)
    _save_mem0_json(treecore_home, {"mode": "oss", "user_id": user_id, "agent_id": "treecore", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, treecore_home: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    if not existing:
        env_path = Path(treecore_home) / ".env"
        if env_path.exists():
            # BOM-tolerant read matching the canonical .env readers in
            # treecore_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "treecore-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "treecore"
...
def _setup_oss_interactive(treecore_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], treecore_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], treecore_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'treecore-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "treecore-user")

    agent_id = input("  Agent ID [treecore]: ").strip()
    agent_id = agent_id or "treecore"
    ...
    if env_writes:
        _write_env(Path(treecore_home) / ".env", env_writes)
    _save_mem0_json(treecore_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def _install_provider_deps(llm_id: str, embedder_id: str, vector_id: str) -> None:
    ...
    for dep in sorted(deps):
        try:
            ...
            # durable data-volume target instead of /opt/treecore (NS-605).
...
def post_setup(treecore_home: str, config: dict) -> None:
    """Entry point called by treecore memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(treecore_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(treecore_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(treecore_home, config, flags)
        return
