...
Configuration
-------------
Secret (lives in $TREECORE_HOME/.env or the environment):
...
Behavioral settings (live in $TREECORE_HOME/mem0.json, set via `treecore memory
setup`):
  ...
  agent_id           — Agent identifier (default: treecore)
...
_DEFAULT_USER_ID = "treecore-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $TREECORE_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "treecore"),
        "oss": {},
    }
    ...
    config_path = get_treecore_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, treecore_home):
        """Write config to $TREECORE_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, treecore_home: str, config: dict) -> None:
        ...
        post_setup(treecore_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $TREECORE_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "treecore")
