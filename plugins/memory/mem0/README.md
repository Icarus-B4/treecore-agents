...
```bash
treecore memory setup    # select "mem0"
```

Or manually:
```bash
treecore config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.treecore/.env
```

## Config

Behavioral settings live in `$TREECORE_HOME/mem0.json` (set them via `treecore memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.treecore/.env`.
...
| `user_id` | `treecore-user` | User identifier on Mem0 |
| `agent_id` | `treecore` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   treecore memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   treecore memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.treecore/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.treecore/.env
   ```
   or in `$TREECORE_HOME/mem0.json`:
   ```json
   {
...
```bash
treecore memory setup mem0 --mode oss \
...
```bash
treecore memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$TREECORE_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.treecore/mem0_qdrant"}}
  }
}
```
...
```bash
treecore memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
treecore memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$TREECORE_HOME/mem0.json`).
