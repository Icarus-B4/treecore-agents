...
```bash
treecore memory setup    # select "openviking"
```
...
```bash
treecore config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.treecore/.env`; for a named profile use
`~/.treecore/profiles/<profile>/.env`.
...
| `OPENVIKING_AGENT` | `treecore` | Hermes peer ID in OpenViking, used for peer-scoped memories |
...
file URIs, such as
`viking://user/peers/treecore/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/treecore/memories/preferences/mem_abc123.md`. Files
