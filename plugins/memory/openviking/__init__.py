...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Hermes peer ID in OpenViking (default: treecore)
...
_SYNC_TRACE_ENV = "TREECORE_OPENVIKING_SYNC_TRACE"
...
def _load_treecore_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("TREECORE_HOME", "")).expanduser() if os.environ.get("TREECORE_HOME") else Path.home() / ".treecore"
...
def _save_treecore_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See treecore-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_treecore_openviking_config()
    ...
    def save_config(self, values: Dict[str, Any], treecore_home: str) -> None:
    ...
    def post_setup(self, treecore_home: str, config: dict) -> None:
        ...
        from treecore_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        treecore_home_path = Path(treecore_home)
        env_path = treecore_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_treecore_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        treecore_home = str(kwargs.get("treecore_home") or "").strip()
        if not treecore_home:
            try:
                from treecore_constants import get_treecore_home
                treecore_home = str(get_treecore_home())
            except Exception:
                treecore_home = str(Path.home() / ".treecore")
        self._treecore_home = treecore_home
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.treecore/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts treecore (#21130).
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._treecore_home:
            return None
        return Path(self._treecore_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._treecore_home:
            return None
        return Path(self._treecore_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_treecore_openviking_config()
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See treecore-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/treecore/.overview.md).
