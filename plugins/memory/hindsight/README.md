...
```bash
treecore memory setup    # select "hindsight"
```
...
```bash
treecore config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.treecore/.env
```
...
Daemon startup logs: `~/.treecore/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p treecore ui start
```
...
Config file: `~/.treecore/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `treecore` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `treecore-{profile}` isolates memory per active Hermes profile. Empty placeholders collapse cleanly (e.g. `treecore-{user}` with no user becomes `treecore`). |
...
The LLM API key is stored in `~/.treecore/.env` as `HINDSIGHT_LLM_API_KEY`.
