...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: treecore)
...
Or via $TREECORE_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    healthy and ``treecore memory status`` would stay green while the daemon
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $TREECORE_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_treecore_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "treecore": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "treecore"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "treecore")
    return str(profile or "treecore")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``treecore-{user}`` with no user becomes ``treecore``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, treecore_home):
        """Write config to $TREECORE_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(treecore_home) / "hindsight"
    ...
    def post_setup(self, treecore_home: str, config: dict) -> None:
        ...
        from treecore_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "treecore")
        ...
        self.save_config(provider_config, treecore_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(treecore_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(treecore_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "treecore"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: treecore-{profile}", "default": ""},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "treecore"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "treecore"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "treecore", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "treecore")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "to cloud / local_external mode via 'treecore memory setup'."
                )
                logger.warning(msg)
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See treecore-agent#6672.
