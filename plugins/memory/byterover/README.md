...
```bash
treecore memory setup    # select "byterover"
```

Or manually:
```bash
treecore config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.treecore/.env
```
...
Working directory: `$TREECORE_HOME/byterover/` (profile-scoped).
