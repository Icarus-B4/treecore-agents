...
def _xai_user_agent() -> str:
    try:
        ...
        return treecore_xai_user_agent()
    except Exception:
        return "treecore-agent/video_gen"
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``treecore_cli/tools_config.py``) so the picker doesn't blindly
...
def _auth_required_response(prompt: str) -> Dict[str, Any]:
    return error_response(
        error=(
            "No xAI credentials found. Sign in via `treecore auth add xai-oauth` "
    ...
    *,
    ...
    try:
        ...
        storage_options = build_xai_storage_options(
            "video_gen",
            filename_prefix="treecore-xai-video",
            extension="mp4",
        )
