...
Scoped strictly to `$TREECORE_HOME` and `/tmp/treecore-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `TREECORE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under TREECORE_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `TREECORE_HOME` or `/tmp/treecore-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$TREECORE_HOME/disk-cleanup/` is itself excluded
- `$TREECORE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
