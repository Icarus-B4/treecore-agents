...
Rules:
  ...
  - empty dirs    → always delete (under TREECORE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly TREECORE_HOME and /tmp/treecore-*
Never touches: ~/.treecore/logs/ or any system directory.
"""
...
try:
    ...
    def get_treecore_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("TREECORE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".treecore").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$TREECORE_HOME/logs/``."""
    return get_treecore_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$TREECORE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under TREECORE_HOME or ``/tmp/treecore-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    treecore_home = get_treecore_home()
    try:
        path.resolve().relative_to(treecore_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/treecore-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("treecore-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "treecore-agent", "backups", "profiles", ".worktrees",
...
# Paths under $TREECORE_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so TREECORE_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        treecore_home = get_treecore_home()
        for parent in ("cron", "cronjobs"):
            base = treecore_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside TREECORE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under TREECORE_HOME, but never recurse into known
    # durable state trees.  Some installs place the Hermes checkout, venv,
    # and desktop build under TREECORE_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    treecore_home = get_treecore_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in treecore_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    treecore_home = get_treecore_home()
    try:
        rel = path.resolve().relative_to(treecore_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "treecore-agent",
        ...
        # Path isn't under TREECORE_HOME (e.g. /tmp/treecore-*) — fall through.
        pass
