...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    ``<kind>_cache`` layout that ``get_treecore_dir`` may return.
    """
    try:
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_treecore_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `treecore gateway`.",
                retryable=False,
            )
            return False
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Pair from the dashboard or run `treecore whatsapp`; remove "
                "WHATSAPP_ENABLED from your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — pair from the dashboard or run `treecore whatsapp`.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `treecore update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".treecore-pkg-hash"
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `treecore update`, so without this check it
            ...
            bridge_env = with_treecore_node_path()
            ...
            # hardcodes ~/.treecore/{image,audio,document}_cache, which diverges
            # under TREECORE_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["TREECORE_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["TREECORE_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["TREECORE_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: treecore whatsapp")
...
# dict in treecore_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in treecore_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``treecore setup`` even
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via treecore_cli.gateway.get_env_value (not os.getenv) so setup-status
