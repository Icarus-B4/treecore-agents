"""
``treecore photon ...`` CLI subcommands — registered by the plugin via
...
# Writable sidecar runtime dir (mirrors to TREECORE_HOME on immutable
...
# wiring imports this module for `treecore --help`).
...
# when called later (gateway start, treecore status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `treecore photon ...` subcommands."""
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``treecore photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.treecore/.env,
    ...
    try:
        from treecore_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  treecore gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `treecore photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`treecore photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.treecore/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `treecore photon telemetry on` / `treecore photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.treecore/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  treecore gateway restart")
    return 0
...
#
# `treecore gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``treecore photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `treecore gateway setup` wizard."""
