...
#
# Out-of-process senders (cron subprocesses, `treecore send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_treecore_home() honors profile overrides — never hardcode ~/.treecore.
    from treecore_constants import get_treecore_home

    return get_treecore_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (treecore status, test collection, plugin discovery).
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `treecore photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `treecore photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `treecore setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, treecore status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: treecore photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: treecore photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `treecore update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``treecore photon install-sidecar``: ``npm ci`` for an exact,
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `treecore photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                raise RuntimeError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `treecore photon setup`)"
                )
        # A `treecore update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `treecore send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`treecore send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "records its address under <treecore-home>/runtime/"
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `treecore photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: treecore photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `treecore gateway setup` alongside every other
    ...
    # Register CLI subcommands — `treecore photon ...`
