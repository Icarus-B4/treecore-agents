...
treecore photon setup --phone +15551234567

# Start the gateway
treecore gateway start
```

`treecore photon setup` does, in order:
...
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.treecore/.env` so the
...
Run `treecore photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.treecore/.env` (the same place every other
...
Management metadata lives in `~/.treecore/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `treecore photon telemetry on\|off` (restart the gateway to apply) |
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `treecore send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<treecore-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `treecore photon status`, a DM and a group roundtrip,
