...
under an immutable ``/opt/treecore`` (read-only for the treecore user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$TREECORE_HOME/photon/sidecar`` (the durable data volume,
...
def dir_writable(path: Path) -> bool:
    ...
    resolver does.
    """
    probe = path / ".treecore-write-probe"
    try:
        probe.touch()
        probe.unlink()
        return True
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_treecore_home() / "photon" / "sidecar"
    try:
