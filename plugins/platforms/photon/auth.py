...
Credential storage mirrors every other Hermes channel:
    ...
    * runtime SDK creds  -> ``~/.treecore/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.treecore/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a treecore dependency
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of treecore-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.treecore/auth.json`` honouring the active Hermes profile."""
    try:
        from treecore_constants import get_treecore_home
        return Path(get_treecore_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.treecore")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # treecore_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from treecore_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.treecore/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.treecore/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.treecore/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: treecore_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `treecore photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `treecore photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `treecore photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `treecore photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `treecore photon setup`)"
