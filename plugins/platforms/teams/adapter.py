...
Requires:
    ...
    manually: ``<treecore-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        treecore_action = data.get("treecore_action", "")
        session_key = data.get("session_key", "")

        if not treecore_action or not session_key:
        ...
        choice = choice_map.get(treecore_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="treecore_approve",
            data={**btn_data_base, "treecore_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="treecore_approve",
                data={**btn_data_base, "treecore_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="treecore_approve",
                    data={**btn_data_base, "treecore_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="treecore_approve",
            data={**btn_data_base, "treecore_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print()
    print_success("Teams configuration saved to ~/.treecore/.env")
