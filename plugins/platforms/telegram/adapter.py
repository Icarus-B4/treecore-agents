...
# trigger its own recovery path. Refs: NousResearch/treecore-agent#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: NousResearch/treecore-agent#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("TREECORE_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "TREECORE_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "TREECORE_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
        ...
        try:
            if app and app.updater and app.updater.running:
                try:
                    ...
                    # Refs: NousResearch/treecore-agent#58270
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `treecore update` or `--replace` handoffs)
        ...
        message = (
            ...
            "with this token, then restart the gateway with 'treecore gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                from treecore_cli.commands import telegram_menu_commands, telegram_menu_max_commands
                if not self._bot:
                    return
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("TREECORE_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("TREECORE_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("TREECORE_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("TREECORE_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("TREECORE_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            disable_fallback = (os.getenv("TREECORE_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            _max_connect = 8
            _init_timeout = _env_float("TREECORE_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set TREECORE_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                            f"TREECORE_TELEGRAM_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set TREECORE_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``treecore model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from treecore_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from treecore_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.treecore/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".treecore" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@treecore_bot.example" are not mentions
...
# treecore_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("TREECORE_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in treecore_cli/gateway.py.
    """
    from treecore_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `treecore setup` to install Telegram support.",
