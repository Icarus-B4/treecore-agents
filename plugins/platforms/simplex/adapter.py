...
Optional environment variables:
    ...
    TREECORE_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``treecore setup`` can describe it even when websockets is
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("TREECORE_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``treecore setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.treecore/.env`` via
    ``treecore_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "treecore_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.treecore/.env"
        )
        return
