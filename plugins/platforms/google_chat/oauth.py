...
- Per-user tokens (keyed by sender email):
    ``${TREECORE_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${TREECORE_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${TREECORE_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${TREECORE_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${TREECORE_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's TREECORE_HOME helper so the token follows the user's
# profile (e.g. tests can override via TREECORE_HOME=/tmp/...).
try:
    from treecore_constants import display_treecore_home, get_treecore_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where treecore_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _treecore_home.py shim).
    def get_treecore_home() -> Path:
        val = os.environ.get("TREECORE_HOME", "").strip()
        return Path(val) if val else Path.home() / ".treecore"
...
def _treecore_home() -> Path:
    """Resolve TREECORE_HOME at call time (NOT module import).

    Tests and ``TREECORE_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.treecore/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _treecore_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _treecore_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _treecore_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _treecore_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _treecore_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `treecore setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `treecore setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into TREECORE_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_treecore_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_treecore_home()}/google_chat_user_token.json"
    )
