...
# ``model_tools`` import time, so ``treecore status``, ``treecore chat``, etc.
...
# ``treecore_plugins.platforms__google_chat.adapter`` once the plugin
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a treecore MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "treecore-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${TREECORE_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _treecore_home = _get_treecore_home()
        except (ModuleNotFoundError, ImportError):
            _treecore_home = _Path.home() / ".treecore"
        self._thread_count_store = _ThreadCountStore(
            _treecore_home / "google_chat_thread_counts.json"
        )
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("TREECORE_HOME", str(_Path.home() / ".treecore"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${TREECORE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a treecore MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``treecore setup``.

    The setup wizard at ``treecore_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.treecore/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. treecore-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.treecore/.env")
    print_info("Restart the gateway: treecore gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``treecore cron`` running as a
    separate process from ``treecore gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `treecore setup` to install Google Chat support.",
