...
# Uses get_treecore_home() so each profile gets its own Matrix store.
from treecore_constants import get_treecore_dir as _get_treecore_dir

_STORE_DIR = _get_treecore_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("TREECORE_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("TREECORE_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "treecore"
...
# + _PLATFORMS["matrix"] static dict in treecore_cli/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        txn_id = f"treecore_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces treecore_cli/setup.py::_setup_matrix
...
def _is_connected(config) -> bool:
    ...
    configured. Read via treecore_cli.gateway.get_env_value so setup-status
