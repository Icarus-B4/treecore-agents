...
def _default_agent_name() -> str:
    ...
    try:
        import socket
        return f"treecore-{socket.gethostname()}"
    except Exception:
        return "treecore-agent"
...
def _active_profile_name() -> str:
    try:
        ...
        return os.getenv("TREECORE_PROFILE", "default") or "default"
    ...
    try:
        ...
        if not profile or profile == "default":
            try:
                ...
                return None
        return os.path.expanduser(f"~/.treecore/profiles/{profile}")
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        ...
        with lock:
            ...
            cmd = ["treecore", "chat", "-q", framed_text, "-Q", "--source", "a2a"]
            if session_id:
            ...
            home = _profile_home(profile)
            if home:
                env["TREECORE_HOME"] = home
            env["TREECORE_A2A_PEER"] = peer
            start = time.time()
            try:
                proc = subprocess.run(
