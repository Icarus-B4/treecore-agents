...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("TREECORE_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid TREECORE_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.treecore/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("TREECORE_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("TREECORE_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="TREECORE_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="TREECORE_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_treecore_home())
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``TREECORE_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("TREECORE_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_treecore_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        ...
        subject = r"(?:treecore|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.treecore/skills/ for new or removed skills")
        ...
        # treecore_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from treecore_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from treecore_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from treecore_cli.commands import discord_skill_commands_by_category
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_treecore_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``treecore update --gateway``
        ...
        *,
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_treecore_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``treecore pairing approve``).
    ...
    # so users approved via ``treecore pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``treecore update`` prompts.
...
# is not in this process (e.g. ``treecore cron`` running standalone) and no live
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "TREECORE_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "TREECORE_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``treecore_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `treecore setup` to install Discord support.",
        # Interactive setup wizard — replaces the central
        # treecore_cli/setup.py::_setup_discord function.  Same shape as Teams.
