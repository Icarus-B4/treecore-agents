...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "treecore-bot")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: treecore_, treecore_1, treecore_2...
...
def check_requirements() -> bool:
    ...
    # treecore setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `treecore gateway setup` flow for the IRC platform.

    Lazy-imports ``treecore_cli.setup`` helpers so the plugin stays importable
    ...
    channel = prompt(
        "Channel to join (e.g. #treecore — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.treecore/.env")
    print_info("Restart the gateway for changes to take effect: treecore gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "treecore-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "treecore-bot"
