...
try:
    ...
    _TREECORE_VERSION = "unknown"
_TREECORE_SLACK_USER_AGENT_PREFIX = f"HermesAgent/{_TREECORE_VERSION}"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from treecore_cli.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `treecore slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `treecore gateway setup` "
                "or in the active profile's ~/.treecore/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `treecore gateway setup` "
                "or add it to your active profile's ~/.treecore/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `treecore gateway setup` "
                "or in the active profile's ~/.treecore/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `treecore gateway setup` "
                "or add it to your active profile's ~/.treecore/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_treecore_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_TREECORE_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # /model work directly without /treecore prefix). A single regex
            ...
            # manifest (see `treecore slack manifest`). In Socket Mode, Slack
            ...
            from treecore_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/treecore$")

            @self._app.command(_slash_pattern)
            async def handle_treecore_command(ack, command):
            ...
            for _action_id in (
                "treecore_approve_once",
                "treecore_approve_session",
                "treecore_approve_always",
                "treecore_deny",
            ):
            ...
            for _action_id in (
                "treecore_confirm_once",
                "treecore_confirm_always",
                "treecore_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("treecore_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^treecore_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("treecore_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'treecore slack manifest' if unsure), and (b) the other bot's "
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_treecore_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_treecore_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_treecore_no_thread_response"):
        ...
        force_process = bool(event.get("_treecore_force_process"))
        ...
        try:
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "treecore_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "treecore_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "treecore_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "treecore_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "treecore_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "treecore_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``treecore_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``treecore_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"treecore_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "treecore_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "treecore_confirm_once": "once",
            "treecore_confirm_always": "always",
            "treecore_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "treecore_approve_once": "once",
            "treecore_approve_session": "session",
            "treecore_approve_always": "always",
            "treecore_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "treecore_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/treecore <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/treecore
        ...
        if slash_name in {"treecore", ""}:
            # Legacy /treecore <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from treecore_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/treecore   ``).
        ...
        # "Running /cmd…" ack shown by handle_treecore_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/treecore <question>" must produce public replies so
...
# ``treecore_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_treecore_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``treecore_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from treecore_cli.config import get_env_value, remove_env_value, save_env_value
    from treecore_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under TREECORE_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from treecore_cli.slack_cli import _build_full_manifest
            from treecore_constants import get_treecore_home
            import json as _json
            ...
            target = Path(get_treecore_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `treecore slack manifest --write` anytime to refresh after "
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `treecore update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://treecore-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``treecore_cli.gateway.get_env_value`` at call time (not via the
    ...
    """
    import treecore_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `treecore setup` to install Slack support.",
        # Interactive setup wizard — replaces treecore_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in treecore_cli/gateway.py.
