...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``TREECORE_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            treecore_home = Path(get_treecore_home()).resolve()
        except Exception:
            treecore_home = Path.home().joinpath(".treecore").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``treecore status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``treecore status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``treecore setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.treecore/.env`` via ``treecore_cli.config``.
    """
    print()
    ...
    try:
        from treecore_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("treecore_cli.config not available; set LINE_* vars manually in ~/.treecore/.env")
        return
