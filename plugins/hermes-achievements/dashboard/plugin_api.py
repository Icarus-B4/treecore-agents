...
Mounted at /api/plugins/treecore-achievements/ by Hermes dashboard.
"""
...
try:
    ...
    import os as _os
    def get_treecore_home() -> Path:  # type: ignore[misc]
        val = (_os.environ.get("TREECORE_HOME") or "").strip()
...
PORT_RE = re.compile(r"\b(port\s+)?(3000|5173|8000|8080|9119|9120)\b.*\b(in use|already|taken|eaddrinuse)\b|\beaddrinuse\b", re.I)
...
def state_path() -> Path:
    return get_treecore_home() / "plugins" / "treecore-achievements" / "state.json"


def snapshot_path() -> Path:
    return get_treecore_home() / "plugins" / "treecore-achievements" / "scan_snapshot.json"


def checkpoint_path() -> Path:
    return get_treecore_home() / "plugins" / "treecore-achievements" / "scan_checkpoint.json"
...
def analyze_messages(session_id: str, title: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugin_events": len(re.findall(r"plugin|dashboard-plugins|__TREECORE_PLUGIN|manifest\.json", full_text, re.I)),
...
def _start_background_scan() -> None:
    ...
    with _BACKGROUND_SCAN_LOCK:
        ...
        thread = threading.Thread(
            ...
            name="treecore-achievements-scan",
            daemon=True,
        )
