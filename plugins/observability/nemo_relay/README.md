...
```bash
treecore plugins enable observability/nemo_relay
```

The `TREECORE_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `TREECORE_HOME` that the
agent run will use:

```bash
env TREECORE_HOME=/tmp/treecore-nemo-relay-test \
  treecore plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`TREECORE_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`TREECORE_HOME` is the Hermes profile/config home used by both
`treecore plugins enable ...` and the later `treecore chat ...` run. If unset,
Hermes uses the user's default home, usually `~/.treecore`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export TREECORE_HOME=/tmp/treecore-nemo-relay-test
treecore plugins enable observability/nemo_relay
treecore chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `treecore` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run treecore plugins enable observability/nemo_relay
uv run treecore chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/treecore_agents-*.whl
treecore plugins enable observability/nemo_relay
```
...
The plugin can configure exporters directly from `TREECORE_NEMO_RELAY_*`
...
```bash
export TREECORE_NEMO_RELAY_ATOF_ENABLED=1
export TREECORE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export TREECORE_NEMO_RELAY_ATIF_ENABLED=1
export TREECORE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `TREECORE_NEMO_RELAY_ATOF_FILENAME`
- `TREECORE_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `TREECORE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `TREECORE_NEMO_RELAY_ATIF_AGENT_NAME`
- `TREECORE_NEMO_RELAY_ATIF_AGENT_VERSION`
- `TREECORE_NEMO_RELAY_ATIF_MODEL_NAME`
- `TREECORE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export TREECORE_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `TREECORE_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`TREECORE_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`TREECORE_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Hermes does not
...
```bash
export TREECORE_HOME=/tmp/treecore-nemo-relay-docs/treecore-home
mkdir -p "$TREECORE_HOME"

cat > "$TREECORE_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
```bash
export TREECORE_NEMO_RELAY_ATOF_ENABLED=1
export TREECORE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/treecore-nemo-relay-docs/subagent/atof
export TREECORE_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export TREECORE_NEMO_RELAY_ATOF_MODE=overwrite
export TREECORE_NEMO_RELAY_ATIF_ENABLED=1
export TREECORE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/treecore-nemo-relay-docs/subagent/atif
export TREECORE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export TREECORE_NEMO_RELAY_ATIF_AGENT_NAME='Hermes Agent E2E'
export TREECORE_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export TREECORE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"treecore.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```bash
mkdir -p /tmp/treecore-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/treecore-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/treecore-nemo-relay-docs/workdir/beta.txt
cd /tmp/treecore-nemo-relay-docs/workdir

export TREECORE_NEMO_RELAY_ATOF_ENABLED=1
export TREECORE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/treecore-nemo-relay-docs/parallel/atof
export TREECORE_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export TREECORE_NEMO_RELAY_ATOF_MODE=overwrite
export TREECORE_NEMO_RELAY_ATIF_ENABLED=1
export TREECORE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/treecore-nemo-relay-docs/parallel/atif
export TREECORE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export TREECORE_NEMO_RELAY_ATIF_AGENT_NAME='Hermes Agent E2E'
export TREECORE_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
```bash
export TREECORE_NEMO_RELAY_PLUGINS_TOML=/tmp/treecore-middleware-test/plugins.toml
```
...
```bash
export TREECORE_HOME=/tmp/treecore-middleware-test/treecore-home
mkdir -p "$TREECORE_HOME" /tmp/treecore-middleware-test/nemo-relay
...
cat > /tmp/treecore-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/treecore-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/treecore-middleware-test/atif"
...
export TREECORE_NEMO_RELAY_PLUGINS_TOML=/tmp/treecore-middleware-test/nemo-relay/plugins.toml
