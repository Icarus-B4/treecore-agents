...
_SESSION_INITIALIZER_NAME = "treecore.nemo_relay.rich_observability"
...
class _Settings:
    ...
    atof_filename: str = "treecore-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "treecore-atif-{session_id}.json"
...
class _Runtime:
    def __init__(
        ...
        self._atof_subscriber_name = f"treecore.nemo_relay.atof.{self.host.runtime_id}"
        self._execution_consumer_name = (
            f"treecore.nemo_relay.rich_observability.{self.host.runtime_id}"
        )
        ...
        with self._sessions_lock:
            ...
            if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
                state.atif_exporter = self.nemo_relay.AtifExporter(
                    ...
                    extra={
                        "source": "treecore-agent",
                        "plugin": "observability/nemo_relay",
                    },
                )
                state.atif_subscriber_name = (
                    f"treecore.nemo_relay.atif.{self.host.runtime_id}.{session_id}"
                )
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("treecore.subagent.stop", kwargs)
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("treecore.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("treecore.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("treecore.turn.end", kwargs))


def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("treecore.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("treecore.approval.response", kwargs))
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("TREECORE_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("TREECORE_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("TREECORE_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("TREECORE_NEMO_RELAY_ATOF_FILENAME") or "treecore-atof.jsonl",
        atof_mode=_env("TREECORE_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("TREECORE_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("TREECORE_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("TREECORE_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "treecore-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("TREECORE_NEMO_RELAY_ATIF_AGENT_NAME") or "Hermes Agent",
        atif_agent_version=_env("TREECORE_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("TREECORE_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _atif_subagent_export_mode() -> str:
    mode = _env("TREECORE_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="treecore-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()
