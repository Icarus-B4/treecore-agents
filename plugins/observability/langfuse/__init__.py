...
load when listed in ``plugins.enabled`` (via ``treecore plugins enable
observability/langfuse`` or ``treecore tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``treecore tools`` or ~/.treecore/.env):
  TREECORE_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  TREECORE_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  TREECORE_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  TREECORE_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  TREECORE_LANGFUSE_RELEASE     - release/version tag
  TREECORE_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  TREECORE_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  TREECORE_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("TREECORE_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("TREECORE_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("TREECORE_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("TREECORE_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("TREECORE_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset TREECORE_LANGFUSE_PUBLIC_KEY / TREECORE_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("TREECORE_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("TREECORE_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("TREECORE_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("TREECORE_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid TREECORE_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("TREECORE_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
