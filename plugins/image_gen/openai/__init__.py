...
three virtual model IDs so the ``treecore tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `treecore tools` → Image "
                    "Generation → OpenAI to configure, or `treecore setup` "
                    "to add the key."
                ),
