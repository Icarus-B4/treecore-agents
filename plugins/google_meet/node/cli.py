"""`treecore meet node ...` subcommand tree.

Wired into the existing ``treecore meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``treecore meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="treecore-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``treecore meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             treecore meet node approve <name> ws://<host>:{args.port} {token}")
        try:
