...
Let the treecore agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where treecore runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `treecore meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `treecore meet` CLI |
...
| `cli.py` | `treecore meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$TREECORE_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `treecore meet node {run,list,approve,remove,status,ping}` |
...
treecore meet install                                      # pip + Chromium
treecore meet setup                                        # preflight
treecore meet auth                                         # optional
treecore meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
treecore meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.treecore/.env
treecore meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
treecore meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
treecore meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.treecore/.env
treecore meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, treecore will **not** switch your system audio input automatically — the
...
treecore plugins enable google_meet
treecore meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
treecore meet node ping my-mac
