...
```bash
treecore plugins enable google_meet
treecore meet install                 # pip deps + Chromium (transcribe only)
treecore meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
treecore meet auth                    # optional; skips guest-lobby wait
treecore meet setup                   # preflight checks
```

`treecore meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or TREECORE_MEET_REALTIME_KEY in ~/.treecore/.env
```
...
pip install playwright websockets && python -m playwright install chromium
treecore plugins enable google_meet
treecore meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
treecore meet node approve my-mac ws://<mac-ip>:18789 <token>
treecore meet node ping my-mac                   # confirm reachable
```

Run `treecore meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `treecore meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `TREECORE_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `treecore_meet_src`). |
...
Local:
```
$TREECORE_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$TREECORE_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `treecore meet node approve`.
