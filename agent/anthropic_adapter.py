...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in treecore_cli/runtime_provider.py.
    """
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See treecore-agent#13848, #17057.
    """
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to treecore's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to treecore's outer conversation
    ...
    # ``~/.treecore/.env``). The result is dual auth —
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``treecore models``) never mutates ``~/.treecore/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      5. Anthropic credential_pool OAuth entry (~/.treecore/auth.json)
...
# Stores credentials in ~/.treecore/.anthropic_oauth.json (our own file).
...
def _get_treecore_oauth_file() -> Path:
    return get_treecore_home() / ".anthropic_oauth.json"
...
def run_treecore_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from treecore_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_treecore_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Hermes-managed OAuth credentials from ~/.treecore/.anthropic_oauth.json."""
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See treecore-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See treecore-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See treecore-agent#13848 (Kimi) and
    treecore-agent#16748 (DeepSeek).
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("Hermes agent", "Claude Code")
                text = text.replace("treecore-agent", "claude-code")
