...
1. **Built-in providers** (``BUILTIN_STT_PROVIDERS`` in
   ...
   ``TREECORE_LOCAL_STT_COMMAND`` is preserved via the built-in
   ``local_command`` path.
...
``~/.treecore/plugins/transcription/<name>/`` (user-installed).
...
class TranscriptionProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``treecore tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``treecore setup`` for
