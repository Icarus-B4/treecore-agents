...
import treecore_cli.auth as auth_mod
...
def _write_through_provider_state_to_global_root(
    ...
    ``treecore_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``treecore model`` / ``treecore auth`` which
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``treecore`` invocation that defaults to the active provider
        (e.g. setup wizard, ``treecore auth status``) would land on
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``TREECORE_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "TREECORE_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "TREECORE_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("treecore_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("treecore_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # re-authed via `treecore model` / `treecore auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `treecore model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``treecore auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `treecore auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `treecore auth remove <provider> <N>` is stable across all source types.
    try:
        from treecore_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        # API-key vs OAuth is a user-visible choice at `treecore setup` ("Claude
        ...
        # Claude Code CLI, treecore_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.treecore/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `treecore setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"treecore_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_treecore_oauth_credentials

        for source_name, creds in (
            ("treecore_pkce", read_treecore_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `treecore auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (treecore auth remove copilot gh_cli) must not pay either on
        ...
        try:
            from treecore_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.treecore/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `treecore auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `treecore auth remove openai-codex` marks
        ...
        # via `treecore auth openai-codex`.
        ...
        # When the user logs in via ``treecore model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``treecore auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from treecore_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.treecore/.env over os.environ — the user's config file is the
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_treecore_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `treecore auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.treecore/.env.
    ...
    try:
        from treecore_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from treecore_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.treecore/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `treecore auth` command that confirmed the source is gone).
