...
def _resolve_args() -> list[str]:
    raw = os.getenv("TREECORE_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set TREECORE_HOME explicitly if they
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        ...
                        "  export TREECORE_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
                        "Alternative: use the `copilot` provider (no ACP, hits the Copilot API\n"
                        "directly with a Copilot subscription token) via `treecore setup`.\n\n"
                        f"Original error:\n{stderr_text}"
                    )
