...
from treecore_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.treecore/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    TREECORE_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``TREECORE_KANBAN_TASK`` / ``TREECORE_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("TREECORE_KANBAN_TASK") or os.getenv("TREECORE_KANBAN_BOARD"):
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``treecore_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``TREECORE_PLATFORM`` or
            ``TREECORE_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("TREECORE_PLATFORM")
        or get_session_env("TREECORE_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``treecore`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.treecore/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``treecore`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against TREECORE_HOME, not cwd
        if not p.is_absolute():
            p = (treecore_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.treecore/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.treecore/skills/`` (including via symlinks) or configured
    ...
    # ~/.treecore/skills/<name> where <name> is a symlink to a checked-out
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    treecore = metadata.get("treecore") or {}
    if not isinstance(treecore, dict):
        treecore = {}
    return {
        "fallback_for_toolsets": treecore.get("fallback_for_toolsets", []),
        "requires_toolsets": treecore.get("requires_toolsets", []),
        "fallback_for_tools": treecore.get("fallback_for_tools", []),
        "requires_tools": treecore.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    treecore = metadata.get("treecore")
    if not isinstance(treecore, dict):
        return []
    raw = treecore.get("config")
    if not raw:
        return []
