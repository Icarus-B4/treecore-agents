...
def _treecore_home_path() -> Path:
    """Resolve the active TREECORE_HOME (profile-aware) without circular imports."""
    try:
        from treecore_constants import get_treecore_home  # local import to avoid cycles
        return get_treecore_home()
    except Exception:
        return Path(os.path.expanduser("~/.treecore"))


def _treecore_root_path() -> Path:
    """Resolve the Hermes root dir (always the parent of any profile, never per-profile)."""
    try:
        from treecore_constants import get_default_treecore_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    treecore_home = _treecore_home_path()
    treecore_root = _treecore_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(treecore_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(treecore_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(treecore_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(treecore_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(treecore_home / "cache" / "bws_cache.enc.json"),
            str(treecore_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved TREECORE_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("TREECORE_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    treecore_dirs = []
    for base in (_treecore_home_path(), _treecore_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in treecore_dirs:
                treecore_dirs.append(real)
        except Exception:
            continue

    for base_real in treecore_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside TREECORE_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Hermes cache files under ``TREECORE_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under TREECORE_HOME and the global Hermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.treecore/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active TREECORE_HOME (profile-aware) AND the global
    # Hermes root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (TREECORE_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    treecore_dirs: list[Path] = []
    for base in (_treecore_home_path(), _treecore_root_path()):
        try:
            real = base.resolve()
            if real not in treecore_dirs:
...
#
# Hermes profiles are separate TREECORE_HOME dirs under
...
#
# Reference: May 2026 incident where a treecore-security profile session
# edited skills under both ``~/.treecore/profiles/treecore-security/skills/``
# AND ``~/.treecore/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under TREECORE_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from TREECORE_HOME.

    ``~/.treecore``              -> ``"default"``
    ``~/.treecore/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _treecore_home_path().resolve()
        root_real = _treecore_root_path().resolve()
...
#
#   <TREECORE_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.treecore/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.treecore/…`` segment and warns
...
# (the agent's view inside the container is plain ``/root/.treecore/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.treecore`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.treecore/…`` and returns the
    ...
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .treecore / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".treecore":
            return i + 4
    return None
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    Otherwise returns a dict with:
      ...
      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.treecore``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.treecore`` (what the
    ...
    TREECORE_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"under the real TREECORE_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.treecore/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.treecore/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host TREECORE_HOME)
    """
...
def get_container_mirror_warning(
    ...
    return (
        ...
        f"{info['inner_path']!r} under the real TREECORE_HOME. Use the "
