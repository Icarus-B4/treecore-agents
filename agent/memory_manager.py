...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        from toolsets import _TREECORE_CORE_TOOLS

        _core_tool_names = set(_TREECORE_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``treecore_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_treecore_home()`` themselves.
        """
        if "treecore_home" not in kwargs:
            from treecore_constants import get_treecore_home
            kwargs["treecore_home"] = str(get_treecore_home())
        for provider in self._providers:
            try:
