...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in treecore_state.py — kept local
# to avoid importing treecore_state at module load time (its module-level
# DEFAULT_DB_PATH = get_treecore_home() / "state.db" breaks tests that
# monkeypatch get_treecore_home to return a str).
...
from treecore_constants import PARTIAL_STREAM_STUB_ID
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.treecore/prefill.json incident, 20/20 blocked with
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `treecore fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from treecore_cli.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.treecore/.env credential/base-url edits made since the last
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from treecore_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from treecore_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("TREECORE_DUMP_REQUESTS"):
                ...
                from treecore_cli.middleware import run_llm_execution_middleware
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: treecore auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `treecore doctor` for credential-chain diagnostics, or")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: treecore config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: treecore config set ANTHROPIC_API_KEY \"\"")
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from treecore_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `treecore model`."
                        )
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`treecore",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    # recovery arms on the TRUE request that overflowed. See treecore-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `treecore auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `treecore model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: treecore portal", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: treecore setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        treecore fallback add   (interactive picker — same as `treecore model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.treecore/config.yaml` to extend the per-call "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                # timeout (TREECORE_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from treecore_cli.lifecycle import has_hook
                    from treecore_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("TREECORE_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
