...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`treecore_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.treecore/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``TREECORE_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``treecore_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
from treecore_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``treecore_cli.config.load_config``
    ...
    setting.  ``TREECORE_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / TREECORE_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("TREECORE_SAFE_MODE"):
        logger.info("TREECORE_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / TREECORE_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``treecore hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`treecore_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_treecore_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / TREECORE_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``TREECORE_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("TREECORE_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `treecore hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``treecore hooks test`` and ``treecore hooks doctor``.

    ``kwargs`` is the same dict that :func:`treecore_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``treecore hooks test`` could
