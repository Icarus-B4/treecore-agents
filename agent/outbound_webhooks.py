...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``TREECORE_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.treecore/config.yaml``)::

    hooks:
      outbound:
        - url: https://ci.example.com/treecore-events
          events: [on_session_end, subagent_stop]
          # secret literal (discouraged) or env var name (preferred):
          secret_env: TREECORE_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `treecore hooks list`
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("TREECORE_SAFE_MODE"):
        logger.info("TREECORE_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
