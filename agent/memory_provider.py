...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `treecore backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - treecore_home (str): The active TREECORE_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.treecore``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "treecore").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'treecore memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], treecore_home: str) -> None:
        ...
        Called by 'treecore memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``treecore_home`` is the active TREECORE_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE TREECORE_HOME.

        ``treecore backup`` only walks TREECORE_HOME, so any provider state kept
        ...
        of the archive, and ``treecore import`` restores them to their original
