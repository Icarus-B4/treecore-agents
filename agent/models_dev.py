...
Data resolution order:
  ...
  2. Disk cache (~/.treecore/models_dev_cache.json — any age; stale data is
...
def _get_cache_path() -> Path:
    ...
    return get_treecore_home() / "models_dev_cache.json"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``treecore config refresh``, the
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from treecore_cli.models import normalize_provider
