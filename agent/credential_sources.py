...
Hermes seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.treecore/.env
    claude_code   — ~/.claude/.credentials.json
    treecore_pkce   — ~/.treecore/.anthropic_oauth.json
    ...
    manual        — user ran `treecore auth add`
...
unify here is **removal**:
    ...
    ``treecore auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), treecore_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.treecore/.env  → clear it
    ...
    """
    from treecore_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.treecore/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Hermes.",
            f"  The pool entry is now suppressed — Hermes will ignore "
            f"{env_var} until you run `treecore auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `treecore auth add anthropic` to re-enable if needed.",
    ])


def _remove_treecore_pkce(provider: str, removed) -> RemovalResult:
    """~/.treecore/.anthropic_oauth.json is ours — delete it outright."""
    from treecore_constants import get_treecore_home

    result = RemovalResult()
    oauth_file = get_treecore_home() / ".anthropic_oauth.json"
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `treecore auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `treecore auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``treecore auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `treecore model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    via ``treecore auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `treecore auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `treecore auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `treecore auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="treecore_pkce",
        remove_fn=_remove_treecore_pkce,
        description="~/.treecore/.anthropic_oauth.json",
    ))
    register(RemovalStep(
