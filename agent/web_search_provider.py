...
``kind: backend``) or ``~/.treecore/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`treecore_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.treecore/.env``) so credentials set
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``treecore tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``treecore tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``treecore_cli/tools_config.py`` to inject this provider as a
