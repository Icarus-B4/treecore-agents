...
* The ``iron-proxy`` binary is auto-installed into ``<treecore_home>/bin/iron-proxy``
...
* A long-lived CA at ``<treecore_home>/proxy/ca.{crt,key}`` is generated on
  first ``treecore egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<treecore_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``treecore egress start``), pidfile
  at ``<treecore_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<treecore_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``treecore egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<treecore_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "TREECORE_IRON_PROXY_MGMT_KEY"
...
def _treecore_bin_dir() -> Path:
    from treecore_constants import get_treecore_home

    return get_treecore_home() / "bin"
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.treecore/proxy/`` just to
    check whether a pidfile exists.
    """
    from treecore_constants import get_treecore_home

    return get_treecore_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<treecore_home>/bin/iron-proxy``  (our managed copy — preferred)
    ...
    managed = _treecore_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``treecore proxy install`` surface lets them
    propagate so the wizard can show a clear error.
    """

    bin_dir = _treecore_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="treecore-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "treecore-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``treecore egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="treecore-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
        ...
        subprocess.run(  # noqa: S603
            [
                ...
                "-key", str(tmp_key),
                "-sha256", "-days", "3650",
                "-subj", "/CN=treecore iron-proxy CA",
...
def mint_proxy_token(prefix: str = "treecore-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<treecore_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``TREECORE_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``treecore egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="treecore-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `treecore egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `treecore egress setup` and use "
            "`treecore egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `treecore egress setup`, "
            "then `treecore egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``treecore egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `treecore egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<treecore_home>/proxy/proxy.yaml``.
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``treecore egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``treecore egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_TREECORE_IRON_PROXY_NONCE_ENV = "TREECORE_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_TREECORE_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `treecore egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `treecore egress setup` first."
        )
    ...
    env[_TREECORE_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # ``treecore egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``treecore egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `treecore egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`treecore egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `treecore secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
