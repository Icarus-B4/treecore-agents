...
Hermes stores sessions in its own SQLite store (``treecore_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``treecore trace upload`` subcommand calls
...
DEFAULT_DATASET_NAME = "treecore-traces"
_TREECORE_VERSION = "treecore-agent"
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.treecore/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `treecore trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/treecore-traces`` dataset. Returns a
