...
Storage
-------
Bundles live in ``~/.treecore/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``TREECORE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``treecore bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under TREECORE_HOME.

    Honors ``TREECORE_BUNDLES_DIR`` for tests; falls back to
    ``<TREECORE_HOME>/skill-bundles``.
    """
    override = os.environ.get("TREECORE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_treecore_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `treecore bundles` CLI subcommand.
# ---------------------------------------------------------------------------
