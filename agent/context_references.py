...
_SENSITIVE_TREECORE_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    treecore_home = get_treecore_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(treecore_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(treecore_home / rel for rel in _SENSITIVE_TREECORE_DIRS)
    ...
    # message text into reference expansion, so `@file:~/.treecore/auth.json` from a
