...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the TREECORE_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "TREECORE_HOME", "TREECORE_PROFILE", "TREECORE_GATEWAY_LOCK_DIR",
    "TREECORE_MAX_ITERATIONS", "TREECORE_MAX_TOKENS", "TREECORE_API_TIMEOUT",
    "TREECORE_REDACT_SECRETS", "TREECORE_NOUS_TIMEOUT_SECONDS",
    "_TREECORE_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "TREECORE_KANBAN_DB", "TREECORE_KANBAN_WORKSPACES_ROOT", "TREECORE_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "TREECORE_KANBAN_",
    "TREECORE_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``treecore_cli.config._parse_env_value``) but never mutates the process
    ...
    from treecore_cli.config import _parse_env_value
...
def build_profile_secret_scope(treecore_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(treecore_home)
    secrets = load_env_file(home / ".env")

    try:
        from treecore_cli.env_loader import get_secret_source_values
