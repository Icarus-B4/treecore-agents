...
``treecore pause`` writes a sentinel file at ``$TREECORE_HOME/ESTOP``;
``treecore resume`` removes it. While the sentinel exists:
...
hold even if the file was created by ``touch ~/.treecore/ESTOP``.
...
def _treecore_home() -> Path:
    """Resolve the active TREECORE_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active TREECORE_HOME."""
    return _treecore_home() / SENTINEL_NAME
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            ...
            "run `treecore resume` to pick things back up."
        )
    return (
