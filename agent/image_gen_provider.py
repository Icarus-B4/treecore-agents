...
as ``kind: backend``) or ``~/.treecore/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``treecore tools``
...
def _images_cache_dir() -> Path:
    """Return ``$TREECORE_HOME/cache/images/``, creating parents as needed."""
    from treecore_constants import get_treecore_home

    path = get_treecore_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$TREECORE_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$TREECORE_HOME/cache/images/``.
