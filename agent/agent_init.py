...
from treecore_cli.timeouts import get_provider_request_timeout
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from treecore_cli.providers import TREECORE_OVERLAYS, get_provider

        overlay = TREECORE_OVERLAYS.get(provider)
    ...
    try:
        ...
        from treecore_cli.models import normalize_provider as normalize_model_provider
        from treecore_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: treecore config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$TREECORE_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_treecore_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$TREECORE_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .treecore.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / TREECORE_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.treecore/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from treecore_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind TREECORE_DEV_CREDITS.
    ...
    # both live under ~/.treecore/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from treecore_logging import setup_logging, setup_verbose_logging
    setup_logging(treecore_home=_ra()._treecore_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that treecore_logging.setup_logging() does not
        ...
        # handler level inside treecore_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from treecore_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from treecore_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = treecore_xai_default_headers()
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `treecore model` to "
                        "select a provider, or run `treecore setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff TREECORE_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["TREECORE_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.treecore/sessions/ alongside gateway sessions
    treecore_home = get_treecore_home()
    agent.logs_dir = treecore_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.treecore/sessions/session_{sid}.json)
    ...
    try:
        from treecore_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `treecore --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from treecore_cli.config import load_config_readonly as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "treecore"
                    except Exception:
                        pass
    ...
    if codex_app_server_auto_compaction not in {"native", "treecore", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, treecore, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from treecore_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from treecore_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from treecore_cli.plugins import get_plugin_context_engine
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from treecore_cli.model_switch import _check_treecore_model_warning

            _treecore_warn = _check_treecore_model_warning(agent.model or "")
            if _treecore_warn:
                _user_msg = (
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_treecore_warn)
        except Exception:
            pass
