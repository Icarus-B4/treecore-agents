...
# Mirror treecore_cli.config.DEFAULT_CONFIG["compression"] keys of the same name.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``treecore_state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["TREECORE_SESSION_ID"] = child_session_id
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `treecore setup` or set OPENROUTER_API_KEY."
                )
...
def compress_context(
    ...
    *,
    ...
    # ``compression.codex_app_server_auto`` (native|treecore|off).
    ...
    try:
        ...
        if agent._session_db:
            ...
            try:
                ...
                if in_place:
                    ...
                    # whose thread lacks the TREECORE_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("TREECORE_SESSION_SOURCE", "cli"),
                        model=agent.model,
                    ...
                    try:
                        from treecore_cli.goals import migrate_goal_to_session
                        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
                    except Exception as _goal_err:
                        logger.debug("Could not migrate goal on compression: %s", _goal_err)
                    # Same boundary hazard for /heartbeat state — carry it too.
                    try:
                        from treecore_cli.heartbeat import migrate_heartbeat_to_session
        ...
        # engines (e.g. treecore-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See treecore-lcm#68. Built-in ContextCompressor
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    if auto_mode not in {"native", "treecore", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "treecore":
        logger.info(
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="treecore_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
