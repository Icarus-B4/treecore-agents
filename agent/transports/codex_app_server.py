...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        spawn_env = treecore_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("TREECORE_KANBAN_TASK"):
            kanban_db = spawn_env.get("TREECORE_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "TREECORE_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("TREECORE_HOME", os.path.expanduser("~/.treecore")),
                        "kanban",
                    ),
                )
            )
