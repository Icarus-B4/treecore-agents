...
MCP server (per `~/.codex/config.toml [mcp_servers.treecore-tools]`) and
...
Scope (what we expose):
  ...
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    ...
    unblock/link)                          write ~/.treecore/kanban.db)
...
Run with: python -m agent.transports.treecore_tools_mcp_server
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on TREECORE_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.treecore/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
    ...
    # only — the kanban tool gates them on TREECORE_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    ...
    try:
        ...
        raise ImportError(
            f"treecore-tools MCP server requires the 'mcp' package: {exc}"
        ) from exc
    ...
    logger.info(
        "treecore-tools MCP server registered %d/%d tools",
        exposed_count,
        len(EXPOSED_TOOLS),
    )
    return mcp
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.treecore_tools_mcp_server`."""
    ...
    os.environ.setdefault("TREECORE_QUIET", "1")
    os.environ.setdefault("TREECORE_REDACT_SECRETS", "true")

    try:
        server = _build_server()
    except ImportError as exc:
        sys.stderr.write(f"treecore-tools MCP server cannot start: {exc}\n")
        return 2
    ...
    try:
        ...
        logger.exception("treecore-tools MCP server crashed")
        sys.stderr.write(f"treecore-tools MCP server error: {exc}\n")
        return 1
    return 0
