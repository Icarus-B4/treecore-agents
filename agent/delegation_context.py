...
TREECORE_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's TREECORE_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "treecore_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "TREECORE_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "TREECORE_KANBAN_TASK",
    "TREECORE_KANBAN_RUN_ID",
    "TREECORE_KANBAN_WORKSPACE",
    "TREECORE_KANBAN_WORKSPACES_ROOT",
    "TREECORE_KANBAN_CLAIM_LOCK",
    "TREECORE_KANBAN_BOARD",
    "TREECORE_KANBAN_DB",
)
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``TREECORE_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$TREECORE_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``TREECORE_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``TREECORE_KANBAN_*`` vars while losing the ContextVar in
