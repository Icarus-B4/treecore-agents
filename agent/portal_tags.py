...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        "product=treecore-agent",
        "client=treecore-client-v<__version__>",
    ]

The version is sourced live from ``treecore_cli.__version__`` so it auto-aligns
...
``treecore_cli.__version__`` is the canonical source of truth.
"""
...
def _treecore_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``treecore_cli`` cannot be imported (should
...
def treecore_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=treecore-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=treecore-client-v{_treecore_version()}"
...
def conversation_tag(session_id: str) -> str:
    ...
    used for ``~/.treecore/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=treecore-agent", treecore_client_tag()]
