...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    ...
    ``home_path`` is whatever ``load_treecore_dotenv()`` already resolved;
    falling back to ``$TREECORE_HOME`` / ``~/.treecore`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from treecore_constants import get_treecore_home

        home_path = get_treecore_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<treecore_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if TREECORE_HOME is ever made traversable.
            try:
