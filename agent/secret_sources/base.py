...
``~/.treecore/.env`` has loaded and BEFORE the rest of Hermes reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per TREECORE_HOME) by the orchestrator in
...
_SOURCE_ENVIRONMENT = ContextVar("treecore_secret_source_environment", default=None)
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``treecore secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``treecore secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved TREECORE_HOME.
        """
    ...
    def remediation(self, kind: Optional["ErrorKind"], cfg: dict) -> str:
        ...
        Called by the startup status printer (and ``treecore secrets ...
        ...
        *what to run* next to fix it — not just what broke.  Sources
        ...
        ``treecore secrets bitwarden token`` for AUTH_FAILED).  Return an
        ...
        generic = {
            ErrorKind.NOT_CONFIGURED: (
                f"Run `treecore secrets {self.name} setup` to finish configuration."
            ),
            ErrorKind.BINARY_MISSING: (
                f"Run `treecore secrets {self.name} setup` to install the helper CLI."
            ),
            ErrorKind.AUTH_FAILED: (
                f"Credentials rejected — run `treecore secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.AUTH_EXPIRED: (
                f"Credentials expired — run `treecore secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.NETWORK: (
