...
so they don't have to live in plaintext in ``~/.treecore/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<treecore_home>/bin/bws`` on
...
* The access token is stored in ``~/.treecore/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``treecore`` invocations don't
  hammer the API.
...
# In-process cache so repeated load_treecore_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `treecore chat -q ...`
...
# <treecore_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.treecore/.env (which
...
_ENCRYPTED_CACHE_INFO = b"treecore-bws-encrypted-cache-v1"
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under treecore_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$TREECORE_HOME` / `~/.treecore` when home is None.
    """
    return _DISK_CACHE.path(home_path)


def _encrypted_disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the encrypted disk cache path under treecore_home/cache/."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<treecore_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``treecore secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="treecore-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
# ---------------------------------------------------------------------------
# Public entry point — called from treecore_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_treecore_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `treecore secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `treecore secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `treecore secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `treecore secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`treecore secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`treecore secrets bitwarden token`) so the
