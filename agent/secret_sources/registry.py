...
``treecore_cli.env_loader._apply_external_secret_sources()``.
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``treecore`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's TREECORE_HOME is ``~/.treecore/profiles/<name>``; the
    default profile (``~/.treecore``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("TREECORE_PROFILE_NAME", "TREECORE_PROFILE"):
