...
are cached in-process and on disk under ``<treecore_home>/cache/op_cache.json``
so back-to-back short-lived ``treecore`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a TREECORE_HOME switch
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_treecore_dotenv()`` after the .env files have loaded.
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            ...
            if isinstance(cfg, dict):
                token_env = str(cfg.get("service_account_token_env") or token_env)
            return (
                "Run `treecore secrets onepassword token` to paste a fresh "
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`treecore secrets onepassword token`) so
