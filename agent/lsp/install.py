...
``<TREECORE_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``treecore lsp
  status``.
...
# ``<TREECORE_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `treecore lsp status` reports the host's presence.
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(treecore_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $TREECORE_HOME/node is not on an arbitrary process's
    ...
    if npm is None:
        ...
        return None
    staging = treecore_lsp_bin_dir().parent  # <TREECORE_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = treecore_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a treecore-owned target dir.
    ...
    console script.
    """
    pip_target = treecore_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = treecore_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``treecore lsp status`` CLI to give users a quick
