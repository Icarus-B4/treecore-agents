...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_treecore_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``TREECORE_PLATFORM`` env var and
    ``TREECORE_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("TREECORE_PLATFORM")
            or get_session_env("TREECORE_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("TREECORE_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.treecore.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_treecore_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.treecore/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.treecore/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``treecore -s <skill>`` or
    a deployment's ``TREECORE_TUI_SKILLS`` env var could force-load a skill an
