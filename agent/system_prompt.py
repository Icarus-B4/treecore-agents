...
upstream prefix cache warm.  See ``treecore-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``TREECORE_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("TREECORE_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Some execution modes (cron) still want TREECORE_HOME persona while keeping
    ...
    # Pointer to the treecore-agent skill + docs for user questions about Hermes itself.
    stable_parts.append(TREECORE_AGENT_HELP_GUIDANCE)
    ...
    # TREECORE_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # under so it doesn't conflate ~/.treecore/skills/ (default profile) with
    # ~/.treecore/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            ...
            "under " + str(get_treecore_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            ...
            f"and writes {get_treecore_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_treecore_home()}/skills/, {get_treecore_home()}/plugins/, "
            f"{get_treecore_home()}/cron/, {get_treecore_home()}/memories/ — those belong to a "
