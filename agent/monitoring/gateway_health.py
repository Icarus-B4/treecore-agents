...
def _base_attrs(*, profile: str, install_id: str, version: str, supervision_mode: str) -> Dict[str, str]:
    ...
    return {
        ...
        "treecore.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("treecore.gateway.up", 1 if gateway_running else 0, base),
        _metric("treecore.gateway.active_agents", active_agents, base),
        _metric("treecore.gateway.busy", 1 if busy else 0, base),
        _metric("treecore.gateway.drainable", 1 if drainable else 0, base),
        _metric("treecore.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("treecore.gateway.state", 1, base, **{"treecore.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"treecore.platform": str(platform), "treecore.platform.state": state},
        ))
        metrics.append(_metric(
            "treecore.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"treecore.platform": str(platform), "treecore.platform.state": state, "treecore.error_code": error_code},
        ))
        if is_degraded:
