...
from treecore_constants import get_treecore_home, get_skills_dir, is_wsl
...
_TREECORE_MD_NAMES = (".treecore.md", "HERMES.md")


def _find_treecore_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.treecore.md`` or ``HERMES.md``.
    ...
    # could pick up a .treecore.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _TREECORE_MD_NAMES:
...
TREECORE_AGENT_HELP_GUIDANCE = (
    "You run on Hermes Agent (by Nous Research). When the user needs help with "
    "Hermes itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://treecore-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `treecore-agent` skill with skill_view(name='treecore-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.treecore/kanban.db`. Your task id is in "
    "`$TREECORE_KANBAN_TASK`; your workspace is `$TREECORE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $TREECORE_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $TREECORE_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${TREECORE_KANBAN_BRANCH:-wt/$TREECORE_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$TREECORE_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`treecore profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `treecore kanban <verb>` for board operations. Use "
...
#
# The treecore-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to treecore-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `treecore computer-use doctor` and share the output. That command "
...
def build_environment_hints() -> str:
    ...
    extra = (os.getenv("TREECORE_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_treecore_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("TREECORE_PLATFORM") or os.environ.get("TREECORE_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.treecore/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "skills, voice, gateway, plugins, or any feature — load the `treecore-agent` skill "
            "first. It has the actual commands (e.g. `treecore config set …`, `treecore tools`, "
            "`treecore setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: treecore setup, treecore setup tools, treecore setup terminal, treecore status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from TREECORE_HOME and return its content, or None.
    ...
    """
    try:
        from treecore_cli.config import ensure_treecore_home
        ensure_treecore_home()
    except Exception as e:
        logger.debug("Could not ensure TREECORE_HOME before loading SOUL.md: %s", e)

    soul_path = get_treecore_home() / "SOUL.md"
...
def _load_treecore_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.treecore.md / HERMES.md — walk to git root."""
    treecore_md_path = _find_treecore_md(cwd_path)
    if not treecore_md_path:
        return ""
    try:
        content = treecore_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = treecore_md_path.name
        try:
            rel = str(treecore_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".treecore.md", context_length=context_length,
            read_path=str(treecore_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", treecore_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .treecore.md / HERMES.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from TREECORE_HOME is independent and always included when present.
    ...
    if (
        ...
        project_context = (
            _load_treecore_md(cwd_path, context_length)
    ...
    # SOUL.md from TREECORE_HOME only — skip when already loaded as identity
    if not skip_soul:
