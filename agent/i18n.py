...
Language resolution order:
    ...
    2. ``TREECORE_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``TREECORE_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("TREECORE_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "TREECORE_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a treecore dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`treecore_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("TREECORE_LANGUAGE")
    if env_lang:
